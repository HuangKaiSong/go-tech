...
@go-tech/service 的答案：**一个 Adapter 接口 + 两个工厂函数**。
...
```ts
import { createAppRequest } from '@go-tech/service';
...
import type { RequestAdapter } from '@go-tech/service';
...
返回值与 `@go-tech/axios` 的 `createRequest` 一致，是一个可直接调用的函数：
...
- **零平台依赖** — 核心逻辑只依赖 `@go-tech/axios` 和 `@tanstack/react-query`，不依赖任何 UI 库 / 路由 / 状态管理
