...
function getPackageRoot() {
  ...
  throw new Error('Cannot resolve @go-tech/scripts package root.');
}
...
async function generateRouteTree(targetDir: string) {
  const routeTreeTmpDir = await mkdtemp(path.join(tmpdir(), 'go-tech-route-tree-'));
...
async function checkAdminTemplate(sourceDir: string, targetDir: string) {
  const tempDir = await mkdtemp(path.join(tmpdir(), 'go-tech-admin-template-'));
