...
go-tech Admin CLI 工具集 — 通过统一入口 `sa` 命令，封装 changelog 生成、版本发布、规范化 Git 提交、依赖更新、清理构建产物等项目自动化脚本。
...
`@go-tech/scripts` 的答案：**一条命令 + 交互式引导 + 配置文件驱动**。
...
`@go-tech/scripts` 通过 [c12](https://github.com/unjs/c12) 加载配置，支持以下配置文件（位于项目根目录）：
...
```ts
// soybean.config.ts
import { defineConfig } from '@go-tech/scripts';
