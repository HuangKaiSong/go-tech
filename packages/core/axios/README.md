...
```ts
import { BACKEND_ERROR_CODE } from '@go-tech/axios';
...
在实际项目中，通常不直接使用 `@go-tech/axios`，而是通过 `@go-tech/service` 进行上层封装。它基于本包提供了**平台适配器模式**、**业务状态码管理**、**Token 自动刷新**、**错误消息去重**等开箱即用的能力。
