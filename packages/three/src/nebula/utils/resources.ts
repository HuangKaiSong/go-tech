...
export default class Resources extends EventEmitter {
  ...
  loadingPercentage: HTMLElement | null = null;
  ...
  constructor(sources: Source[], options = {}) {
    super();
  ...
  setLoaders() {
    ...
    // Set up KTX2Loader
    this.loaders.ktx2Loader.setTranscoderPath(this.options.ktx2TranscoderPath).detectSupport(this.renderer.instance);
  ...
  startLoading() {
    for (const source of this.sources) {
      switch (source.type) {
        ...
        case 'audio': {
          this.loaders.audioLoader!.load(source.path as string, file => {
            this.sourceLoaded(source, file);
          });
          break;
        }
        case 'objModel': {
          this.loaders.objLoader!.load(source.path as string, file => {
            this.sourceLoaded(source, file);
          });
          break;
        }
        case 'hdrTexture': {
          this.loaders.hdrTextureLoader!.load(source.path as string, file => {
            this.sourceLoaded(source, file);
          });
          break;
        }
        case 'svg': {
          this.loaders.svgLoader!.load(source.path as string, file => {
            this.sourceLoaded(source, file);
          });
          break;
        }
        ...
        case 'video': {
          this.loadVideoTexture(source.path as string).then(file => {
            this.sourceLoaded(source, file);
          });
          break;
        }
        case 'ktx2Texture': {
          this.loaders.ktx2Loader!.load(source.path as string, file => {
            this.sourceLoaded(source, file);
          });
          break;
        }
