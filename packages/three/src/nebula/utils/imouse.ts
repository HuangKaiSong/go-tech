...
import { detectDeviceType } from '../tools/dom';
...
export default class IMouse {
  ...
  getMouseScreen(x: number, y: number): THREE.Vector2 {
    return new THREE.Vector2(x - this.sizes.width / 2, -(y - this.sizes.height / 2));
  }
  ...
  listenForDesktop() {
    window.addEventListener('mousemove', event_ => {
      ...
      this.normalizedMouse = this.getNormalizedMouse(event_.clientX, event_.clientY);
    });
  }
   ...
   */
  updateTouchPosition(touch: Touch) {
    ...
    this.normalizedMouse = this.getNormalizedMouse(touch.clientX, touch.clientY);
  }
  ...
  judgeIsMouseMoving() {
    this.isMouseMoving =
      Math.abs(this.mouseDOMDelta.x) >= this.mouseMoveOffset || Math.abs(this.mouseDOMDelta.y) >= this.mouseMoveOffset;
  }
