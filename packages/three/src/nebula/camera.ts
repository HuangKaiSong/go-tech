...
export default class Camera {
  ...
  throttle(func: (this: Camera, ...args: any[]) => void, limit: number): (...args: any[]) => void {
    let inThrottle = false;
    // oxlint-disable-next-line func-names
    return function (this: Camera, ...args: any[]) {
      const context = this;
      if (!inThrottle) {
  ...
  resize() {
    if (this.orthographic) {
      ...
      (this.instance as THREE.OrthographicCamera).left = (-this.frustumSize * aspect) / 2;
      (this.instance as THREE.OrthographicCamera).right = (this.frustumSize * aspect) / 2;
      (this.instance as THREE.OrthographicCamera).top = this.frustumSize / 2;
      (this.instance as THREE.OrthographicCamera).bottom = -this.frustumSize / 2;

      this.instance.updateProjectionMatrix();
    } else {
      (this.instance as THREE.PerspectiveCamera).aspect = this.sizes.width / this.sizes.height;
      this.instance.updateProjectionMatrix();
    }
