...
interface Params {
  ...
  /** 光晕缩放 */
  haloScale: number
  mouseLerpSpeed: number // 鼠标跟随插值速度（0-1，值越大跟随越快）
  animated: boolean // 动画旋转
  anamorphic: boolean // 变形镜头效果
  secondaryGhosts: boolean // 次级幽灵
  starBurst: boolean // 星爆效果（性能消耗大）
  ghostScale: number // 幽灵缩放
  aditionalStreaks: boolean // 额外条纹
  followMouse: boolean // 跟随鼠标
}
