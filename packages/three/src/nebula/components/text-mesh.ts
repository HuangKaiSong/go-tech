...
export default class TextMesh {
  ...
  constructor(options: TextMeshOptions = {}) {
    ...
    const { font, fontOptions, force, position, rotation, texts, totalMass } = {
      ...defaultOptions,
      ...options
    };
    ...
    for (const [index, text] of this.texts.entries()) {
      ...
      for (const [index_, letter] of [...text].entries()) {
        ...
        if (letter === ' ') {
          ...
          const box = new CANNON.Box(new CANNON.Vec3(mesh.size.x, mesh.size.y, mesh.size.z).scale(0.5));

          const initialPosition = new CANNON.Vec3(words.letterOff, this.getOffsetY(index), 0);
  ...
  setConstraints(): void {
    for (const group of this.textGroups) {
      ...
      for (let index = 0; index < letters.length; index += 1) {
        ...
        const nextLetter = index === letters.length - 1 ? null : letters[index + 1];

        // oxlint-disable-next-line no-continue
        if (!nextLetter) continue;

        // 使用 ConeTwistConstraint，因为它比其他约束更刚性，适合我们的目的
        const constraint = new CANNON.ConeTwistConstraint(letter.body, nextLetter.body, {
  ...
  onClick(): void {
    ...
    if (intersects.length > 0) {
      ...
      const { face, object } = object_;

      // oxlint-disable-next-line no-use-before-define
      if (!isMesh(object)) return;
      if (!face) return;

      const impulse = new THREE.Vector3().copy(face.normal).negate().multiplyScalar(this.force);
      ...
      for (const group of this.textGroups) {
        for (const letter of group.children as LetterMesh[]) {
          ...
          body.applyLocalImpulse(new CANNON.Vec3(impulse.x, impulse.y, impulse.z), new CANNON.Vec3());
        }
      }
    }
  }
  ...
  updateGroup(group: TextGroup, _groupIndex: number): void {
    let shouldReset = false;
    let shouldAddGround = false;

    for (const [, letter] of (group.children as LetterMesh[]).entries()) {
      ...
      if (!group.isGroundDisplayed && letter.body.position.y + letter.body.initPosition.y <= 10) {
