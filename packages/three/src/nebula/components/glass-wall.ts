...
export default class GlassWall {
  ...
  bgScene: THREE.WebGLRenderTarget;
  targetLightPosition: THREE.Vector3;
  targetPoint: THREE.Vector2;
  maxDistance: number;
  initialChromaticAberration: number;

  glassWallGroup: THREE.Group | null = null;
  glassMaterial: THREE.ShaderMaterial | null = null;
  glassMaterial2: THREE.MeshPhysicalMaterial | null = null;
  ...
  constructor() {
    ...
    this.bgScene = new THREE.WebGLRenderTarget(this.sizes.width, this.sizes.height);
  ...
  updateLightPositionSmooth(): void {
    ...
    if (this.glassMaterial) {
      this.glassMaterial.uniforms.uChromaticAberration.value = chromaticAberration;
    }
  }
  ...
  setGlassWall(): void {
    ...
    const glassGeometry = new RoundedBoxGeometry(THICK, HEIGHT, THICK, 128, 0.2);
  ...
  update(): void {
    ...
    if (this.glassMaterial && this.glassWallGroup) {
  ...
  debuggerInit(): void {
    if (this.debugActive && this.debug && this.glassMaterial) {
