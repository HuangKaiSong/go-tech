...
type PlanetSpeedConfig = {
  baseSpeed: number;
  speedMultiplier: number;
};

type PlanetParamsOverride = Partial<{
  ambientLight: string;
  ambientLightIntensity: number;
...
type PlanetConfig = {
  angleOffset: number;
  name: string;
  params?: PlanetParamsOverride;
  radius: number;
  texture: string;
};

type HarmonicInfluence = {
  angleOffset: number;
  radialOffset: number;
  skewOffset: number;
...
type TrajectoryPoint = Vec3 & { time: number };

type SpeedMode = 'custom' | 'size_based' | 'uniform';

type MotionPresetName = 'chaos' | 'classic' | 'flower' | 'spiral' | 'stable';
type SpeedPresetName = 'artistic' | 'chaotic' | 'harmonic' | 'physics_based' | 'uniform';
type HarmonicPresetName = 'chaotic' | 'golden' | 'lissajous' | 'mandala';

type TweakpaneChangeEvent<T> = { value: T };
...
export default class Planets {
  ...
  planets: PlanetWithMeta[];
  motionParams: MotionParams;
  harmonicsGlobal: HarmonicsGlobal;
  harmonicsConfig: HarmonicsGroup[];
  sharedLighting: SharedLighting;
  sharedAtmosphereIntensity: number;
  sharedAtmosphereThickness: number;
  timeOffset: number;
  showTrajectories: boolean;
  trajectories: TrajectoryPoint[][];
  planetSpeeds: PlanetSpeedConfig[];
  speedMode: SpeedMode;
  ...
  calculatePseudoThreeBodyPosition(planetIndex: number, time: number): Vec3 {
    // 为三个星体配置不同的椭圆轨道参数
    const ellipseConfigs = [
      ...
      {
        // Planet C (中等) - 中层轨道
        a: Number(this.motionParams.baseRadius), // 中等长轴
  ...
  // 计算谐波叠加对星体轨道的影响
  getHarmonicInfluence(planetIndex: number, baseAngle: number): HarmonicInfluence {
  ...
  // 记录轨迹点
  recordTrajectory(planetIndex: number, position: Vec3): void {
  ...
  debugInit(): void {
    ...
    ambientFolder
      .addBinding(this.sharedLighting, 'ambientLight', {
        label: '环境光颜色',
        picker: 'inline'
      })
      .on('change', ({ value }: TweakpaneChangeEvent<string>) => {
        this.sharedLighting.ambientLight = value;
        this.propagateLighting();
      });

    ambientFolder
      .addBinding(this.sharedLighting, 'ambientLightIntensity', {
        label: '环境光强度',
        min: 0.0,
        max: 3.0,
        step: 0.1
    ...
    pointFolder
      .addBinding(this.sharedLighting, 'pointLightColor', {
        label: '点光源颜色',
        picker: 'inline'
      })
      .on('change', ({ value }: TweakpaneChangeEvent<string>) => {
    ...
    pointFolder
      .addBinding(this.sharedLighting, 'pointLightIntensity', {
        label: '点光源强度',
        min: 0.0,
        max: 5.0,
        step: 0.1
    ...
    lightPosFolder
      .addBinding(this.sharedLighting, 'pointLightPosition', {
        label: '光源位置',
        min: -5,
        max: 5,
        step: 0.1
    ...
    // 大气强度（共享）
    atmosFolder
      .addBinding(this, 'sharedAtmosphereIntensity', {
        label: '大气强度',
        min: 0.0,
        max: 10.0,
        step: 0.1
      ...
      .on('change', (e: TweakpaneChangeEvent<number>) => {
        if (typeof e?.value === 'number') this.sharedAtmosphereIntensity = e.value;
    ...
    // 大气厚度（共享）
    atmosFolder
      .addBinding(this, 'sharedAtmosphereThickness', {
        label: '大气厚度',
        min: 0.5,
        max: 10.0,
        step: 0.1
      ...
      .on('change', (e: TweakpaneChangeEvent<number>) => {
        if (typeof e?.value === 'number') this.sharedAtmosphereThickness = e.value;
        this.planets.forEach(p => {
          if (p.atmosphereMaterial) {
    ...
    basicFolder.addBinding(this.motionParams, 'baseRadius', {
      label: '基础半径',
      min: 2.0,
      max: 8.0,
      step: 0.1
    ...
    basicFolder.addBinding(this.motionParams, 'primarySpeed', {
      label: '运动速度',
      min: 0.1,
      max: 1.0,
      step: 0.05
    ...
    basicFolder
      .addBinding(this.motionParams, 'yOffsetScale', {
        label: 'Y轴错开程度',
        min: 0.0,
        max: 5.0,
        step: 0.1
    ...
    perturbFolder.addBinding(this.motionParams, 'perturbationRadius', {
      label: '扰动幅度',
      min: 0.0,
      max: 3.0,
      step: 0.1
    ...
    perturbFolder.addBinding(this.motionParams, 'perturbationSpeed', {
      label: '扰动频率',
      min: 0.1,
      max: 2.0,
      step: 0.1
    ...
    perturbFolder.addBinding(this.motionParams, 'harmonicCount', {
      label: '谐波数量',
      min: 1,
      max: 5,
      step: 1
    ...
    interactionFolder.addBinding(this.motionParams, 'interactionStrength', {
      label: '作用强度',
      min: 0.0,
      max: 2.0,
      step: 0.1
    ...
    interactionFolder.addBinding(this.motionParams, 'interactionPhase', {
      label: '相位差',
      min: 0.0,
      max: 1.0,
      step: 0.05
    ...
    complexityFolder.addBinding(this.motionParams, 'chaosStrength', {
      label: '混沌强度',
      min: 0.0,
      max: 1.0,
      step: 0.05
    ...
    complexityFolder.addBinding(this.motionParams, 'verticalAmplitude', {
      label: '垂直幅度',
      min: 0.0,
      max: 2.0,
      step: 0.1
    ...
    complexityFolder.addBinding(this.motionParams, 'verticalFreq', {
      label: '垂直频率',
      min: 0.1,
      max: 2.0,
      step: 0.1
    ...
    controlFolder
      .addBinding(this, 'showTrajectories', {
        label: '显示轨迹'
      })
      .on('change', () => {
        if (!this.showTrajectories) {
    ...
    ] as const satisfies ReadonlyArray<{ text: string; value: MotionPresetName }>;
    ...
    speedFolder
      ...
      .on('change', (e: TweakpaneChangeEvent<SpeedMode>) => {
    ...
    this.planetSpeeds.forEach((speedConfig, index) => {
      ...
      planetFolder
        .addBinding(speedConfig, 'speedMultiplier', {
          label: '速度倍数',
          min: 0.2,
          max: 2.0,
          step: 0.1
    ...
    ] as const satisfies ReadonlyArray<{ text: string; value: SpeedPresetName }>;
    ...
    harmonicsFolder
      .addBinding(this.harmonicsGlobal, 'enabled', {
        label: '启用共振'
      })
      .on('change', () => {
        this.clearTrajectories();
    ...
    harmonicsFolder
      .addBinding(this.harmonicsGlobal, 'strength', {
        label: '全局强度',
        min: 0.0,
        max: 2.0,
        step: 0.05
    ...
    this.harmonicsConfig.forEach((config, planetIndex) => {
      ...
      config.harmonics.forEach((harmonic, harmonicIndex) => {
        ...
        harmonicFolder
          .addBinding(harmonic, 'radialStrength', {
            label: '径向强度',
            min: -0.6,
            max: 0.6,
            step: 0.01
        ...
        harmonicFolder
          .addBinding(harmonic, 'angleStrength', {
            label: '角向强度',
            min: -0.6,
            max: 0.6,
            step: 0.01
        ...
        harmonicFolder
          .addBinding(harmonic, 'skewStrength', {
            label: '椭圆偏置',
            min: -0.6,
            max: 0.6,
            step: 0.01
        ...
        harmonicFolder
          .addBinding(harmonic, 'verticalStrength', {
            label: '垂直强度',
            min: -0.4,
            max: 0.4,
            step: 0.01
        ...
        harmonicFolder
          .addBinding(harmonic, 'verticalPhase', {
            label: '垂直相位',
            min: 0,
            max: Math.PI * 2,
            step: 0.01
    ...
    ] as const satisfies ReadonlyArray<{ text: string; value: HarmonicPresetName }>;

    harmonicPresets.forEach(preset => {
      harmonicPresetFolder
        .addButton({
          title: preset.text
        })
        .on('click', () => {
  ...
  // 获取特定星体的运动速度
  getPlanetSpeed(planetIndex: number): number {
  ...
  // 预设速度配置
  applySpeedPreset(presetName: SpeedPresetName): void {
  ...
  getPlanetInfo(index: number): { mass: string; name: string; radius: number } {
    const configs = [
      { name: 'Planet A', radius: RADIUS * 1.2, mass: 'Large' },
      { name: 'Planet B', radius: RADIUS * 0.7, mass: 'Small' },
      { name: 'Planet C', radius: Number(RADIUS), mass: 'Medium' }
    ];
    return configs[index];
  }

  // 应用谐波预设，快速生成具有特色的多频共振轨道
  applyHarmonicPreset(presetName: HarmonicPresetName): void {
    const TWO_PI = Math.PI * 2;

    const applyConfig = (
      updater: (harmonic: HarmonicConfig, context: { harmonicIndex: number; planetIndex: number }) => void
    ) => {
      this.harmonicsConfig.forEach((config, planetIndex) => {
        if (!config) return;
        config.enabled = true;
    ...
    switch (presetName) {
      ...
      case 'lissajous':
        ...
        applyConfig((harmonic, context) => {
          ...
          harmonic.radialStrength = 0.1 + 0.03 * context.harmonicIndex;
      ...
      case 'mandala':
        ...
        applyConfig((harmonic, context) => {
          ...
          harmonic.angleStrength = -0.2 + context.harmonicIndex * 0.06;
  ...
  // 随机化运动参数
  randomizeMotionParameters(): void {
  ...
  // 应用运动预设
  applyMotionPreset(presetName: MotionPresetName): void {
    // oxlint-disable-next-line default-case
    switch (presetName) {
      case 'classic':
  ...
  // 获取特定星球
  getPlanet(index: number): PlanetWithMeta | undefined {
    return this.planets[index];
  }

  // 获取所有星球
  getAllPlanets(): PlanetWithMeta[] {
    return this.planets;
  }
