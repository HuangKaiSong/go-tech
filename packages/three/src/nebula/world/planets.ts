...
type SpeedMode = 'uniform' | 'size_based' | 'custom'

type MotionPresetName = 'classic' | 'stable' | 'chaos' | 'spiral' | 'flower'
type SpeedPresetName = 'uniform' | 'physics_based' | 'artistic' | 'harmonic' | 'chaotic'
type HarmonicPresetName = 'golden' | 'lissajous' | 'mandala' | 'chaotic'
...
export default class Planets {
  ...
  // 辅助方法：获取星体配置信息
  getPlanetInfo(index: number): { name: string; radius: number; mass: string } {
    const configs = [
  ...
  applyHarmonicPreset(presetName: HarmonicPresetName): void {
    ...
    const applyConfig = (
      updater: (harmonic: HarmonicConfig, context: { planetIndex: number; harmonicIndex: number }) => void,
    ) => {
      this.harmonicsConfig.forEach((config, planetIndex) => {
        if (!config)
          return
