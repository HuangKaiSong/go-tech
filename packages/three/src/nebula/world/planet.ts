...
export default class Planet {
  ...
  planet: THREE.Mesh<THREE.IcosahedronGeometry, THREE.ShaderMaterial> | null;
  radius: number;
  textureName: string;
  ...
  params: PlanetParams;
  geometry: THREE.IcosahedronGeometry | null = null;
  material: THREE.ShaderMaterial | null = null;

  atmosphereGeometry: THREE.IcosahedronGeometry | null = null;
  atmosphereMaterial: THREE.ShaderMaterial | null = null;
  atmosphere: THREE.Mesh<THREE.IcosahedronGeometry, THREE.ShaderMaterial> | null = null;

  lightIndicatorGeometry: THREE.IcosahedronGeometry | null = null;
  lightIndicatorMaterial: THREE.MeshBasicMaterial | null = null;
  lightIndicator: THREE.Mesh<THREE.IcosahedronGeometry, THREE.MeshBasicMaterial> | null = null;

  lightingDebugFolder: FolderApi | null = null;
  ...
  init(): void {
    // 创建星球几何体（增加细分以支持置换）
    this.geometry = new THREE.IcosahedronGeometry(this.radius, this.experience.quality.planetDetail);

    // 设置主纹理 & 各项异性过滤
    const texture = this.resources.items[this.textureName] as THREE.Texture;
    ...
    // 设置法线贴图 & 各项异性过滤
    const normalMap = this.resources.items.planetNormal as THREE.Texture;
    normalMap.generateMipmaps = true;
    normalMap.minFilter = THREE.LinearMipMapLinearFilter;
    normalMap.anisotropy = 8;

    // 设置置换贴图 & 各项异性过滤
    const displacementMap = this.resources.items.planetDisplacement as THREE.Texture;
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        // 点光源位置
        uPointLightPosition: {
          value: new THREE.Vector3(this.params.pointLightPosition.x, 1, this.params.pointLightPosition.y)
        },
  ...
  createAtmosphere(): void {
    ...
    this.atmosphereGeometry = new THREE.IcosahedronGeometry(atmosphereRadius, this.experience.quality.planetDetail);
    ...
    this.atmosphereMaterial = new THREE.ShaderMaterial({
      ...
      uniforms: {
        // 点光源位置
        uPointLightPosition: {
          value: new THREE.Vector3(this.params.pointLightPosition.x, 0, this.params.pointLightPosition.y)
        },
  ...
  // 更新光源指示器位置
  updateLightIndicatorPosition(): void {
    if (this.lightIndicator) {
      this.lightIndicator.position.set(this.params.pointLightPosition.x, 0, this.params.pointLightPosition.y);
    }
  }

  // 更新光源指示器颜色
  updateLightIndicatorColor(): void {
  ...
  // 设置自转速度
  setRotationSpeed(x: number, y: number, z = 0): void {
    this.rotation.x = x;
    this.rotation.y = y;
    this.rotation.z = z;
  }
  ...
  debuggerInit(): void {
    ...
    // 大气层颜色
    atmosphereFolder
      .addBinding(this.params, 'atmosphereDayColor', {
        label: '白天大气颜色',
        picker: 'inline'
      })
      .on('change', ({ value }: { value: THREE.Color }) => {
    ...
    // 黄昏大气颜色
    atmosphereFolder
      .addBinding(this.params, 'atmosphereTwilightColor', {
        label: '黄昏大气颜色',
        picker: 'inline'
      })
      .on('change', ({ value }: { value: THREE.Color }) => {
  ...
  applyLightingPreset(presetName: string): void {
    ...
    if (this.atmosphereMaterial) {
      ...
      this.atmosphereMaterial.uniforms.uAtmosphereTwilightColor.value = new THREE.Color(
        this.params.atmosphereTwilightColor
      );
  ...
  destroy(): void {
    ...
    if (this.lightIndicator) {
      ...
      if (this.lightIndicatorGeometry) {
