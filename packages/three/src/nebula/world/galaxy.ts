...
export default class Galaxy {
  ...
  pixelRatio: Experience['sizes']['pixelRatio'];
  debug: Experience['debug']['ui'];
  debugActive: Experience['debug']['active'];
  time: Experience['time'];
  textures: Experience['resources']['items'];

  parameters: Parameters;
  material: THREE.ShaderMaterial;
  galaxy: THREE.Points | null = null;
  ...
  setGalaxy() {
    ...
    for (let i = 0; i < this.parameters.count; i += 1) {
      ...
      const ringConstraint =
        (1.0 - distanceToRingCenter ** this.parameters.ringFalloff) * this.parameters.constraintStrength;
  ...
  debuggerInit() {
    if (this.debugActive) {
      ...
      ringFolder
        .addBinding(this.parameters, 'innerRadius', {
          min: 0.0,
          max: 2.0,
          step: 0.1,
          label: '内环半径'
      ...
      ringFolder
        .addBinding(this.parameters, 'ringFalloff', {
          min: 0.1,
          max: 5.0,
          step: 0.1,
          label: '环形衰减'
      ...
      ringFolder
        .addBinding(this.parameters, 'constraintStrength', {
          min: 0.0,
          max: 3.0,
          step: 0.05,
          label: '约束强度'
