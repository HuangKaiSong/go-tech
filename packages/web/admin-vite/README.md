# @go-tech/web-admin-vite

go-tech Admin 的 Vite 配置预设包，用来把 admin 应用里重复的构建、代理、插件、别名和开发服务器配置收口到一个共享入口。
...
```bash
pnpm --filter @go-tech/web-admin-vite typecheck
pnpm --filter @go-tech/web-admin-vite build
```
