...
```bash
pnpm add @go-tech/materials
```

**依赖**：`@go-tech/color`、`@go-tech/ui`、`@go-tech/utils`
...
```tsx
import { AdminLayout, LAYOUT_SCROLL_EL_ID } from '@go-tech/materials';
...
```tsx
import { PageTab } from '@go-tech/materials';
...
```ts
import { LAYOUT_SCROLL_EL_ID } from '@go-tech/materials';
...
颜色混合来自 `@go-tech/color` 的 `transformColorWithOpacity` 与 `addColorAlpha`。
...
- [`@go-tech/web-admin-theme`](../admin-theme) — 主题状态、预设系统、Ant Design 集成
- [`@go-tech/color`](../../@core/color) — 核心色板生成与颜色工具
- [`@go-tech/ui`](../ui) — 基础 UI 元素
