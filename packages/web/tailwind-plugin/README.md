...
- **语义化色阶** — `primary` / `destructive` / `success` / `warning` / `info` / `carbon` 自动展开 50–950 全色阶 + `DEFAULT` + `foreground`，由 `@go-tech/color` 推导
...
```bash
pnpm add -D @go-tech/tailwind-plugin tailwindcss
```
...
- 这份文件需要**手工维护**，不要尝试用脚本基于 `@go-tech/color` 重新生成（surface 色无法从 primary 推导）
- 运行时 `primary` / `destructive` / `success` / `warning` / `info` / `carbon` 的 50–950 色阶仍然通过 `@go-tech/color` 的 `getColorPalette` 实时算出（见 [generate.ts](src/generate.ts)）
...
- [`@go-tech/color`](../../@core/color) — OKLCH 色板生成与色彩工具
- [`@go-tech/web-admin-theme`](../admin-theme) — React 主题状态 + Ant Design 集成
- [`@go-tech/adapter-antd-theme`](../antd-theme) — Ant Design 主题算法适配
