...
import { Settings, Users, FileText, Building, Droplets, Clock, Calendar, LayoutDashboard, Package, Eye, ShoppingCart, Plus, Minus, ArrowUpCircle, Check } from "lucide-react";
...
const upgradePlans = [
  {
    ...
    price: 3200,
    features: ["管理層", "代理列表", "客戶列表", "合同列表", "單位列表", "水電列表", "跟進列表", "日程", "dashboard"],
  },
  {
    id: "plan-c",
    name: "套餐C",
    ...
    price: 6800,
    features: ["管理層", "代理列表", "客戶列表", "合同列表", "單位列表", "水電列表", "跟進列表", "日程", "dashboard", "高級報表", "API接口", "多公司管理"],
  },
];
...
const mockOrders = [
  {
    ...
    addons: [
      { name: "額外50個單位", quantity: 2, price: "$500" },
    ],
...
const MyOrders = () => {
  ...
  const [paymentType, setPaymentType] = useState<"addons" | "upgrade">("addons");
  ...
  const handlePaymentSelect = (method: string) => {
    if (paymentType === "addons") {
      console.log("購買增值服務:", { method, orderId: selectedOrderId, services: selectedServices });
    } else {
      console.log("套餐升級:", { method, orderId: selectedOrderId, upgradePlan: selectedUpgradePlan });
    }
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            {mockOrders.length === 0 ? (
              ...
              mockOrders.map((order) => (
                <Card key={order.id} className="border border-border hover:shadow-lg transition-shadow">
                  <CardHeader className="pb-4">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <Settings className="w-6 h-6 text-primary" />
                          <h3 className="text-xl font-bold text-foreground">{order.planName}</h3>
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-primary">
                          {order.price} <span className="text-base">{order.currency}</span>
                        </div>
                        <p className="text-xs text-muted-foreground">訂單編號：{order.id}</p>
                      </div>
                    </div>
                  </CardHeader>
                  ...
                  <CardContent>
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      {/* 包含功能 */}
                      <div>
                        <h4 className="font-medium text-foreground border-b border-foreground/30 pb-2 mb-3">包含功能</h4>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 mt-4">
            ...
            {upgradePlans.map((plan) => {
              const currentOrder = mockOrders.find(o => o.id === selectedOrderId);
              const isCurrentPlan = currentOrder?.planName === plan.name;
              const isLowerPlan = plan.name === "套餐B" && currentOrder?.planName === "套餐C";
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xs text-muted-foreground">HKD/年</span>
                    </div>
                  </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-lg font-bold">
                <span>總計</span>
                <span className="text-primary">${(getUpgradePrice() + calculateUpgradeAddonsTotal()).toLocaleString()} HKD</span>
              </div>
            </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 mt-4">
            <p className="text-center text-muted-foreground">
              應付金額：<span className="text-xl font-bold text-primary">${getPaymentAmount().toLocaleString()} HKD</span>
            </p>
            <div className="space-y-3">
              <button
