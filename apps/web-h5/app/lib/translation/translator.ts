...
const DEFAULT_PROVIDER: TranslationProvider =
  (process.env.TRANSLATION_PROVIDER as TranslationProvider) || "deepl";
...
async function translateWithDeepL(
  ...
  const authKey = '5fe62a6a-be8f-43e3-b04a-85fa4d80f3b8:fx'

  const deeplClient = new deepl.DeepLClient(authKey);
  // oxlint-disable-next-line no-nested-ternary
  const targetLang = locale === 'zh-cn' ? 'zh-HANS' : locale === 'zh-hk' ? 'zh-HANT' : 'en-US'

  // DeepL 支持批量文本（作为数组传入）
  const response = await deeplClient.translateText(texts, null, targetLang);
 ...
 * 批量翻译（对外公开）
 */
export async function translateBatchByAI(
  texts: string[],
  locale: string
): Promise<Record<string, string>> {
  // 去重（避免重复翻译）
