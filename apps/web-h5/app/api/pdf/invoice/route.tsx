...
// ─── Invoice fetch 抽离 ───
async function fetchInvoice(id: string): Promise<{ fetchMs: number; invoice: any } | null> {
  try {
    ...
    if (invoice?.platformPackageDto?.packageItemList) {
      invoice.platformPackageDto.packageItemList = invoice.platformPackageDto.packageItemList.filter(
...
async function generatePdf(id: string): Promise<GenerateResult | null> {
  ...
  const useCjkFont = sourceTexts.some(v => typeof v === 'string' && CJK_TEXT_RE.test(v));
