...
import { Button, Dialog, DialogContent, DialogHeader, DialogTitle, Empty, Separator, toast, UploadedFile } from '@go-tech-frontend/ui';
...
import { OrderItemInfoType, OrderItemTypeEnum, OrderTypeEnum, PlatformPackageDto } from '../constants/order';
...
export const Upgrade: FC<UpgradeProps> = ({
  ...
  const getUpgradeProrationInfo = () => {
    ...
    const totalDays = dyas || expiry.diff(dayjs(new Date(order.activateDate || order.payTime!)), "day") + 1;
    ...
    return { months, ratio, newExpiryDate, remainingCredit, usedDays, remainingDays, totalDays };
  };
  ...
  const handleFpsPaymentConfirm = async (voucherFile: UploadedFile) => {
    ...
    Object.entries(upgradeSelectedServices).map(
      ([serviceId, quantity]) => {
  ...
  useEffect(() => {
    // 获取可升级套餐
    fetch('/go-tech/platform/platformPackage/enabledList').then(res => res.json()).then(res => {
        ...
        <DialogContent className="sm:max-w-lg max-h-[calc(100dvh-2rem)] flex flex-col overflow-hidden">
          ...
          {(() => {
            if (upgradePlans.length === 0) {
              return (
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="沒有可用的升級方案" />
              )
            } else {
              return (
                <div className="space-y-4 mt-4 flex-1 min-h-0 overflow-y-auto pr-1">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {plan.packageItemList.filter(feature => feature.level < 2).slice(0, 6).map((feature, idx) => (
                            <span
                              key={idx}
                  ...
                  {/* 費用匯總 */}
                  {(() => {
                    const { months, newExpiryDate, remainingCredit, usedDays, remainingDays, totalDays } = getUpgradeProrationInfo();
                    ...
                    return (
                      <div className="space-y-2">
                        <div className="p-3 rounded-lg bg-[#FFF8F5] border text-xs text-muted-foreground space-y-1">
                          <div>
                            升級後按原訂單時長 <span className="font-medium text-foreground">{months} 個月</span> 重新計費，
                            新到期日：<span className="font-medium text-foreground">{newExpiryDate}</span>。
                          </div>
                          <div>
                            原套餐已使用 <span className="font-medium text-foreground">{usedDays}</span> / {totalDays} 天，
                            剩餘 <span className="font-medium text-foreground">{remainingDays}</span> 天，
                            可抵扣餘額 <span className="font-medium text-foreground">${remainingCredit?.toLocaleString()} HKD</span>。
                          </div>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-lg font-bold">
                          <span>總計</span>
                          <span className="text-primary">${fmt(getUpgradePrice() + calculateUpgradeAddonsTotal())} HKD</span>
                        </div>
                      </div>
                    );
                  })()}
