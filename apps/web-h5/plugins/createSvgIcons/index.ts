...
async function parseSvgToSymbol(compiler: SVGCompiler, svgCode: string, id: string, path: string) {
...
export async function createSvgSpriteHtml(opt: SvgIconsOpt): Promise<SvgSpriteResult> {
  ...
  const files = await fg(
    iconDirs.map(dir => `${normalizePath(dir)}/**/*.svg`),
    {
      onlyFiles: true,
      absolute: true,
      unique: true
  ...
  for (const file of files) {
    ...
    const optimized = options.svgoOptions ? optimize(content, { path: file, multipass: true }).data : content;
