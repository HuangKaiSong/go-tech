...
/** 携带已翻译 message 的 HTTP 错误。 服务端 error.message 始终为中文，httpFetch 在非 2xx 时自动翻译后抛出。 */
export class HttpError extends Error {
  ...
  constructor(message: string, init: { data: unknown; response: Response; status: number }) {
    super(message);
    this.name = 'HttpError';
...
/**
 * Fetch 封装：签名与原生 fetch 一致。 - 2xx：原样返回 Response - 非 2xx：解析 body 中的 message，翻译为当前语言后抛出 HttpError
 *
 * @example
 *   try {
