...
import {
  Building2, ArrowLeft, Edit2, Save, X, Users, FileText, Shield, ChevronRight,
  Layers, DollarSign,
} from "lucide-react";
...
import {
  getPositionById, savePosition, getPositionMembers,
  type Position, type PositionMember,
} from "@/api/position";
import { getDepartmentOptions, type DepartmentOption } from "@/api/department";
import {
  getMenuTree, getPositionMenuIds, assignPositionMenus, type MenuTreeNode,
} from "@/api/menu";
...
export default function RoleDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      loadPosition(),
      getPositionMembers(id).then((res) => setMembers(res.data ?? [])).catch(() => setMembers([])),
      getDepartmentOptions().then((res) => setDepts(res.data ?? [])).catch(() => {}),
      getMenuTree().then((res) => setMenuTree(res.data ?? [])).catch(() => {}),
      getPositionMenuIds(id).then((res) => setMenuChecked(new Set(res.data ?? []))).catch(() => {}),
  ...
  const startEdit = () => { if (position) { setForm(toForm(position)); setEditing(true); } };
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {position.level && <Badge variant="outline" className={levelColors[position.level] || ""}>{position.level}</Badge>}
              <Badge variant="secondary" className={position.statusCode === 1 ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}>
                {t(position.status)}
              </Badge>
            </div>
        ...
        {editing ? (
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={cancelEdit} disabled={saving}>
              <X className="h-4 w-4 mr-1" />{t("取消")}
            </Button>
            <Button size="sm" onClick={handleSave} disabled={saving}>
              <Save className="h-4 w-4 mr-1" />{saving ? t("儲存中…") : t("儲存")}
            </Button>
          </div>
        ) : (
          hasPerm(POSITION_PERM.EDIT) && (
            <Button variant="outline" size="sm" onClick={startEdit}>
              <Edit2 className="h-4 w-4 mr-1" />{t("編輯")}
            </Button>
          )
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: t("在職人數"), value: String(position.memberCount ?? members.length), icon: Users, color: "text-primary" },
          ...
          <Card id="basic">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                <Field label={t("職位名稱")} required editing={editing}
                  view={position.title}
                  edit={<Input value={form?.title ?? ""} onChange={(e) => setForm((f) => f && { ...f, title: e.target.value })} />} />
                <Field label={t("職位代碼")} editing={editing}
                  view={position.code || "—"}
                  edit={<Input value={form?.code ?? ""} onChange={(e) => setForm((f) => f && { ...f, code: e.target.value })} />} />
                <Field label={t("所屬部門")} required editing={editing}
                  view={position.departmentName || "—"}
                  edit={
                    <Select value={form?.departmentId != null ? String(form.departmentId) : ""}
                      onValueChange={(v) => setForm((f) => f && { ...f, departmentId: v ? Number(v) : undefined })}>
                      <SelectTrigger><SelectValue placeholder={t("選擇部門")} /></SelectTrigger>
                      <SelectContent>
                        {depts.map((d) => <SelectItem key={d.id} value={String(d.id)}>{d.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  } />
                <Field label={t("職等")} editing={editing}
                  view={position.level || "—"}
                  edit={
                    <Select value={form?.level ?? ""} onValueChange={(v) => setForm((f) => f && { ...f, level: v })}>
                      <SelectTrigger><SelectValue placeholder={t("選擇職等")} /></SelectTrigger>
                      <SelectContent>
                        {LEVEL_OPTIONS.map((l) => <SelectItem key={l} value={l}>{l}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  } />
                <Field label={t("薪資範圍")} editing={editing}
                  view={position.salaryRange || "—"}
                  edit={<Input placeholder={t("如：40K-65K")} value={form?.salaryRange ?? ""} onChange={(e) => setForm((f) => f && { ...f, salaryRange: e.target.value })} />} />
                <Field label={t("狀態")} editing={editing}
                ...
                <Field label={t("薪資下限 (HK$)")} editing={editing}
                  view={position.salaryMin != null ? String(position.salaryMin) : "—"}
                  edit={<Input type="number" value={form?.salaryMin ?? ""} onChange={(e) => setForm((f) => f && { ...f, salaryMin: e.target.value })} />} />
                <Field label={t("薪資上限 (HK$)")} editing={editing}
                  view={position.salaryMax != null ? String(position.salaryMax) : "—"}
                  edit={<Input type="number" value={form?.salaryMax ?? ""} onChange={(e) => setForm((f) => f && { ...f, salaryMax: e.target.value })} />} />
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between text-base">
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-normal text-muted-foreground">{t("已選 {{checked}} / {{total}}", { checked: menuChecked.size, total: countNodes(menuTree) })}</span>
                </span>
                {hasPerm(POSITION_PERM.ASSIGN_MENU) && (
                  <Button size="sm" onClick={handleSaveMenus} disabled={menuSaving}>
                    <Save className="h-4 w-4 mr-1" />{menuSaving ? t("保存中…") : t("保存授權")}
                  </Button>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-xs text-muted-foreground mb-2">{t("勾選該職位可存取的菜單與按鈕，勾選目錄會級聯選中其下全部子項。")}</p>
