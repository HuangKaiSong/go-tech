...
import {
  ...
  User, Loader2, AlertCircle, RotateCcw, Upload, Paperclip, X, File,
} from "lucide-react";
import {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-muted-foreground">
        ...
        <div className="flex gap-2 mt-3">
          ...
          <Button variant="link" size="sm" onClick={() => navigate("/employees/onboarding")}>{t("返回列表")}</Button>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-5">
            ...
            <Avatar className="h-16 w-16 text-lg">
              <AvatarFallback className="bg-primary/10 text-primary font-bold">{(detail.employeeName || "").slice(-2)}</AvatarFallback>
            </Avatar>
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                <span className="flex items-center gap-1"><Briefcase className="h-3 w-3" />{detail.onboardingNo}</span>
                <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{t("入職 {{date}}", { date: detail.planDate || "—" })}</span>
        ...
        <div className="w-72 shrink-0 space-y-4">
          ...
          <Card>
            <CardContent className="pt-5 space-y-3">
              ...
              <InfoLine label={t("試用期")} value={detail.probation != null ? t("{{n}} 個月", { n: detail.probation }) : "—"} />
