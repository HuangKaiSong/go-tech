...
import { type EmployeeUpdatePayload, getEmployeeById, updateEmployee } from '@/api/employee';
  ...
  const display =
    typeof value === 'boolean'
      ? value
        ...
        : t('否')
      : selectOpts && value
        ? t(optLabel(String(value)))
        : String(value || '—');
  const inferredType =
    type ||
    (selectOpts ? 'select' : typeof value === 'boolean' ? 'boolean' : typeof value === 'number' ? 'number' : 'text');
...
function DeptPositionEditable({
  ...
  const Cell = ({ children, label }: { children: React.ReactNode; label: string }) => (
    <div className="min-w-0">
      <div className="flex items-center gap-1 mb-0.5">
        <p className="text-xs text-muted-foreground truncate">{label}</p>
  ...
  if (!isEditing) {
    return (
      <>
        <Cell label={t('部門')}>
          <p className="text-sm font-medium truncate">{emp.department || '—'}</p>
        </Cell>
        <Cell label={t('職位')}>
          <p className="text-sm font-medium truncate">{emp.position || '—'}</p>
        </Cell>
      </>
    );
  }
  return (
    <>
      <Cell label={t('部門')}>
        ...
        >
          <SelectTrigger className="h-8 text-sm">
            <SelectValue placeholder={t('請選擇部門')} />
          </SelectTrigger>
          <SelectContent>
            {departments.map(d => (
      ...
      <Cell label={t('職位')}>
        ...
        >
          <SelectTrigger className="h-8 text-sm">
            <SelectValue placeholder={deptId ? t('請選擇職位') : t('請先選擇部門')} />
          </SelectTrigger>
          <SelectContent>
            {positions.map(p => (
...
function ScheduleEditable({
  ...
  const scheduleId: string =
    rawScheduleId || (fallbackName ? String(schedules.find(s => s.name === fallbackName)?.id ?? '') : '');

  // 以绑定的 scheduleId 为准，从班次表取当前班次；取不到再回退冗余字段
  const current = schedules.find(s => String(s.id) === scheduleId);
  const shiftType: string = current
    ? (SCHEDULE_TYPE_TEXT[current.type] ?? '—')
    : String(formData.shiftType ?? emp.shiftType ?? '') || '—';
  const shiftCode: string = current ? current.name : String(formData.shiftCode ?? emp.shiftCode ?? '') || '—';
  ...
  if (!isEditing) {
    return (
      <>
        <Cell label={t('班次類型')}>
          <p className="text-sm font-medium truncate">{t(shiftType)}</p>
        </Cell>
        <Cell label={t('班次名稱')}>
          <p className="text-sm font-medium truncate">{shiftCode}</p>
        </Cell>
      </>
    );
  }
...
export default function EmployeeDetail() {
    ...
    getEmployeeById(numId)
      .then(res => {
        if (res.data) {
          ...
          const clean = Object.fromEntries(Object.entries(mapped).filter(([, v]) => v != null && v !== ''));
  ...
  const fp = (
    fieldKey: string,
    label: string,
    opts?: { colSpan?: number; type?: 'text' | 'number' | 'date' | 'select' | 'boolean' }
  ) => ({
    label,
    fieldKey,
    value: emp[fieldKey],
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <Badge variant="secondary" className={statusMap[emp.status]?.color || ''}>
                  <span
                    className={`inline-block h-1.5 w-1.5 rounded-full mr-1.5 ${statusMap[emp.status]?.dot || ''}`}
                  />
