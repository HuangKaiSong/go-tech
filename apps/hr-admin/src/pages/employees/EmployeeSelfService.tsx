...
const initialForm: SelfForm = {
  name: "", englishName: "", alias: "", gender: "男", birthday: "", nationality: "", maritalStatus: "",
  idNumber: "", idCardUrl: "", phone: "", email: "", address: "",
...
/** 居中提示卡片（加載中/失效/成功） */
function CenterCard({ icon: Icon, iconClass, title, desc }: {
  icon: any; iconClass: string; title: string; desc: React.ReactNode;
}) {
  return (
...
export default function EmployeeSelfService() {
  ...
  const handleIdFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!okType) { toast.error("僅支援圖片或 PDF 檔案"); return; }
    if (file.size > 10 * 1024 * 1024) { toast.error("檔案大小不可超過 10MB"); return; }
    setUploading(true);
    try {
  ...
  const removeIdFile = () => { set("idCardUrl", ""); setIdFileName(""); };
  ...
  const handleSubmit = async () => {
    // 必填校驗（按頁面順序逐項提示第一個缺失）
    const required: { v: string; msg: string }[] = [
    ...
    for (const r of required) {
      if (!r.v.trim()) { toast.error(r.msg); return; }
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) {
      toast.error("電子郵箱格式不正確"); return;
    }
    setSubmitting(true);
    try {
  ...
  if (loading) {
    return <CenterCard icon={Loader2} iconClass="text-primary animate-spin" title="載入中…" desc="正在校驗邀請連結，請稍候。" />;
  }
  if (invalid) {
  ...
  if (submitted) {
    return (
      <CenterCard
        ...
        title="資料提交成功！"
        desc={<>感謝您填寫個人資料，HR 團隊將會審核您的資訊。<br />如有疑問，請聯繫人事部門。</>}
      />
    );
  }
      ...
      <div className="bg-primary text-primary-foreground py-4 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-sm opacity-80">
            {form.name ? `歡迎 ${form.name}！` : "歡迎！"}請填寫以下個人資料，帶 <span className="text-destructive-foreground font-medium">*</span> 為必填項目
          </p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto py-8 px-4 space-y-6">
        ...
        <SectionCard icon={User} title="基本資料" description="請填寫您的個人基本資訊">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FieldRow label="英文姓名">
              <Input placeholder="請輸入英文姓名（與證件一致）" value={form.englishName} onChange={(e) => set("englishName", e.target.value)} />
            </FieldRow>
            <FieldRow label="暱稱">
            ...
            <FieldRow label="性別">
              <Select value={form.gender} onValueChange={(v) => set("gender", v)}>
                ...
                <SelectContent>{["男", "女", "其他"].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </FieldRow>
            <FieldRow label="出生日期">
              <Input type="date" value={form.birthday} onChange={(e) => set("birthday", e.target.value)} onClick={(e) => e.currentTarget.showPicker?.()} />
            </FieldRow>
            <FieldRow label="國籍" required>
              <Select value={form.nationality || undefined} onValueChange={(v) => set("nationality", v)}>
                <SelectTrigger><SelectValue placeholder="請選擇" /></SelectTrigger>
                <SelectContent>{["香港", "中國", "台灣", "新加坡", "澳洲", "其他"].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </FieldRow>
            <FieldRow label="婚姻狀況">
              <Select value={form.maritalStatus} onValueChange={(v) => set("maritalStatus", v)}>
                <SelectTrigger><SelectValue placeholder="請選擇" /></SelectTrigger>
                <SelectContent>{["未婚", "已婚", "離婚", "喪偶"].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </FieldRow>
          </div>
        </SectionCard>
        ...
        <SectionCard icon={FileText} title="身份證明文件" description="用於勞動合約及社保登記">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FieldRow label="證件號碼" required>
              <Input placeholder="請輸入證件號碼" value={form.idNumber} onChange={(e) => set("idNumber", e.target.value)} />
            </FieldRow>
          </div>
          <div className="mt-4">
        ...
        <SectionCard icon={Phone} title="聯絡資訊">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <FieldRow label="通訊地址">
                <Input placeholder="請輸入完整地址" value={form.address} onChange={(e) => set("address", e.target.value)} />
              </FieldRow>
            </div>
          </div>
        </SectionCard>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <FieldRow label="關係">
              <Select value={form.emergencyContactRelation} onValueChange={(v) => set("emergencyContactRelation", v)}>
                <SelectTrigger><SelectValue placeholder="請選擇" /></SelectTrigger>
                <SelectContent>{["配偶", "父母", "兄弟姐妹", "子女", "朋友", "其他"].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </FieldRow>
            <FieldRow label="聯絡電話">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FieldRow label="最高學歷">
              <Select value={form.education} onValueChange={(v) => set("education", v)}>
                <SelectTrigger><SelectValue placeholder="請選擇" /></SelectTrigger>
                <SelectContent>{["高中/職", "大專", "學士", "碩士", "博士"].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </FieldRow>
            <FieldRow label="畢業學校">
        ...
        <SectionCard icon={Landmark} title="銀行帳戶資訊" description="用於薪資發放">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FieldRow label="帳戶持有人姓名" required>
              <Input placeholder="需與本人姓名一致" value={form.bankAccountHolder} onChange={(e) => set("bankAccountHolder", e.target.value)} />
            </FieldRow>
          </div>
        </SectionCard>
