...
import {
  ArrowLeft, Calendar, Phone, Mail, Briefcase, ClipboardCheck,
...
export default function OffboardingDetail() {
  ...
  const [detail, setDetail] = useState<OffboardingDetailData | null>(null);
  ...
  const fetchDetail = useCallback(async (silent = false) => {
    const id = Number(offId);
    if (!offId || Number.isNaN(id)) {
      setError(t("無效的離職單編號"));
      setLoading(false);
      return;
    }
    ...
    setError("");
    try {
      const res = await getOffboardingById(id);
      setDetail(res.data);
    } catch (err: any) {
      setError(err.message || t("獲取離職單詳情失敗"));
      if (!silent) {
        setDetail(null);
      }
    } finally {
  ...
  const handleToggle = async (task: LifecycleTask) => {
    ...
    if (detail.statusCode === 4 || detail.statusCode === 5) {
      toast.error(detail.statusCode === 4 ? t("該離職單已完成，任務不可修改") : t("該離職單已取消，任務不可修改"));
      return;
    }
    setTogglingId(task.id);
    try {
      const res = await toggleOffboardingTask(task.id, !task.done);
      if (res.data === 4) {
        toast.success(t("全部任務已完成，{{name}} 已正式轉為「已離職」", { name: detail.employeeName }));
      } else {
        toast.success(task.done ? t("已取消完成「{{name}}」", { name: task.name }) : t("已完成「{{name}}」", { name: task.name }));
      }
  ...
  const handleUploadClick = (task: LifecycleTask) => {
    if (!detail) return;
    if (detail.statusCode === 4 || detail.statusCode === 5) {
      toast.error(detail.statusCode === 4 ? t("該離職單已完成，附件不可修改") : t("該離職單已取消，附件不可修改"));
      return;
    }
  ...
  if (error || !detail) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-muted-foreground">
        ...
        <p className="text-lg font-medium">{error || t("找不到此離職記錄")}</p>
        <div className="flex gap-2 mt-3">
          <Button variant="outline" size="sm" onClick={() => fetchDetail()}>
            <RotateCcw className="h-3.5 w-3.5 mr-1.5" />{t("重新載入")}
          </Button>
          <Button variant="link" size="sm" onClick={() => navigate("/employees/offboarding")}>{t("返回列表")}</Button>
        </div>
      </div>
    );
  }
  ...
  const daysUntil = detail.lastWorkDate
    ? Math.ceil((new Date(detail.lastWorkDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
    : 0;
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-5">
            ...
            <Avatar className="h-16 w-16 text-lg">
              <AvatarFallback className="bg-destructive/10 text-destructive font-bold">{(detail.employeeName || "").slice(-2)}</AvatarFallback>
            </Avatar>
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                {daysUntil > 0 && detail.statusCode !== 4 && (
                  <span className="text-xs text-muted-foreground bg-muted/60 px-2 py-0.5 rounded">
                    {t("距最後工作日 {{n}} 天", { n: daysUntil })}
                  </span>
                )}
                {detail.actualDate && (
                  <span className="text-xs text-success bg-success/10 px-2 py-0.5 rounded">
                    {t("實際離職 {{date}}", { date: detail.actualDate })}
                  </span>
                )}
              </div>
              <p className="text-sm text-muted-foreground">{detail.department || "—"} · {detail.position || "—"}</p>
              <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                <span className="flex items-center gap-1"><Briefcase className="h-3 w-3" />{detail.offboardingNo}</span>
                <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{t("最後工作日 {{date}}", { date: detail.lastWorkDate || "—" })}</span>
      ...
      <div className="flex gap-6">
        ...
        <div className="w-72 shrink-0 space-y-4">
          ...
          <Card>
            <CardContent className="pt-5 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium">{t("離職進度")}</p>
                <span className="text-lg font-bold text-primary">{progress}%</span>
              </div>
              <Progress value={progress} className="h-2" />
              <p className="text-xs text-muted-foreground">{t("{{done}} / {{total}} 項任務已完成", { done: doneCount, total: tasks.length })}</p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-5 space-y-3">
              <p className="text-sm font-semibold mb-2">{t("離職資訊")}</p>
              <InfoLine label={t("離職類型")} value={detail.leaveTypeName ? t(detail.leaveTypeName) : "—"} />
              <InfoLine label={t("離職原因")} value={detail.leaveReason ? t(detail.leaveReason) : "—"} />
              <InfoLine label={t("提出日期")} value={detail.applyDate || "—"} />
              <InfoLine label={t("負責 HR")} value={detail.hrOwnerName || "—"} />
              <InfoLine label={t("直屬主管")} value={detail.managerName || "—"} />
              <InfoLine label={t("交接人")} value={detail.handoverToName || t("待指定")} />
              {detail.remark && (
                <>
                  <Separator />
                  <div>
        ...
        <div className="flex-1 min-w-0 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold">
              {activeCategory === "all" ? t("全部任務") : t(activeCategory)}
            </h2>
          ...
          {locked && (
            <div className="rounded-lg border bg-muted/40 px-4 py-2.5 text-xs text-muted-foreground flex items-center gap-2">
              ...
              {detail.statusCode === 4 ? t("該離職單已完成，任務清單已鎖定") : t("該離職單已取消，任務清單已鎖定")}
            </div>
          )}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {!locked && (
                            <Button
                              ...
                              onClick={(e) => { e.stopPropagation(); handleUploadClick(task); }}
                            >
                      ...
                      <div className="flex items-center gap-4 mt-1.5 text-xs text-muted-foreground">
                        ...
                        {task.done && task.doneTime && (
                          <span className="flex items-center gap-1 text-success">
                            ...
                            {task.doneUserName ? `${task.doneUserName} · ` : ""}{task.doneTime}
                          </span>
                        )}
                      ...
                      {task.attachments && task.attachments.length > 0 && (
                        <div className="mt-2.5 space-y-1.5">
                          {task.attachments.map((att) => (
                            <div key={att.id} className="flex items-center gap-2 bg-muted/40 rounded-md px-2.5 py-1.5 text-xs group/file">
                              ...
                              {(att.uploaderName || att.uploadTime) && (
                                <span className="text-muted-foreground shrink-0">
                                  {att.uploaderName ? `${att.uploaderName} · ` : ""}{att.uploadTime || ""}
                                </span>
                              )}
                              {!locked && (
                                <button
