...
import {
  calibratePerfTask,
  getPerfTaskBreakdown,
  getPerfTaskDetail,
  type PerfIndicatorBreakdown,
  type PerfReviewerProgress,
  type PerfTaskStatus,
  submitPerfScore
...
/** 按满分折算 5 星显示 */
function StarRow({ maxScore, score }: { maxScore: number; score: number }) {
...
export default function PerformanceEvalDetail() {
  ...
  const handleSubmit = () => {
    ...
    for (const it of items) {
      ...
      if (Number.isNaN(n) || n < 0 || n > it.maxScore) {
        toast({
          title: t('「{{name}}」評分需在 0~{{max}} 之間', { name: it.name, max: it.maxScore }),
          variant: 'destructive'
        });
        return;
      }
  ...
  const { managerWeight: wMgr, peerWeight: wPeer, selfWeight: wSelf } = data;
  ...
  const peerParticipants = new Set(breakdown.flatMap(c => c.peerReviews.map(p => p.reviewerName).filter(Boolean))).size;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="page-description">
              {data.employeeName} — {data.planName}
            </p>
          </div>
          <Badge variant="secondary" className={`${taskStatusColors[data.taskStatus] || ''} text-sm px-3 py-1`}>
            {t(data.taskStatusText)}
          </Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-medium mt-2">
                  {t('自評 {{self}}% · 互評 {{peer}}% · 主管 {{mgr}}%', { self: wSelf, peer: wPeer, mgr: wMgr })}
                </p>
              </div>
              <div className="text-center">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">{t('評估流程')}</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <div key={i} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {step.na ? t('無此環節') : step.done ? step.date || t('已完成') : t('待進行')}
                    </p>
                  </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={calib.grade}
                  placeholder={t('如 A / B')}
                  onChange={e => setCalib(p => ({ ...p, grade: e.target.value }))}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  value={calib.reason}
                  placeholder={t('與自動值不同時建議填寫')}
                  onChange={e => setCalib(p => ({ ...p, reason: e.target.value }))}
                />
              </div>
            </div>
            <div className="flex justify-end mt-4">
              <Button
                ...
                disabled={calibrateMutation.isPending || calib.finalScore === ''}
              >
                ...
                <div className="flex items-center justify-between mb-3 gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {t('權重 {{weight}}% · 滿分 {{max}} · {{type}}', {
                        weight: it.weight,
                        max: it.maxScore,
                        type: t(it.scoringTypeText)
                      })}
                    </p>
                  </div>
        ...
        <TabsContent value="overview">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t('自評（{{self}}%）、同事互評（{{peer}}%）、主管評分（{{mgr}}%）按在場方權重加權', {
                  self: wSelf,
                  peer: wPeer,
                  mgr: wMgr
                })}
                  ...
                  {breakdown.length === 0 ? (
                    ...
                    breakdown.map(c => (
                      <TableRow key={c.indicatorId}>
                        ...
                        <TableCell className="text-center">
                          <ScoreBadge score={peerAvg(c)} />
                        </TableCell>
                        <TableCell className="text-center">
                          <ScoreBadge score={c.managerScore} />
                        </TableCell>
                        <TableCell className="text-center">
                          <ScoreBadge score={weightedOf(c)} />
                        </TableCell>
                      </TableRow>
                    ))
                  )}
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {t('權重佔比：{{weight}}%', { weight: wSelf })}
                  {selfDone ? t(' · 提交時間：{{time}}', { time: lastTime(selfR) || '-' }) : t(' · 尚未提交')}
                </CardDescription>
              </CardHeader>
            </Card>
            {categories.map(cat => (
                        ...
                        <CardContent className="p-5">
                          <div className="flex items-start justify-between mb-2 gap-3">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {t('權重 {{weight}}% · 滿分 {{max}}', { weight: item.weight, max: item.maxScore })}
                              </p>
                            </div>
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {t('權重佔比：{{weight}}% · 共 {{count}} 位同事參與', { weight: wPeer, count: peerParticipants })}
                </CardDescription>
              </CardHeader>
            </Card>
            {categories.map(cat => (
                        ...
                        <CardContent className="p-5">
                          <div className="flex items-center justify-between mb-3 gap-3">
                            <div>
                              <h4 className="font-semibold">{item.name}</h4>
                              <p className="text-xs text-muted-foreground">
                                {t('權重 {{weight}}% · 共 {{count}} 位評價', {
                                  weight: item.weight,
                                  count: item.peerReviews.length
                                })}
                              </p>
                            </div>
                            {peerAvg(item) != null && (
                              <Badge variant="secondary">{t('均分：{{avg}}', { avg: peerAvg(item) })}</Badge>
                            )}
                          </div>
                          <Separator className="my-3" />
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {t('權重佔比：{{weight}}%', { weight: wMgr })}
                  {mgrDone ? t(' · 評價時間：{{time}}', { time: lastTime(mgrR) || '-' }) : t(' · 尚未提交')}
                </CardDescription>
              </CardHeader>
            </Card>
            {categories.map(cat => (
                        ...
                        <CardContent className="p-5">
                          <div className="flex items-start justify-between mb-2 gap-3">
                            ...
                            {item.managerScore != null && (
                              <StarRow score={item.managerScore} maxScore={item.maxScore} />
                            )}
                          </div>
                          <Separator className="my-3" />
                          <div className="flex items-start gap-2">
