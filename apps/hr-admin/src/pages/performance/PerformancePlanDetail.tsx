...
import { ArrowLeft, BarChart3, Download, Edit2, Loader2, Send, Target, Trash2 } from 'lucide-react';
...
export default function PerformancePlanDetail() {
  ...
  const { data: result } = useQuery({
    ...
    enabled: Boolean(planId) && Boolean(notDraft)
  });
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-5 gap-x-8">
            ...
            <InfoField label={t('狀態')}>
              <Badge variant="secondary" className={statusColors[data.planStatus] || ''}>
                {t(data.planStatusText)}
              </Badge>
            </InfoField>
            ...
            <div className="mb-5">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-sm font-semibold">
                  {result.finishedTasks} / {result.totalTasks}
                </span>
              </div>
                ...
                {result.grades.map(g => {
                  ...
                  return (
                    <div key={g.grade} className="flex items-center gap-3">
                      <Badge variant="outline" className="font-semibold w-12 justify-center shrink-0">
                        {g.grade}
                      </Badge>
      ...
      <AlertDialog open={pubOpen} onOpenChange={setPubOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(
                '發佈後將按考核範圍生成逐員工任務並分配自評/互評/主管評，方案轉為「進行中」且不可再編輯。確定發佈「{{name}}」？',
                { name: data.name }
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <AlertDialog open={delOpen} onOpenChange={setDelOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('確定刪除方案「{{name}}」？此操作不可復原。', { name: data.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('取消')}</AlertDialogCancel>
