...
import { getPayrollConfig, savePayrollConfig, type PayrollConfig } from "@/api/payrollSetting";
...
export default function PayrollParams() {
  ...
  const emptyConfig: PayrollConfig = {
    standardMonthlyHours: 174, defaultOtMultiplier: 1, otHoursCap: 40,
    anomalyThreshold: 20, deductionCapRatio: 50, note: "",
  };
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t("已保存薪資參數")); queryClient.invalidateQueries({ queryKey: ["payrollConfig"] }); },
    onError: (e: Error) => toast.error(e.message || t("保存失敗")),
  });

  const update = (k: keyof PayrollConfig, v: string) =>
    setConfig(prev => ({ ...prev, [k]: v === "" ? 0 : Number(v) }));

  const rows: { key: keyof PayrollConfig; name: string; unit: string; step?: string; desc: string }[] = [
    { key: "standardMonthlyHours", name: "月標準工時", unit: "小時", desc: "時薪換算 / 加班時薪基數（缺省 174）" },
    { key: "defaultOtMultiplier", name: "加班倍率缺省", unit: "倍", step: "0.01", desc: "員工檔案 ADW 加班倍數優先；此為缺省（香港 OT 多為合約值）" },
    { key: "otHoursCap", name: "月加班時數上限", unit: "小時", desc: "超過即在核算時告警（缺省 40h）" },
    { key: "anomalyThreshold", name: "實發異動門檻", unit: "%", desc: "與上期實發環比超過此比例則標記異動（缺省 20%）" },
    { key: "deductionCapRatio", name: "扣款佔工資上限", unit: "%", desc: "《僱傭條例》第 32 條：任一發薪期扣款不得逾此比例（缺省 50%）" },
  ];
