...
import { cancelBonusPenalty, deleteBonusPenalty, getBonusPenaltyDetail, submitBonusPenalty } from '@/api/bonusPenalty';
...
export default function BonusPenaltyDetail() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('記錄已刪除'));
      navigate('/payroll/bonus-penalty');
    },
  ...
  if (!record) {
    return (
      <div className="p-6">
        ...
        <Card>
          <CardContent className="p-12 text-center text-muted-foreground">{t('找不到該記錄')}</CardContent>
        </Card>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className={typeColors[typeText]}>
                {t(typeText)}
              </Badge>
              <Badge variant="outline" className={statusColors[statusText]}>
                {t(statusText)}
              </Badge>
            </div>
            <p className="text-muted-foreground mt-1">
              {t('編號 #{{id}} · 建立於 {{date}}', { id: record.id, date: record.createTime?.slice(0, 10) || '—' })}
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                {isEditing ? (
                  <Select value={editForm.category} onValueChange={v => setEditForm(p => ({ ...p, category: v }))}>
                    <SelectTrigger className="w-[200px] h-8">
                      <SelectValue placeholder={t('選擇類別')} />
                    </SelectTrigger>
                    <SelectContent>
                      {categories.map(c => (
                ...
                {isEditing ? (
                  <Select value={editForm.employeeId} onValueChange={v => setEditForm(p => ({ ...p, employeeId: v }))}>
                    <SelectTrigger className="w-[280px] h-8">
                      <SelectValue placeholder={t('選擇員工')} />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <Row icon={Building2} label={t('部門')}>
                <span className="text-sm font-medium">{record.departmentName || '—'}</span>
              </Row>
              <Row icon={DollarSign} label={t('金額')}>
                {isEditing ? (
                  ...
                  <span className={`text-sm font-medium ${record.type === 1 ? 'text-success' : 'text-destructive'}`}>
              ...
              <Row icon={Calendar} label={t('計入月份')}>
                {isEditing ? (
                  <Input
                    ...
                    onChange={e => setEditForm(p => ({ ...p, applyMonth: e.target.value }))}
                  />
                ) : (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{t('原因說明')}</CardTitle>
            </CardHeader>
            <CardContent>
              {isEditing ? (
                <Textarea
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{t('備註')}</CardTitle>
            </CardHeader>
            <CardContent>
              {isEditing ? (
                <Input
      ...
      <AlertDialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('確定要取消此{{type}}記錄嗎？取消後該筆金額將不會計入 {{month}} 的薪資計算。', {
                type: t(typeText),
                month: record.applyMonth
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('確定要刪除此{{type}}記錄嗎？此操作無法復原。', { type: t(typeText) })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('返回')}</AlertDialogCancel>
