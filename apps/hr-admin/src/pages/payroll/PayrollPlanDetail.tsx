...
import {
  getPayrollPlanDetail, togglePayrollPlan, deletePayrollPlan, type PlanItem,
} from "@/api/payroll";
...
export default function PayrollPlanDetail() {
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, enabled }: { id: number; enabled: boolean }) => togglePayrollPlan(id, enabled),
    onSuccess: (_r, v) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button variant="outline" className="mt-4" onClick={() => navigate("/payroll/structure")}>
          <ArrowLeft className="h-4 w-4 mr-2" />{t("返回薪資結構")}
        </Button>
      </div>
    );
  }
        ...
        <div className="flex gap-2">
          {hasPerm(PAYROLL_PERM.PLAN_TOGGLE) && (
            <Button variant="outline" onClick={() => setShowDisable(true)}>
              <Power className="h-4 w-4 mr-2" />{plan.enabled ? t("停用") : t("啟用")}
            </Button>
          )}
          ...
          {hasPerm(PAYROLL_PERM.PLAN_EDIT) && (
            <Button onClick={() => navigate(`/payroll/structure/${plan.id}/edit`)}>
              <Pencil className="h-4 w-4 mr-2" />{t("編輯方案")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                ...
                <div>
                  ...
                  <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20">{t("{{n}} 人", { n: plan.applicableCount })}</Badge>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("建立日期")}</p>
                  <p className="text-sm text-foreground flex items-center gap-1">
                    <CalendarDays className="h-3.5 w-3.5 text-muted-foreground" />{plan.createTime?.slice(0, 10) || "—"}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("最後更新")}</p>
                  <p className="text-sm text-foreground flex items-center gap-1">
                    <CalendarDays className="h-3.5 w-3.5 text-muted-foreground" />{plan.updateTime?.slice(0, 10) || "—"}
                  </p>
                </div>
              </div>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{plan.probationRatio != null ? `${plan.probationRatio}%` : "—"}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("加班費計算基準")}</p>
                  <p className="text-sm font-medium text-foreground">{overtimeBaseLabel(plan.overtimeBase) === "—" ? "—" : t(overtimeBaseLabel(plan.overtimeBase))}</p>
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-foreground">
                      {plan.retireEmpRate != null ? `${plan.retireEmpRate}%` : "—"} / {plan.retireEmployerRate != null ? `${plan.retireEmployerRate}%` : "—"}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">{t("供款基數下限")}</p>
                    <p className="text-sm font-medium text-foreground">{plan.retireMin != null ? `HK$${fmt(plan.retireMin)}` : "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">{t("供款基數上限")}</p>
                    <p className="text-sm font-medium text-foreground">{plan.retireMax != null ? `HK$${fmt(plan.retireMax)}` : "—"}</p>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-3">{t("※ 此為方案默認值；員工檔案另有設定時，核算以員工檔案為準（法定 MPF 上下限與封頂由系統固定）。")}</p>
              </div>
              <Separator />
              <div>
                <p className="text-sm font-medium mb-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-foreground">{taxTypeLabel(plan.taxType) === "—" ? "—" : t(taxTypeLabel(plan.taxType))}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">{t("稅率")}</p>
                    <p className="text-sm font-medium text-foreground">{plan.taxRate != null ? `${plan.taxRate}%` : "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">{t("起徵點")}</p>
                    <p className="text-sm font-medium text-foreground">{plan.taxThreshold != null ? `HK$${fmt(plan.taxThreshold)}` : "—"}</p>
                  </div>
                </div>
                {plan.taxRegion === "香港" && (
                  <p className="text-xs text-muted-foreground mt-3">{t("※ 香港無 PAYE，稅款僅供資訊 / IR56 口徑，不從實發中扣除。")}</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{plan.paymentMethod ? t(plan.paymentMethod) : "—"}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("發薪日（每月）")}</p>
                  <p className="text-sm font-medium text-foreground">{plan.payDay != null ? t("{{n}} 號", { n: plan.payDay }) : "—"}</p>
                </div>
              </div>
              <p className="text-xs text-muted-foreground mt-3">{t("員工資料中若另有指定支付方式，將優先於方案預設值。")}</p>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <Gift className="h-4 w-4 text-success" />{t("加項明細")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {earnings.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">{t("無加項")}</p>}
                  {earnings.map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-success/5 border border-success/10">
                      <div>
                        <div className="text-sm font-medium text-foreground flex items-center gap-1.5">
                          {item.name}
                          {item.mpfIncluded && <Badge variant="outline" className="text-[10px] px-1 py-0 bg-primary/10 text-primary border-primary/20">{t("計MPF")}</Badge>}
                        </div>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <Shield className="h-4 w-4 text-warning" />{t("減項明細")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {deductions.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">{t("無減項")}</p>}
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <Users className="h-4 w-4 text-primary" />{t("適用員工")}
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {plan.enabled
                ? t("停用「{{name}}」後，該方案將不再適用於新的薪資計算。目前適用 {{count}} 名員工。", { name: plan.name, count: plan.applicableCount })
      ...
      <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("確定要刪除「{{name}}」嗎？此操作無法復原。", { name: plan.name })}{plan.applicableCount > 0 && t("目前仍有 {{count}} 名員工適用此方案，需先解除分配才能刪除。", { count: plan.applicableCount })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("取消")}</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => deleteMutation.mutate(plan.id)}>
              {t("確認刪除")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
