...
export default function PayrollDistribute() {
  ...
  const submitM = useMutation({
    mutationFn: (id: number) => submitDist(id),
    onSuccess: () => {
  ...
  const executeM = useMutation({
    mutationFn: (id: number) => executeDist(id),
    onSuccess: () => {
  ...
  const cancelM = useMutation({
    mutationFn: (id: number) => cancelDist(id),
    onSuccess: () => {
      toast.success(t('已取消'));
      invalidate();
  ...
  const stats = [
    ...
    {
      label: '待審核',
      value: t('{{n}} 筆', { n: records.filter(r => r.status === 2).length }),
      icon: Users,
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-[160px]">
                <SelectValue placeholder={t('狀態篩選')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('全部狀態')}</SelectItem>
                {Object.entries(DIST_STATUS_TEXT).map(([k, v]) => (
              ...
              {filtered.map(r => (
                ...
                >
                  ...
                  <TableCell className="text-right text-accent-foreground">
                    {num(r.totalEmployerContribution)}
                  </TableCell>
                  <TableCell>{r.payDate ?? '—'}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className={statusColors[r.status]}>
                      {t(DIST_STATUS_TEXT[r.status])}
                    </Badge>
                  </TableCell>
      ...
      <AlertDialog open={Boolean(cancelTarget)} onOpenChange={() => setCancelTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('確定要取消 {{period}} 的發薪批次嗎？此操作無法恢復。', { period: cancelTarget?.period ?? '' })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
