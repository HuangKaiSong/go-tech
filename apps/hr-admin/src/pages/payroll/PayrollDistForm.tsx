...
import { generateDist, getGeneratableCalcBatches, submitDist } from '@/api/payrollDist';
...
export default function PayrollDistForm() {
  ...
  const { confirmLeave, isDirty, markSaved } = useUnsavedChanges({
    calcBatchId,
    payDate,
    payMethod,
    payBank,
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('發薪設定')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>
                  {t('薪資期間')} <span className="text-destructive">*</span>
                </Label>
                <Select
                  value={calcBatchId}
                  onValueChange={v => {
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t('選擇已確認的核算月份')} />
                  </SelectTrigger>
                  <SelectContent>
                    {calcBatches.map(b => (
                      <SelectItem key={b.id} value={String(b.id)}>
                        {fmtPeriod(b.period)}（{t('{{n}} 人', { n: b.employeeCount })} · {t('實發')} {num(b.totalNet)}）
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {calcBatches.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    {t('沒有可生成發放的核算批次，請先在「薪資計算」確認批次。')}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                <Label>
                  {t('發薪日期')} <span className="text-destructive">*</span>
                </Label>
                <Input type="date" value={payDate} onChange={e => setPayDate(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>
                  {t('發薪方式')} <span className="text-destructive">*</span>
                </Label>
                <Select value={payMethod} onValueChange={setPayMethod}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('選擇發薪方式')} />
                  </SelectTrigger>
                  <SelectContent>
                    {PAY_METHODS.map(m => (
              ...
              <div className="space-y-2">
                <Label>
                  {t('發薪銀行')} <span className="text-destructive">*</span>
                </Label>
                <Select value={payBank} onValueChange={setPayBank}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('選擇發薪銀行')} />
                  </SelectTrigger>
                  <SelectContent>
                    {PAY_BANKS.map(b => (
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('發薪期間')}</span>
                  <span className="font-medium">{fmtPeriod(period)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('發薪日期')}</span>
                  <span className="font-medium">{payDate || '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('發薪方式')}</span>
                  <span className="font-medium">{payMethod ? t(payMethod) : '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('發薪銀行')}</span>
                  <span className="font-medium">{payBank ? t(payBank) : '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('選取人數')}</span>
                  <span className="font-medium">
                    {selectedItems.length} / {t('{{n}} 人', { n: items.length })}
                  </span>
                </div>
                <Separator />
                <div className="flex justify-between font-semibold text-base">
                  <span>{t('實發總額')}</span>
                  <span className="text-primary">HK$ {num(totalNet)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
                  ...
                  {filtered.map(emp => {
                    ...
                    return (
                      <TableRow key={emp.employeeId} className={on ? '' : 'opacity-50'}>
                        <TableCell>
                          <Checkbox checked={on} onCheckedChange={() => toggleOne(emp.employeeId)} />
                        </TableCell>
                        <TableCell>
                          <div>
