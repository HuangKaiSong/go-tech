...
export default function PayrollDistApproval() {
  ...
  const approveM = useMutation({
    mutationFn: () => approveDist(approveTarget!.id, approveComment),
    onSuccess: () => {
  ...
  const rejectM = useMutation({
    mutationFn: () => rejectDist(rejectTarget!.id, rejectReason),
    onSuccess: () => {
      toast.success(t('已駁回'));
  ...
  const returnM = useMutation({
    mutationFn: () => returnDist(returnTarget!.id, returnReason),
    onSuccess: () => {
      toast.success(t('已退回修改'));
  ...
  const reassignM = useMutation({
    mutationFn: () => reassignDist(reassignTarget!.id, Number(reassignTo), reassignComment),
    onSuccess: () => {
      toast.success(t('已轉簽'));
  ...
  const stats = [
    ...
    {
      label: '待審核金額',
      value: `HK$ ${num(pending.reduce((s, r) => s + (r.totalNet ?? 0), 0))}`,
      icon: Wallet,
      color: 'text-primary'
    },
    {
      label: '已發放',
      value: t('{{n}} 筆', { n: all.filter(r => r.status === 5).length }),
      icon: CheckCircle,
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Tabs value={tab} onValueChange={setTab} className="space-y-4">
        <TabsList>
          <TabsTrigger value="pending" className="gap-2">
            <Clock className="h-4 w-4" /> {t('待我審核')}
            {pending.length > 0 && (
          ...
          <TabsTrigger value="history" className="gap-2">
            <History className="h-4 w-4" /> {t('已審核')}
          </TabsTrigger>
        </TabsList>
                  ...
                  <Card key={record.id} className="border-l-4 border-l-primary">
                    <CardContent className="p-6">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div>
                          <p className="text-xs text-muted-foreground">{t('發薪人數')}</p>
                          <p className="font-semibold text-foreground">{t('{{n}} 人', { n: record.employeeCount })}</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">{t('應發總額')}</p>
                          <p className="font-semibold text-foreground">HK$ {num(record.totalGross)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">{t('扣款總額')}</p>
                          <p className="font-semibold text-destructive">- HK$ {num(record.totalDeduction)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">{t('實發總額')}</p>
                          <p className="font-bold text-primary">HK$ {num(record.totalNet)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">{t('發薪日期')}</p>
                      ...
                      <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground">
                          {levelTitles.map((title, i) => (
                            <div
                              key={title}
                              className={`flex items-center gap-1 ${i < level ? 'text-success' : i === level ? 'text-primary font-medium' : ''}`}
                            >
                              {i < level ? (
                ...
                <div className="flex gap-2">
                  <Select value={logAction} onValueChange={setLogAction}>
                    <SelectTrigger className="w-36 h-9">
                      <SelectValue placeholder={t('操作類型')} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {logs.map(l => (
                    <TableRow key={l.id}>
                      ...
                      <TableCell
                        ...
                        onClick={() => navigate(`/payroll/distribute/${l.batchId}`)}
                      >
                        {l.period ?? `#${l.batchId}`}
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={actionColors[l.action] || ''}>
                          {t(l.action)}
                        </Badge>
                      </TableCell>
      ...
      <ApproveConfirmDialog
        ...
        batch={approveTarget}
        levelTitle={
          approveTarget ? levelTitles[Math.min(approveTarget.approvalLevel ?? 0, TOTAL_LEVELS - 1)] : undefined
        }
      ...
      <ReassignDialog
        ...
        levelTitle={
          reassignTarget ? levelTitles[Math.min(reassignTarget.approvalLevel ?? 0, TOTAL_LEVELS - 1)] : undefined
        }
      ...
      <Dialog open={Boolean(previewTarget)} onOpenChange={() => setPreviewTarget(null)}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('批次 #{{id}} · {{count}} 人 · 實發 HK$ {{net}}', {
                id: previewTarget?.id ?? '',
                count: previewTarget?.employeeCount ?? 0,
                net: num(previewTarget?.totalNet)
              })}
            </DialogDescription>
          </DialogHeader>
          ...
          {(previewDetail?.anomalies?.length ?? 0) > 0 && (
            <div className="mb-2">
              <p className="text-sm font-medium text-foreground mb-2 flex items-center gap-1">
                <AlertTriangle className="h-4 w-4 text-warning" /> {t('異常項目')}
              </p>
              {previewDetail!.anomalies.map((a, i) => (
                <div
                  key={i}
                  className={`flex items-center gap-2 p-2 rounded-md border text-sm mb-1 ${anomalyTone(a.severity)}`}
                >
            ...
            <div className="mt-4">
              ...
              <div className="space-y-0">
                {levelTitles.slice(0, TOTAL_LEVELS).map((title, i) => {
                  ...
                  const log = (previewDetail?.logs ?? []).find(
                    l => l.level === level && (l.action === '審核通過' || l.action === '終審通過')
                  );
                  return (
                    <div key={title} className="flex items-start gap-3">
                      <div className="flex flex-col items-center">
                        <div
                          className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${done ? 'bg-success text-success-foreground' : current ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
                        >
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                ...
                navigate(`/payroll/distribute/${id}`);
              }}
            >
              {t('查看完整詳情')}
            </Button>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('駁回 {{period}}，批次退回草稿', { period: rejectTarget?.period ?? '' })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ArrowLeft className="h-5 w-5 text-warning" /> {t('退回修改')}
            </DialogTitle>
            <DialogDescription>
              {t('將 {{period}} 退回給提交者修改', { period: returnTarget?.period ?? '' })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">
                {t('退回原因')} <span className="text-destructive">*</span>
              </label>
              <Textarea
                placeholder={t('請說明需修改的內容...')}
                value={returnReason}
            ...
            <Button
              ...
              onClick={() => {
                if (!returnReason.trim()) {
                  toast.error(t('請填寫退回原因'));
                  return;
                }
                returnM.mutate();
              }}
            >
              <ArrowLeft className="h-4 w-4" /> {t('確認退回')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
