...
export default function LeaveSettings() {
  ...
  const saveTypeMut = useMutation({
    ...
    onSuccess: () => {
      toast.success(tr('已儲存假別設定'));
      setDialogOpen(false);
      invalidateTypes();
    }
  });
  const toggleMut = useMutation({
    mutationFn: ({ enabled, id }: { enabled: boolean; id: number }) => toggleLeaveType(id, enabled),
    onSuccess: invalidateTypes
  });
  const deleteMut = useMutation({
    mutationFn: deleteLeaveType,
    onSuccess: () => {
      toast.success(tr('已刪除假別'));
      invalidateTypes();
    }
  });
  const savePolicyMut = useMutation({
    ...
    onSuccess: () => {
      toast.success(tr('已儲存年假規則'));
      qc.invalidateQueries({ queryKey: ['annualPolicy'] });
    }
  });
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{tr('管理假別類型、年假規則與請假政策')}</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="types" className="w-full">
        ...
        <TabsContent value="types" className="space-y-4">
          <Card>
            <CardHeader className="flex-row items-center justify-between space-y-0">
              ...
              {hasPerm(LEAVE_PERM.ADD) && (
                <Button onClick={openAdd}>
                  <Plus className="h-4 w-4 mr-1" /> {tr('新增假別')}
                </Button>
              )}
            </CardHeader>
            <CardContent>
                    ...
                    types.map(t => {
                      ...
                      return (
                        <TableRow key={t.id}>
                          ...
                          <TableCell>
                            {t.paid ? (
                              <Badge variant="default">{tr('帶薪')}</Badge>
                            ) : (
                              <Badge variant="outline">{tr('無薪')}</Badge>
                            )}
                          </TableCell>
                          <TableCell>
                            {t.maxPerYear == null ? tr('不限') : tr('{{n}} 天', { n: t.maxPerYear })}
                          </TableCell>
                          <TableCell>
                            {t.maxPerRequest == null ? tr('不限') : tr('{{n}} 天', { n: t.maxPerRequest })}
                          </TableCell>
                          <TableCell>
                            {t.requireProof ? (
                              <Badge variant="secondary">{tr('≥ {{n}} 天', { n: t.proofThresholdDays })}</Badge>
                            ) : (
                              '—'
                            )}
                          </TableCell>
                          ...
                          <TableCell>
                            <Switch
                              checked={t.enabled}
                              onCheckedChange={() => t.id && toggleMut.mutate({ id: t.id, enabled: !t.enabled })}
                            />
                          </TableCell>
        ...
        <TabsContent value="annual" className="space-y-4">
          <div className="flex justify-end">
            <Button
              onClick={() => policy && savePolicyMut.mutate(policy)}
              disabled={!policy || savePolicyMut.isPending}
            >
              {savePolicyMut.isPending ? (
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <Input
                                ...
                                placeholder={tr('不限')}
                                onChange={e =>
                                  updateTier(idx, { maxYears: e.target.value === '' ? null : Number(e.target.value) })
                                }
                              />
                              <span className="text-sm text-muted-foreground">{tr('年')}</span>
                            </div>
              ...
              <Card>
                ...
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-md">
                    ...
                    <Switch
                      checked={policy.proRateFirstYear}
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-md">
                    ...
                    <Switch
                      checked={policy.probationEligible}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between p-3 border rounded-md">
                    ...
                    <Switch
                      checked={policy.carryOverEnabled}
                  ...
                  {policy.carryOverEnabled && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-3 border-l-2 border-primary/30">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          value={policy.carryOverExpireMonths}
        ...
        <DialogContent className="max-w-2xl">
            ...
            <div className="grid grid-cols-2 gap-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setEditing({ ...editing, maxPerYear: e.target.value === '' ? null : Number(e.target.value) })
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={editing.maxPerRequest ?? ''}
                  onChange={e =>
                    setEditing({ ...editing, maxPerRequest: e.target.value === '' ? null : Number(e.target.value) })
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={editing.advanceApplyDays}
              ...
              <div className="flex items-center justify-between p-3 border rounded-md">
                ...
                <Switch
                  checked={editing.requireProof}
                  onCheckedChange={v => setEditing({ ...editing, requireProof: v })}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  disabled={!editing.requireProof}
                  value={editing.proofThresholdDays}
          ...
          <DialogFooter>
            ...
            <Button onClick={saveType} disabled={saveTypeMut.isPending}>
              {saveTypeMut.isPending && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}
              {tr('儲存')}
            </Button>
          </DialogFooter>
        </DialogContent>
