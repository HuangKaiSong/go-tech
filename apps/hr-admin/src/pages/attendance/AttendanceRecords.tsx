import { BarChart3, ChevronLeft, ChevronRight, Clock, Table2 } from 'lucide-react';
...
import { ATTENDANCE_STATUS_TEXT, exportRecords, getRecordList, SOURCE_SUPPLEMENT } from '@/api/attendance';
...
export default function AttendanceRecords() {
  ...
  const yearOptions = useMemo(() => {
    ...
    return base.includes(year) ? base : [...base, year].toSorted();
  }, [year]);
  ...
  const filteredData = useMemo(() => {
    return rows.filter(row => !search || row.name.includes(search) || row.department.includes(search));
  }, [rows, search]);
  ...
  const dailyTrend = useMemo(() => {
    const days: Record<string, { normal: number; total: number }> = {};
    filteredData.forEach(r => {
  ...
  const handlePrevMonth = () => {
    const m = Number.parseInt(month === 'all' ? '01' : month);
    if (m <= 1) {
      setYear(String(Number.parseInt(year) - 1));
      setMonth('12');
    } else setMonth(String(m - 1).padStart(2, '0'));
  };
  const handleNextMonth = () => {
    const m = Number.parseInt(month === 'all' ? '12' : month);
    if (m >= 12) {
      setYear(String(Number.parseInt(year) + 1));
      setMonth('01');
    } else setMonth(String(m + 1).padStart(2, '0'));
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      rest,
      absent,
      rate: total > 0 ? ((normal / total) * 100).toFixed(1) : '0'
    };
  }, [filteredData]);
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-4 mb-4">
        {[
          ...
          <Card
            ...
            onClick={
              s.clickable ? () => navigate(`/attendance/records/monthly?year=${year}&month=${month}`) : undefined
            }
          >
                            ...
                            .map((p, i) => (
                              ...
                              >
                                <img
                                  src={p.url}
                                  alt={p.label}
                                  className="h-9 w-9 rounded object-cover border hover:opacity-80 transition-opacity"
                                />
                              </a>
                            ))}
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          ...
                          {row.source === SOURCE_SUPPLEMENT && (
                            <Badge
                              variant="outline"
                              className="font-normal text-primary border-primary/30"
                              title={row.remark}
                            >
                              {t('補卡')}
                            </Badge>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t('出勤狀態分布')}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t('各部門出勤統計')}</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
          ...
          <Card className="lg:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t('每日出勤率趨勢')}</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
