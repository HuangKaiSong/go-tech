          ...
          {mapUrl && (
            <div>
              ...
              <div className="rounded-lg overflow-hidden border bg-muted">
                <iframe src={mapUrl} width="100%" height="220" className="border-0" title={t('地圖預覽')} />
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {t('座標：{{lat}}, {{lng}}（有效範圍 {{radius}} 公尺）', {
                  lat: form.lat,
                  lng: form.lng,
                  radius: form.radius
                })}
  ...
  const openLocDialog = (loc?: AttendanceLocation) => {
    if (loc) {
      ...
      setLocForm({
        ...
        ruleId: loc.ruleId != null ? String(loc.ruleId) : '',
        enabled: loc.enabled
      });
    } else {
      setEditingLoc(false);
  ...
  const openSchedDialog = (s?: AttendanceSchedule) => {
    if (s) {
      ...
      setSchedForm({
        ...
        lateGrace: s.lateGrace ?? 0,
        earlyLeaveGrace: s.earlyLeaveGrace ?? 0,
  ...
  const openRuleDialog = (r?: AttendanceRule) => {
    if (r) {
      ...
      setRuleForm({
        ...
        missedClockAllowAppeal: r.missedClockAllowAppeal,
  ...
  const toggleWorkDay = (day: number) => {
    setSchedForm(prev => ({
      ...
      workDays: prev.workDays.includes(day)
        ...
        : [...prev.workDays, day].toSorted((a, b) => a - b)
    }));
  };
  const stats = [
    {
                ...
                <TableBody>
                  {locations.map(loc => (
                    <TableRow key={loc.id}>
                      ...
                      <TableCell>
                        <Badge variant={loc.enabled ? 'default' : 'secondary'}>
                          {loc.enabled ? t('啟用') : t('停用')}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          {hasPerm(CLOCKIN_PERM.EDIT) && (
                            <Button variant="ghost" size="icon" onClick={() => openLocDialog(loc)}>
                              <Pencil className="h-4 w-4" />
                            </Button>
                          )}
                          {hasPerm(CLOCKIN_PERM.DELETE) && (
                            <Button variant="ghost" size="icon" onClick={() => removeLoc(loc.id)}>
                    ...
                    <TableRow key={s.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          {SCHEDULE_TYPE_TEXT[s.type] ? t(SCHEDULE_TYPE_TEXT[s.type]) : '—'}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge variant={s.enabled ? 'default' : 'secondary'}>{s.enabled ? t('啟用') : t('停用')}</Badge>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          {hasPerm(CLOCKIN_PERM.EDIT) && (
                          ...
                          {hasPerm(CLOCKIN_PERM.DELETE) && (
                            <Button variant="ghost" size="icon" onClick={() => removeSched(s.id)}>
                              <Trash2 className="h-4 w-4 text-destructive" />
                            </Button>
                          )}
                        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                        <div>
                          ...
                          <p className="text-foreground">
                            {r.requirePhoto ? t('需拍照') : t('免拍照')} /{' '}
                            {r.requireLocation ? t('需定位') : t('免定位')}
                          </p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-foreground">
                            {r.missedClockAllowAppeal
                              ? t('可補卡（{{n}}天內）', { n: r.appealDeadlineDays })
                              : t('不可補卡')}
                          </p>
                        </div>
                      </div>
                ...
                {rules.length === 0 && <p className="text-center text-muted-foreground py-8">{t('暫無打卡規則')}</p>}
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>{t('班次類型')}</Label>
              <Select
                value={String(schedForm.type)}
                onValueChange={v => setSchedForm(p => ({ ...p, type: Number(v) }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label>{t('遲到寬限（分鐘）')}</Label>
                <Input
                  type="number"
                  value={schedForm.lateGrace}
                  onChange={e => setSchedForm(p => ({ ...p, lateGrace: Number(e.target.value) }))}
                />
              </div>
              <div>
                <Label>{t('早退寬限（分鐘）')}</Label>
                <Input
                  type="number"
                  value={schedForm.earlyLeaveGrace}
                  onChange={e => setSchedForm(p => ({ ...p, earlyLeaveGrace: Number(e.target.value) }))}
                />
              </div>
            </div>
            <div>
      ...
      <Dialog open={ruleDialog} onOpenChange={setRuleDialog}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
              ...
              {ruleForm.missedClockAllowAppeal && (
                <div>
                  ...
                  <Input
                    ...
                    onChange={e => setRuleForm(p => ({ ...p, appealDeadlineDays: Number(e.target.value) }))}
                  />
                </div>
              )}
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveRuleFn} disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : null}
              {t('儲存')}
            </Button>
          </DialogFooter>
