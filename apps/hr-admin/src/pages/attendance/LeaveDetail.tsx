...
export default function LeaveDetail() {
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-muted-foreground">{t("找不到該請假記錄")}</p>
        <Button variant="outline" className="mt-4" onClick={() => navigate("/attendance/leave")}>
          <ArrowLeft className="h-4 w-4 mr-2" />{t("返回請假管理")}
        </Button>
      </div>
    );
  }
  ...
  const handleAction = (action: string) => {
    toast.success(t("已{{action}}請假申請 {{code}}", { action: t(action), code: data.code }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-0.5 text-sm">{t("提交於 {{time}}", { time: data.submitTime })}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("申請人")}</p>
                  <p className="text-sm font-medium text-foreground flex items-center gap-1">
                    <User className="h-3.5 w-3.5 text-muted-foreground" />{data.applicant}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("部門")}</p>
                  <p className="text-sm font-medium text-foreground flex items-center gap-1">
                    <Building2 className="h-3.5 w-3.5 text-muted-foreground" />{data.department}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("職位")}</p>
                  <p className="text-sm font-medium text-foreground">{data.position}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("假別")}</p>
                  <Badge variant="outline">{data.leaveType}</Badge>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("起始日期")}</p>
                  <p className="text-sm font-medium text-foreground">{data.startDate}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("結束日期")}</p>
                  <p className="text-sm font-medium text-foreground">{data.endDate}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{t("請假天數")}</p>
                  <p className="text-sm font-bold text-primary">{t("{{n}} 天", { n: data.days })}</p>
                </div>
              </div>
              ...
              <div>
                <p className="text-xs text-muted-foreground mb-1">{t("請假事由")}</p>
              ...
              {data.attachments.length > 0 && (
                <>
                  <Separator className="my-4" />
                  <div>
                    <p className="text-xs text-muted-foreground mb-2">{t("附件")}</p>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <p className="text-sm text-muted-foreground">{t("審批意見")}</p>
                  <Textarea
                    placeholder={t("請輸入審批意見（選填）...")}
                ...
                <div className="flex items-center gap-3">
                  <Button onClick={() => handleAction("核准")} className="bg-success hover:bg-success/90 text-success-foreground">
                    <CheckCircle2 className="h-4 w-4 mr-1" />{t("核准")}
                  </Button>
                  <Button variant="destructive" onClick={() => handleAction("駁回")}>
                    <XCircle className="h-4 w-4 mr-1" />{t("駁回")}
                  </Button>
                  <Button variant="outline" onClick={() => handleAction("轉簽")}>
                    <Send className="h-4 w-4 mr-1" />{t("轉簽")}
                  </Button>
                </div>
              </CardContent>
            </Card>
          ...
          {canWithdraw && (
            <div className="flex justify-end">
              <Button variant="outline" className="text-destructive border-destructive/30 hover:bg-destructive/5" onClick={() => handleAction("撤回")}>
                <RotateCcw className="h-4 w-4 mr-1" />{t("撤回申請")}
              </Button>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
                  ...
                  return (
                    <div key={step.id} className="relative flex gap-3">
                      ...
                      <div className={`pb-6 flex-1 ${isLast ? "pb-0" : ""}`}>
                        <p className="text-sm font-medium text-foreground">{t(step.nodeName)}</p>
                        {step.approver && (
                        ...
                        {step.status === "current" && !step.action && (
                          <p className="text-xs text-warning mt-1 font-medium">{t("等待審批中...")}</p>
                        )}
                      </div>
                    </div>
                  );
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{t("假期餘額")}</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {[
                  ...
                  <div key={item.type}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="text-muted-foreground">{t(item.type)}</span>
                      <span className="text-foreground font-medium">{t("{{a}} / {{b}} 天", { a: item.total - item.used, b: item.total })}</span>
                    </div>
