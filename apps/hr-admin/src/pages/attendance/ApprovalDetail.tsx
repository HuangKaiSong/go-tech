...
import {
  ArrowLeft, ArrowRight, CalendarDays, CheckCircle2, Clock, XCircle,
  User, Building2, FileText, Paperclip, MessageSquare,
  RotateCcw, DollarSign, MapPin, Loader2, Send, ClipboardCheck
} from "lucide-react";
...
import {
  getApprovalById, approveApproval, rejectApproval, withdrawApproval, transferApproval,
  APPROVAL_TYPE_TEXT, ACTION_TEXT, type Approval,
} from "@/api/approval";
...
const nodeStepStatus = (node: { statusCode: number; current?: boolean }): StepStatus => {
...
export default function ApprovalDetail() {
  ...
  useEffect(() => {
    ...
    getLeaveTypes()
      .then((res) => {
        ...
        (res.data ?? []).forEach((lt) => { map[lt.code] = lt.name; });
        setLeaveTypeMap(map);
      })
  ...
  const payloadEntries = Object.entries(data.payload ?? {})
    .filter(([k, v]) => v !== null && v !== undefined && v !== "" && !HIDDEN_PAYLOAD_KEYS.includes(k));
  ...
  const actionConfigs: Record<string, ActionConfig> = {
    approve: {
      title: "確認核准", desc: `確定要核准 ${data.applicantName} 的${typeText}嗎？`,
      btnLabel: "確認核准", btnClass: "bg-success hover:bg-success/90 text-success-foreground",
      icon: CheckCircle2, iconClass: "text-success",
      target: { label: "已通過", className: "bg-success/10 text-success border-success/20" },
    },
    reject: {
      title: "確認駁回", desc: `確定要駁回 ${data.applicantName} 的${typeText}嗎？駁回後申請流程將終止。`,
      btnLabel: "確認駁回", btnClass: "bg-destructive hover:bg-destructive/90 text-destructive-foreground",
      icon: XCircle, iconClass: "text-destructive",
      target: { label: "已駁回", className: "bg-destructive/10 text-destructive border-destructive/20" },
    },
    transfer: {
      title: "轉簽審批", desc: "將此節點轉交給其他人處理，審批層級不變。",
      btnLabel: "確認轉簽", btnClass: "",
      icon: Send, iconClass: "text-primary",
    },
    withdraw: {
      title: "確認撤回", desc: `確定要撤回申請 ${data.code}？撤回後需重新提交。`,
      btnLabel: "確認撤回", btnClass: "bg-destructive hover:bg-destructive/90 text-destructive-foreground",
      icon: RotateCcw, iconClass: "text-destructive",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <TypeIcon className="h-4 w-4 text-primary" />{typeText}資訊
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground flex items-center gap-1">
                    <User className="h-3.5 w-3.5 text-muted-foreground" />{data.applicantName}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">部門</p>
                  <p className="text-sm font-medium text-foreground flex items-center gap-1">
                    <Building2 className="h-3.5 w-3.5 text-muted-foreground" />{data.departmentName || "—"}
                  </p>
                </div>
                {data.subType && (
                  <div>
                    ...
                    <div className="space-y-1">
                      {data.attachments!.map((url, i) => (
                        <a key={i} href={url} target="_blank" rel="noreferrer"
                          ...
                          <Paperclip className="h-3.5 w-3.5" />{url.split("/").pop() || url}
                        </a>
                      ))}
                    </div>
                  </div>
          ...
          {showActions && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <MessageSquare className="h-4 w-4 text-primary" />審批操作
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  {hasPerm(APPROVAL_PERM.APPROVE) && (
                    <Button onClick={() => setActionDialog("approve")} className="bg-success hover:bg-success/90 text-success-foreground">
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <Clock className="h-4 w-4 text-primary" />審批流程
              </CardTitle>
            </CardHeader>
            <CardContent>
              {nodes.length === 0 ? (
                ...
                <div className="relative">
                  {nodes.map((node, index) => {
                    ...
                    const StepIcon = st === "completed" ? CheckCircle2
                      : st === "rejected" ? XCircle
                      : st === "current" ? Clock
                      : FileText;
                    return (
                      <div key={node.id} className="relative flex gap-3">
                        {!isLast && (
                          <div className={`absolute left-[15px] top-[32px] w-0.5 h-[calc(100%-16px)] ${stepLineStyle[st]}`} />
                        )}
                        <div className={`relative z-10 flex items-center justify-center w-8 h-8 rounded-full border-2 shrink-0 ${stepCircleStyle[st]}`}>
                          <StepIcon className="h-4 w-4" />
                        </div>
                        <div className={`flex-1 ${isLast ? "" : "pb-6"}`}>
                          <p className="text-sm font-medium text-foreground">第 {node.levelNo} 級 · {node.name}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">審批人：{node.approverName || "—"}</p>
                          <div className="mt-1.5 flex items-center gap-1.5">
                            <Badge variant="outline" className={`text-xs ${stepBadgeStyle[st]}`}>{node.status}</Badge>
                            {node.current && (
                              <Badge className="text-xs bg-warning/10 text-warning border-warning/20 border">當前</Badge>
                            )}
                          </div>
          ...
          {history.length > 0 && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <Clock className="h-4 w-4 text-primary" />操作紀錄
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                    ...
                    <div key={h.id} className="text-sm py-1.5 border-b last:border-b-0">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <Badge variant="outline" className="text-xs shrink-0">{h.actionName ?? ACTION_TEXT[h.action]}</Badge>
            ...
            <>
              ...
              <div className="rounded-lg border bg-muted/30 p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant="outline" className="text-xs shrink-0">{typeText}</Badge>
                </div>
                <p className="text-sm font-medium mt-1">
                  {data.applicantName}{data.summary ? ` — ${data.summary}` : ""}
                </p>
              </div>

              {actionDialog === "transfer" && (
                <div className="space-y-2">
                  <Label>轉簽對象 <span className="text-destructive">*</span></Label>
                  <Select value={transferTo} onValueChange={setTransferTo}>
                    <SelectTrigger><SelectValue placeholder="選擇要轉交的人員" /></SelectTrigger>
                    <SelectContent>
                      {employees
              ...
              {cfg.target && (
                <div className="rounded-lg border p-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className={curStatus?.color || ""}>{curStatus?.label || data.status || "待審核"}</Badge>
                    <ArrowRight className="h-4 w-4 text-muted-foreground shrink-0" />
                    <Badge variant="secondary" className={cfg.target.className}>{cfg.target.label}</Badge>
                  </div>
                </div>
              )}
