...
import { ArrowLeft, Plus, Trash2, GripVertical, Save, Settings2, GitBranch, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { hasPerm } from "@/lib/auth";
import { APPROVAL_PERM } from "@/lib/perms";
import {
  getRuleById, saveRule, APPROVAL_TYPE_TEXT, SCOPE_ALL, SCOPE_DEPARTMENT,
...
const FIELD_OPTIONS: Record<number, { value: string; label: string }[]> = {
  1: [{ value: "days", label: "請假天數" }, { value: "subType", label: "假別" }],
  2: [{ value: "amount", label: "報銷金額" }, { value: "subType", label: "報銷類別" }],
  3: [{ value: "hours", label: "加班時數" }, { value: "subType", label: "加班類型" }],
  4: [{ value: "days", label: "出差天數" }, { value: "subType", label: "目的地類型" }],
  5: [{ value: "days", label: "在職天數" }, { value: "subType", label: "離職原因" }],
  6: [],
  8: [],
};
...
const TIMEOUT_OPTIONS = [
  { value: 1, label: "自動提醒" }, { value: 4, label: "自動駁回" },
];

let uid = 0;
const genId = () => `tmp-${++uid}`;
const emptyLevel = (): LevelForm => ({
  id: genId(), name: "", approverType: 3, positionId: "", employeeId: "",
  condField: "", condOp: "gt", condValue: "",
});
...
export default function ApprovalRuleForm() {
  ...
  useEffect(() => {
    getDepartmentOptions().then((r) => setDepartments(r.data ?? [])).catch(() => setDepartments([]));
    getPositionOptions().then((r) => setPositions(r.data ?? [])).catch(() => setPositions([]));
    getActiveEmployeeOptions().then((r) => setEmployees(r.data ?? [])).catch(() => setEmployees([]));
  }, []);
  ...
  useEffect(() => {
    ...
    getRuleById(Number(ruleId))
      .then((res) => {
        const d = res.data;
        if (!d) { toast.error("審批規則不存在"); return; }
        setName(d.name);
  ...
  const handleSave = async () => {
    ...
    for (const [i, lvl] of levels.entries()) {
      if (!lvl.name.trim()) { toast.error(`第 ${i + 1} 級：請填寫節點名稱`); return; }
      if (lvl.approverType === 3 && !lvl.positionId) { toast.error(`第 ${i + 1} 級：請選擇職位`); return; }
      if (lvl.approverType === 4 && !lvl.employeeId) { toast.error(`第 ${i + 1} 級：請選擇人員`); return; }
      if (lvl.condField && !String(lvl.condValue).trim()) { toast.error(`第 ${i + 1} 級：請填寫觸發條件的值`); return; }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>規則名稱 <span className="text-destructive">*</span></Label>
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：請假審批流程" />
                </div>
                <div className="space-y-2">
                  <Label>申請類型 <span className="text-destructive">*</span></Label>
                  <Select value={type} onValueChange={(v) => { setType(v); setLevels((p) => p.map((l) => ({ ...l, condField: "", condValue: "" }))); }}>
                ...
                <div className="space-y-2">
                  <Label>適用範圍 <span className="text-destructive">*</span></Label>
                  <Select
                    value={Number(scopeType) === SCOPE_ALL ? "all" : departmentId}
                    onValueChange={(v) => {
                      if (v === "all") { setScopeType(String(SCOPE_ALL)); setDepartmentId(""); }
                      else { setScopeType(String(SCOPE_DEPARTMENT)); setDepartmentId(v); }
                    }}
                  >
              ...
              <div className="space-y-2">
                <Label>規則描述</Label>
                <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="描述此審批規則的用途..." rows={2} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base flex items-center gap-2">
                  <GitBranch className="h-4 w-4 text-primary" />審批層級
                </CardTitle>
                <Button variant="outline" size="sm" onClick={addLevel}><Plus className="h-4 w-4 mr-1" />新增層級</Button>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              {levels.map((lvl, index) => (
                <div key={lvl.id} className="border rounded-lg p-4 space-y-3 bg-muted/30">
                  <div className="flex items-center justify-between">
                    ...
                    {levels.length > 1 && (
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => removeLevel(lvl.id)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input value={lvl.name} onChange={(e) => updateLevel(lvl.id, { name: e.target.value })} placeholder="例如：部門主管審批" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">審批人類型</Label>
                      <Select value={String(lvl.approverType)} onValueChange={(v) => updateLevel(lvl.id, { approverType: Number(v), positionId: "", employeeId: "" })}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {approverTypes.map((a) => <SelectItem key={a.value} value={String(a.value)}>{a.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-1.5">
                      ...
                      {lvl.approverType === 3 ? (
                        <Select value={lvl.positionId} onValueChange={(v) => updateLevel(lvl.id, { positionId: v })}>
                          <SelectTrigger><SelectValue placeholder="選擇職位" /></SelectTrigger>
                          <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      <Select value={lvl.condField || "none"} onValueChange={(v) => updateLevel(lvl.id, { condField: v === "none" ? "" : v, condValue: "" })} disabled={!type}>
                        <SelectTrigger className="flex-1"><SelectValue placeholder={type ? "選擇欄位" : "請先選擇申請類型"} /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="none">不設條件</SelectItem>
                          {availableFields.map((f) => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                      <Select value={lvl.condOp} onValueChange={(v) => updateLevel(lvl.id, { condOp: v })} disabled={!lvl.condField}>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader className="pb-3"><CardTitle className="text-base">流程設定</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>審批時限（小時）</Label>
              ...
              <div className="space-y-2">
                ...
                <Select value={timeoutAction} onValueChange={setTimeoutAction}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {TIMEOUT_OPTIONS.map((o) => <SelectItem key={o.value} value={String(o.value)}>{o.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <Separator />
