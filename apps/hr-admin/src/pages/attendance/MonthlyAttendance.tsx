...
import { getMonthlyOverview, CELL_REST, type MonthlyAttendanceRow } from "@/api/attendance";

/** 狀態碼 → 格子樣式與文案（0休息 1正常 2遲到 3早退 4曠工 5請假 6出差） */
const CELL_CONFIG: Record<number, { label: string; name: string; bg: string; text: string }> = {
...
export default function MonthlyAttendance() {
  ...
  const month = monthParam && monthParam !== "all"
    ? Number(monthParam)
    : now.getMonth() + 1;
        ...
        <CardContent className="p-3">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[180px] max-w-sm">
              ...
              <Input placeholder={t("搜尋員工姓名...")} className="pl-9 h-8" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <Select value={deptFilter} onValueChange={setDeptFilter}>
              <SelectTrigger className="w-[130px] h-8"><SelectValue placeholder={t("部門")} /></SelectTrigger>
              <SelectContent>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b bg-muted/30">
                    <th className="sticky left-0 z-10 bg-muted/80 backdrop-blur px-3 py-2 text-left font-medium text-muted-foreground min-w-[100px]">{t("員工")}</th>
                    {days.map((d) => (
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={days.length + 1} className="text-center py-8 text-muted-foreground">{t("暫無記錄")}</td></tr>
                  ) : (
                      ...
                      <tr key={row.employeeId} className="border-b hover:bg-muted/30">
                        ...
                        {days.map((d) => {
                          ...
                          if (status === undefined || status === null) {
                            return <td key={d.day} className="px-0.5 py-1.5 text-center"><span className="text-muted-foreground/30">·</span></td>;
                          }
