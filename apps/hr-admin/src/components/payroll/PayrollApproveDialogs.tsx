      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-success" /> {t('審核通過確認')}
          </DialogTitle>
          <DialogDescription>
            {t('確認通過 {{period}} 的發薪批次', { period: batch?.period ?? '' })}
...
function Row({
  ...
  const colorClass = /text-(destructive|primary|success|warning)/.test(valueClass)
    ? valueClass
    : `text-foreground ${valueClass}`;
  return (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Send className="h-5 w-5 text-primary" /> {t('轉簽（移交）')}
          </DialogTitle>
          <DialogDescription>
            {t('將 {{period}} 的發薪批次移交給其他人審核', { period: batch?.period ?? '' })}
            {levelTitle ? `（${t(levelTitle)}）` : ''}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-foreground mb-1.5 block">
              {t('轉簽對象')} <span className="text-destructive">*</span>
            </label>
            <Select value={targetId} onValueChange={onTargetChange}>
              <SelectTrigger>
                <SelectValue placeholder={t('選擇在職員工')} />
              </SelectTrigger>
              <SelectContent>
                {employees.map(e => (
