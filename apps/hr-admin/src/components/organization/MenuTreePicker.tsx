...
export function countNodes(tree: MenuTreeNode[]): number {
  let n = 0;
  const walk = (list: MenuTreeNode[]) => list.forEach((x) => { n++; walk(x.children ?? []); });
  walk(tree);
  return n;
}
...
export function MenuTreePicker({
  tree, checked, onChange, loading, emptyHint,
}: {
  tree: MenuTreeNode[];
  checked: Set<number>;
  ...
  if (tree.length === 0) {
    return <div className="text-sm text-muted-foreground text-center py-10">{emptyHint ?? t("尚無菜單資料，請先於「菜單管理」建立菜單。")}</div>;
  }
