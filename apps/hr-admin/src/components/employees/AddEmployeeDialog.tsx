import { CheckCircle2, Copy, Loader2, Minus, Plus, Send, UserPlus } from 'lucide-react';
import { useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { toast } from 'sonner';
import { type AttendanceSchedule, getScheduleOptions, SCHEDULE_TYPE_TEXT } from '@/api/attendance';
import type { DepartmentOption } from '@/api/department';
import { type EmployeeSavePayload, inviteEmployee, submitEmployee } from '@/api/employee';
...
export function AddEmployeeDialog({ onOpenChange, onSuccess, open }: AddEmployeeDialogProps) {
  ...
  /** 选择薪资方案：记录 planId，并以方案退休金供款率兜底回填员工供款比例 （员工档案优先——仅回填当前为空的字段，不覆盖 HR 已手填的值）。 */
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      setForm(prev => ({
        ...
        orsoEmployerRatio:
          prev.orsoEmployerRatio.trim() || (detail.retireEmployerRate != null ? String(detail.retireEmployerRate) : '')
      }));
    } catch {
      /* 拉取方案详情失败时静默，不影响手动填写 */
    }
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        >
            ...
            {inviteLink ? (
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm text-muted-foreground">
                  {t(
                    '請將以下連結複製發送給員工，員工打開後即可自行填寫個人資料；提交後該員工會進入「待HR完善」狀態。'
                  )}
                </p>
              ...
              <>
                <p className="text-sm text-muted-foreground">
                  {t(
                    '輸入員工的姓名與聯絡方式，系統將生成個人資料填寫連結；複製發給員工，員工提交後 HR 再補充組織與薪資相關欄位。'
                  )}
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <FieldRow label="員工姓名" required error={errors.name}>
                    <Input
                      placeholder={t('請輸入姓名')}
                      value={form.name}
                  ...
                  <FieldRow label="電子郵箱" required error={errors.email}>
                    <Input
                      type="email"
                  ...
                  <FieldRow label="手機號碼" required error={errors.phone}>
                    <Input
                  ...
                  <FieldRow label="預定入職日期" required error={errors.joinDate}>
                    <Input
                      type="date"
                      value={form.joinDate}
          ...
          <TabsContent value="manual" className="pt-4">
            {step === 1 && <StepTwoEmployee form={form} setField={setField} errors={errors} />}
            {step === 2 && (
              <StepOneHR
                form={form}
                setField={setField}
                ...
                adwOvertime={adwOvertime}
                setAdwOvertime={setAdwOvertime}
                bankRatio={bankRatio}
                setBankRatio={setBankRatio}
                partTime={partTime}
                setPartTime={setPartTime}
                flexibleHours={flexibleHours}
                setFlexibleHours={setFlexibleHours}
                overtimeLeave={overtimeLeave}
                setOvertimeLeave={setOvertimeLeave}
              />
            )}
...
function StepOneHR({
  ...
  partTime: number;
  planOptions: PayrollPlanOption[];
  positions: PositionOption[];
  probation: number;
  schedules: AttendanceSchedule[];
  setAdwOvertime: (n: number) => void;
  setBankRatio: (n: number) => void;
  setField: SetField;
  setFlexibleHours: (n: number) => void;
  setOvertimeLeave: (n: number) => void;
  setPartTime: (n: number) => void;
  setProbation: (n: number) => void;
}) {
  const { t } = useTranslation();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-x-8 gap-y-4">
        ...
        <FieldRow label="假期分類列表">
          <SelectField options={['年假A類', '年假B類', '特殊假期']} />
        </FieldRow>
        ...
        <FieldRow label="薪資方案" required error={errors.salaryPlanId}>
          <Select value={form.salaryPlanId} onValueChange={onSelectPlan}>
            <SelectTrigger className={errors.salaryPlanId ? 'border-destructive' : ''}>
        ...
        <FieldRow label="通過試用日期">
          <Input
            type="date"
            value={form.probationPassDate}
            onChange={e => setField('probationPassDate', e.target.value)}
            onClick={e => e.currentTarget.showPicker?.()}
          />
        </FieldRow>
        <FieldRow label="工資發放群組">
          <SelectField
            options={['月結A組', '月結B組', '半月結']}
            value={form.payrollGroup}
            onValueChange={v => setField('payrollGroup', v)}
          />
        </FieldRow>
        ...
        <FieldRow label={ss.contributionType}>
          <SelectField
            options={['標準供款', '自願供款', '豁免']}
            value={form.contributionType}
            onValueChange={v => setField('contributionType', v)}
          />
        </FieldRow>

        <FieldRow label="每日工作時數">
          <Input type="number" value={form.dailyHours} onChange={e => setField('dailyHours', e.target.value)} />
        </FieldRow>
        <FieldRow label={ss.mpfType}>
          <SelectField
            options={['僱員強制', '僱主自願', '行業計劃']}
            value={form.mpfType}
            onValueChange={v => setField('mpfType', v)}
          />
        </FieldRow>

        <FieldRow label="每週工作日數">
          <Input type="number" value={form.weeklyDays} onChange={e => setField('weeklyDays', e.target.value)} />
        </FieldRow>
        <FieldRow label={ss.empMpfType}>
          <SelectField
            options={['標準', '特別']}
            value={form.empMpfType}
            onValueChange={v => setField('empMpfType', v)}
          />
        </FieldRow>

        <FieldRow label="每月工作数">
          <Input type="number" value={form.monthlyHours} onChange={e => setField('monthlyHours', e.target.value)} />
        </FieldRow>
        <FieldRow label={ss.orsoType}>
          <SelectField
            options={['僱員供款', '僱主供款', '雙方供款']}
            value={form.orsoType}
            onValueChange={v => setField('orsoType', v)}
          />
        </FieldRow>

        <FieldRow label="兼職員工">
          <RadioGroup
            value={partTime === 1 ? 'yes' : 'no'}
            onValueChange={v => setPartTime(v === 'yes' ? 1 : 0)}
            className="flex gap-4"
          >
            <div className="flex items-center gap-1.5">
              <RadioGroupItem value="yes" id="pt-y" />
              <Label htmlFor="pt-y" className="text-sm">
                {t('是')}
              </Label>
            </div>
            <div className="flex items-center gap-1.5">
              <RadioGroupItem value="no" id="pt-n" />
              <Label htmlFor="pt-n" className="text-sm">
                {t('否')}
              </Label>
            </div>
          </RadioGroup>
        </FieldRow>
        <FieldRow label={ss.orsoEmpRatio}>
          <Input type="number" value={form.orsoEmpRatio} onChange={e => setField('orsoEmpRatio', e.target.value)} />
        </FieldRow>

        <FieldRow label="彈性上班時間">
          <RadioGroup
            value={flexibleHours === 1 ? 'yes' : 'no'}
            onValueChange={v => setFlexibleHours(v === 'yes' ? 1 : 0)}
            className="flex gap-4"
          >
            <div className="flex items-center gap-1.5">
              <RadioGroupItem value="yes" id="flex-y" />
              <Label htmlFor="flex-y" className="text-sm">
                {t('是')}
              </Label>
            </div>
            <div className="flex items-center gap-1.5">
              <RadioGroupItem value="no" id="flex-n" />
              <Label htmlFor="flex-n" className="text-sm">
                {t('否')}
              </Label>
            </div>
          </RadioGroup>
        </FieldRow>
        <FieldRow label={ss.orsoEmployerRatio}>
          <Input
            type="number"
            value={form.orsoEmployerRatio}
            onChange={e => setField('orsoEmployerRatio', e.target.value)}
          />
        </FieldRow>

        <FieldRow label="加班換銷假">
          <Switch checked={overtimeLeave === 1} onCheckedChange={c => setOvertimeLeave(c ? 1 : 0)} />
        </FieldRow>
        <FieldRow label={ss.employerMpf}>
          <Input value={form.employerMpf} onChange={e => setField('employerMpf', e.target.value)} />
        </FieldRow>
        ...
        <FieldRow label="適用於每日平均工資(ADW)的加班費">
          <div className="flex items-center gap-2">
            <Button
              ...
              size="icon"
              className="h-8 w-8"
              onClick={() => setAdwOvertime(Math.max(0, adwOvertime - 1))}
            >
              <Minus className="h-3 w-3" />
            </Button>
            ...
            <Button
              ...
              size="icon"
              className="h-8 w-8"
              onClick={() => setAdwOvertime(adwOvertime + 1)}
            >
              <Plus className="h-3 w-3" />
            </Button>
          </div>
        </FieldRow>
        <FieldRow label={ss.employerOrso}>
          <Input value={form.employerOrso} onChange={e => setField('employerOrso', e.target.value)} />
        </FieldRow>

        <FieldRow label="門市分類列表">
          <SelectField
            options={['總部', '分店A', '分店B']}
            value={form.storeCategory}
            onValueChange={v => setField('storeCategory', v)}
          />
        </FieldRow>
        <FieldRow label="銀行戶口比例">
          <div className="flex items-center gap-2">
            <Button
              ...
              size="icon"
              className="h-8 w-8"
              onClick={() => setBankRatio(Math.max(0, bankRatio - 5))}
            >
              <Minus className="h-3 w-3" />
            </Button>
            ...
            <Button
              ...
              size="icon"
              className="h-8 w-8"
              onClick={() => setBankRatio(Math.min(100, bankRatio + 5))}
            >
              <Plus className="h-3 w-3" />
            </Button>
          </div>
...
function StepTwoEmployee({
  ...
  return (
    <div className="space-y-6">
      ...
      <Group title="基本資料">
        <FieldRow label="姓名" required error={errors.name}>
          <Input
            value={form.name}
            onChange={e => setField('name', e.target.value)}
            className={errors.name ? 'border-destructive' : ''}
        ...
        <FieldRow label="英文姓名">
          <Input value={form.englishName} onChange={e => setField('englishName', e.target.value)} />
        </FieldRow>
        <FieldRow label="性別">
          <SelectField
            options={[
        ...
        <FieldRow label="身份證號碼" required error={errors.idNumber}>
          <Input
            value={form.idNumber}
      ...
      <Group title="聯絡資訊">
        <FieldRow label="聯絡電話" required error={errors.phone}>
          <Input
            value={form.phone}
            onChange={e => setField('phone', e.target.value)}
            className={errors.phone ? 'border-destructive' : ''}
        ...
        <FieldRow label="通訊地址" required error={errors.address}>
          <Input
            value={form.address}
      ...
      <Group title="銀行資訊">
        <FieldRow label="銀行名稱" required error={errors.bankName}>
          <Input
            value={form.bankName}
        ...
        <FieldRow label="銀行帳號" required error={errors.bankAccount}>
          <Input
