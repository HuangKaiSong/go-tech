...
import { UserCheck, CheckCircle2, Loader2, Check, ChevronsUpDown } from "lucide-react";
...
import { getScheduleOptions, type AttendanceSchedule } from "@/api/attendance";
...
interface CompleteForm {
  ...
  positionId: string;
  position: string;
  managerId: string;      // 直屬主管（在職員工 id，字串）
  ...
  salaryType: string;
  baseSalary: string;
  salaryPlanId: string;   // 薪資方案（payroll_plan id，字串）
  payrollGroup: string;
  adwOvertime: string;
  bankRatio: string;
  joinDate: string;
  ...
  employerOrso: string;
  paymentType: string;
  scheduleId: string;     // 打卡班次（attendance_schedule id，字串）
  remark: string;         // 備註 → complete_remark
}

/** 完善表單的空白預設（新增字段集中一處，避免多處遺漏） */
const EMPTY_COMPLETE_FORM: CompleteForm = {
  departmentId: "", department: "", positionId: "", position: "", managerId: "", employmentType: "", probation: "3",
  probationPassDate: "", storeCategory: "", salaryType: "", baseSalary: "", salaryPlanId: "", payrollGroup: "",
  adwOvertime: "0", bankRatio: "100", joinDate: "", mpfType: "", empMpfType: "", contributionType: "", orsoType: "",
  orsoEmpRatio: "", orsoEmployerRatio: "", employerMpf: "", employerOrso: "", paymentType: "銀行轉帳", scheduleId: "", remark: "",
};

export function HRCompleteDialog({ open, onOpenChange, employee, onSuccess }: HRCompleteDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {t("員工已透過邀請連結提交個人資料，請 HR 補充組織與薪資相關欄位後提交，系統將自動建立員工檔案並轉入「入職管理」流程。")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-lg border bg-muted/30 p-4 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            <div><span className="text-muted-foreground">{t("工號：")}</span>{employee.employeeNo || "—"}</div>
            <div><span className="text-muted-foreground">{t("姓名：")}</span>{employee.name || "—"}</div>
            <div><span className="text-muted-foreground">{t("電話：")}</span>{employee.phone || "—"}</div>
            <div><span className="text-muted-foreground">{t("部門意向：")}</span>{employee.department || "—"}</div>
            <div><span className="text-muted-foreground">{t("職位意向：")}</span>{employee.position || "—"}</div>
            <div><span className="text-muted-foreground">{t("預定入職：")}</span>{employee.joinDate || "—"}</div>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="員工編號"><Input value={employee.employeeNo || ""} placeholder={t("系統自動生成")} readOnly /></Field>
            <Field label="確認部門" required error={errors.department}>
              ...
              >
                <SelectTrigger className={errors.department ? "border-destructive" : ""}><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
            ...
            <Field label="確認職位" required error={errors.position}>
              ...
              >
                <SelectTrigger className={errors.position ? "border-destructive" : ""}><SelectValue placeholder={form.departmentId ? t("請選擇") : t("請先選擇部門")} /></SelectTrigger>
                <SelectContent>
                  ...
                  >
                    <span className={cn("truncate", !form.managerId && "text-muted-foreground")}>
                      {(() => {
                        ...
                        return sel ? `${sel.name}${sel.employeeNo ? `（${sel.employeeNo}）` : ""}` : t("請搜尋並選擇在職員工");
                      })()}
                    </span>
                  ...
                  <Command>
                    ...
                    <CommandList>
                      ...
                      {managerOptions.map((o) => (
                        ...
                        >
                          <Check className={cn("mr-2 h-4 w-4", form.managerId === String(o.id) ? "opacity-100" : "opacity-0")} />
                          {o.name}{o.employeeNo ? `（${o.employeeNo}）` : ""}
                        </CommandItem>
                      ))}
                    </CommandList>
                  </Command>
            ...
            <Field label="僱用類型" required error={errors.employmentType}>
              <Select value={form.employmentType || undefined} onValueChange={(v) => set("employmentType", v)}>
                <SelectTrigger className={errors.employmentType ? "border-destructive" : ""}><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {["全職", "兼職", "合約", "實習"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="試用期（月）">
            ...
            <Field label="通過試用日期">
              <Input type="date" value={form.probationPassDate} onChange={(e) => set("probationPassDate", e.target.value)} onClick={(e) => e.currentTarget.showPicker?.()} />
            </Field>
            <Field label="門市分類">
              <Select value={form.storeCategory || undefined} onValueChange={(v) => set("storeCategory", v)}>
                ...
                <SelectContent>
                  {["總部", "分店A", "分店B"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="打卡班次">
              <Select value={form.scheduleId || undefined} onValueChange={(v) => set("scheduleId", v)}>
                <SelectTrigger><SelectValue placeholder={t("請選擇班次")} /></SelectTrigger>
                <SelectContent>
                  {schedules.map((sc) => <SelectItem key={sc.id} value={String(sc.id)}>{sc.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="薪資類型" required error={errors.salaryType}>
              <Select value={form.salaryType || undefined} onValueChange={(v) => set("salaryType", v)}>
                <SelectTrigger className={errors.salaryType ? "border-destructive" : ""}><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {SALARY_TYPES.map((o) => <SelectItem key={o.code} value={o.code}>{t(o.label)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="基本薪金 (HK$)" required error={errors.baseSalary}>
              <Input type="number" value={form.baseSalary} onChange={(e) => set("baseSalary", e.target.value)} placeholder="0" className={errors.baseSalary ? "border-destructive" : ""} />
            </Field>
            <Field label="薪資方案">
              <Select value={form.salaryPlanId || undefined} onValueChange={handlePickPlan}>
                <SelectTrigger><SelectValue placeholder={t("請選擇薪資方案")} /></SelectTrigger>
                <SelectContent>
            ...
            <Field label="工資發放群組">
              <Select value={form.payrollGroup || undefined} onValueChange={(v) => set("payrollGroup", v)}>
                <SelectTrigger><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {["月結A組", "月結B組", "半月結"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="ADW 加班費倍數">
              <Input type="number" value={form.adwOvertime} onChange={(e) => set("adwOvertime", e.target.value)} placeholder="0" />
            </Field>
            <Field label="銀行戶口比例（%）">
              <Input type="number" value={form.bankRatio} onChange={(e) => set("bankRatio", e.target.value)} placeholder="100" />
            </Field>
            <Field label="確認入職日期" required error={errors.joinDate}>
              <Input type="date" value={form.joinDate} onChange={(e) => set("joinDate", e.target.value)} onClick={(e) => e.currentTarget.showPicker?.()} className={errors.joinDate ? "border-destructive" : ""} />
            </Field>
            <Field label="負責 HR">
            ...
            <Field label="供款類型">
              <Select value={form.contributionType || undefined} onValueChange={(v) => set("contributionType", v)}>
                <SelectTrigger><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {["標準供款", "自願供款", "豁免"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="強積金類型">
              <Select value={form.mpfType || undefined} onValueChange={(v) => set("mpfType", v)}>
                <SelectTrigger><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {["僱員強制", "僱主自願", "行業計劃"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="僱員強積金類型">
              <Select value={form.empMpfType || undefined} onValueChange={(v) => set("empMpfType", v)}>
                <SelectTrigger><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {["標準", "特別"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="僱主強積金">
            ...
            <Field label="公積金類型">
              <Select value={form.orsoType || undefined} onValueChange={(v) => set("orsoType", v)}>
                <SelectTrigger><SelectValue placeholder={t("請選擇")} /></SelectTrigger>
                <SelectContent>
                  {["僱員供款", "僱主供款", "雙方供款"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="僱員公積金比例（%）">
            ...
            <Field label="僱主公積金比例（%）">
              <Input type="number" value={form.orsoEmployerRatio} onChange={(e) => set("orsoEmployerRatio", e.target.value)} />
            </Field>
            <Field label="僱主公積金">
            ...
            <Field label="支付方式">
              <Select value={form.paymentType || undefined} onValueChange={(v) => set("paymentType", v)}>
                ...
                <SelectContent>
                  {["銀行轉帳", "現金", "支票"].map((o) => <SelectItem key={o} value={o}>{t(o)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
          </div>
          <Field label="備註">
            <Textarea value={form.remark} onChange={(e) => set("remark", e.target.value)} placeholder={t("其他需要注意的事項...")} rows={2} />
          </Field>
        </div>
...
function Field({
  ...
  return (
    <div className="space-y-1.5">
      <Label className="text-sm">{t(label)} {required && <span className="text-destructive">*</span>}</Label>
      {children}
