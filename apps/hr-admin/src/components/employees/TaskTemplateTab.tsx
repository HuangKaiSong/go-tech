import { ListChecks, Loader2, Pencil, Plus, RotateCcw, Trash2, User, UserCog } from 'lucide-react';
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{t('模板總數')}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold">{list.filter(x => x.assigneeType === 1).length}</p>
              <p className="text-xs text-muted-foreground">{t('HR 負責')}</p>
            </div>
          </CardContent>
        </Card>
      </div>
...
function TemplateEditDialog({
  ...
  const submit = async () => {
    if (!name.trim()) {
      toast.error(t('請填寫任務名稱'));
      return;
    }
    if (!category) {
      toast.error(t('請選擇任務分類'));
      return;
    }
    setSubmitting(true);
    try {
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-1.5">
            <Label className="text-sm">
              {t('任務名稱')} <span className="text-destructive">*</span>
            </Label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-sm">
                {t('分類')} <span className="text-destructive">*</span>
              </Label>
