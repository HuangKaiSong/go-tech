...
import {
  ...
  CalendarDays, DollarSign, MapPin, FileText, Timer, Loader2, ClipboardCheck
} from "lucide-react";
import { toast } from "sonner";
import {
  getMyPending, approveApproval, rejectApproval,
  APPROVAL_TYPE_TEXT, type Approval,
} from "@/api/approval";
...
export default function PendingApprovalTab() {
  ...
  const handleBatchSubmit = async () => {
    ...
    const results = await Promise.allSettled(
      selected.map((id) => batchAction === "approve"
          ...
          {selected.length > 0 && (
            <>
              ...
              <Button size="sm" className="bg-success hover:bg-success/90 text-success-foreground" onClick={() => setBatchAction("approve")}>
                <CheckCircle2 className="h-3.5 w-3.5 mr-1" />{t("批量核准")}
              </Button>
              <Button size="sm" variant="destructive" onClick={() => setBatchAction("reject")}>
                <XCircle className="h-3.5 w-3.5 mr-1" />{t("批量駁回")}
              </Button>
            </>
          )}
        </div>
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge variant="outline" className="font-normal gap-1">
                        <TypeIcon className="h-3 w-3" />{t(APPROVAL_TYPE_TEXT[r.type] ?? r.typeName ?? "")}
                      </Badge>
                    </TableCell>
      ...
      <Dialog open={!!batchAction} onOpenChange={() => setBatchAction(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("即將 {{action}} {{n}} 筆申請", { action: batchAction === "approve" ? t("核准") : t("駁回"), n: selected.length })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
              ...
              <div className="space-y-1">
                {sorted.filter((r) => selected.includes(r.id)).map((r) => (
                  <div key={r.id} className="flex items-center justify-between text-sm">
                    ...
                    <span>{r.applicantName} - {r.summary}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setBatchAction(null)}>{t("取消")}</Button>
            <Button
            ...
            >
              ...
              {t("確認")}{batchAction === "approve" ? t("核准") : t("駁回")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
