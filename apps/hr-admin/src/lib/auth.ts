...
/** 存入登入返回的可存取菜單路由(path)集合，供側邊欄過濾。 傳 undefined 表示後端未返回（舊 session）——不寫入，讀取時回退為 null（放行全部）。 */
export function setRoutes(routes?: string[]) {
  if (routes == null) return;
  localStorage.setItem(ROUTES_KEY, JSON.stringify(routes));
}

/** 取當前用戶可存取的菜單路由集合。 返回 null 表示「未提供」→ 側邊欄放行全部（兼容舊 session / 未接權限的後端）； 返回數組（含空數組）表示「已按權限過濾」→ 僅顯示命中的菜單。 */
...
/** 存入登入返回的權限碼集合（hr_menu.perms，含頁簽/操作按鈕權限點）。 傳 undefined 表示後端未返回（舊 session / 未接權限）——不寫入，讀取時回退為 null（放行全部）。 */
export function setPerms(perms?: string[]) {
  if (perms == null) return;
  localStorage.setItem(PERMS_KEY, JSON.stringify(perms));
}

/** 取當前用戶權限碼集合。 返回 null 表示「未提供」→ 放行全部（兼容舊 session / 未接權限的後端）； 返回數組（含空數組）表示「已按職位過濾」→ 僅命中的權限碼放行。 */
...
/** 判斷當前用戶是否擁有某權限碼。 未提供權限集合（null）時一律放行，保證未接權限的後端 / 舊 session 功能不受影響。 */
...
/** 判斷當前用戶是否可存取某菜單路由（前綴匹配，用於儀表板卡片級門控）。 未提供路由集合（null）時一律放行，語義與 hasPerm 一致。 */
