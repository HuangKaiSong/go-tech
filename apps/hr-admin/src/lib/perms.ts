/**
 * 員工模組權限碼（對應 hr_menu.perms）。職位即權限單元，登入時隨 EmployeeLoginVO.perms
 * 返回並緩存，經 auth.ts 的 hasPerm() 判定。未提供權限集合（舊 session / 未接權限後端）
 * 時 hasPerm 一律放行，保證功能不被鎖死。
 *
...
/**
 * 其餘功能菜單的操作按鈕權限碼（對應 hr_menu.perms，menu_type=2）。
 * 種子 SQL：E:\javaProject\GithubProject\claude-code-pms\其余模块按钮_hr_menu种子.sql
 * 用法：頁面 import 對應常量，用 hasPerm(X) 包住按鈕；未授權時 hasPerm 放行全部。
 */
