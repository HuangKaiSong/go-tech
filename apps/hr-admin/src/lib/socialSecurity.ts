 ...
 * 社會保障 / 退休金 —— 按國籍的欄位標籤配置。
 *
 * 設計：各國社保欄位「結構同構」，共用員工表同一組中性槽位，不另建表。 這裡只做「標籤 + 區塊標題」按國籍切換；底層欄位 key 不變： contributionType / mpfType / empMpfType /
 * employerMpf orsoType / orsoEmpRatio / orsoEmployerRatio / employerOrso 核算引擎（Phase 3）再按 nationality 選對應演算法與法定參數。
 */
