...
/**
 * 所有「菜单基路径」——与 AppSidebar 叶子 url 一致，也应与 hr_menu 中 menu_type=1 的 path 对齐。 明细/表单等子路由（如
 * /employees/:id、/organization/roles/new）不在此列， 通过「最长前缀」归属到其所属菜单再鉴权。 新增侧边栏菜单项时，请同步在此登记。
 */
...
/**
 * 找当前路径归属的菜单基路径（最长的、按路径段匹配的前缀）。 例：/employees/123 → /employees；/employees/onboarding/5 → /employees/onboarding。
