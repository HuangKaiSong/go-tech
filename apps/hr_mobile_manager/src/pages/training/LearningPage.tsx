          ...
          <div className="flex items-center gap-2">
            {['課程內容', '隨堂測驗', '完成學習'].map((label, i) => (
              <div key={i} className="flex-1">
                <div
                  className={`h-1.5 rounded-full mb-1 transition-colors ${i <= currentStep ? 'bg-primary' : 'bg-muted'}`}
                />
                <p
                  className={`text-[10px] text-center ${i <= currentStep ? 'text-primary font-medium' : 'text-muted-foreground'}`}
                >
                  {label}
                </p>
              </div>
                      ...
                      {q.options.map((opt, oi) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2.5">
                              <div
                                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${selected ? 'border-primary' : 'border-muted-foreground/30'}`}
                              >
              ...
              <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-3.5 flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-destructive/70 mt-0.5">
                    你答對了 {quizScore}/{learningContent.quiz.length} 題，請重新作答。
                  </p>
                  <button
                    onClick={() => {
                      setQuizSubmitted(false);
                      setQuizAnswers(learningContent.quiz.map(() => null));
                    }}
          ...
          <div className="text-center py-8">
            ...
            <div className="flex items-center justify-center gap-1.5 mb-6">
              ...
              <span className="text-xs text-[hsl(var(--success))] font-medium">
                測驗成績：{quizScore}/{learningContent.quiz.length} 全部通過
              </span>
            </div>
