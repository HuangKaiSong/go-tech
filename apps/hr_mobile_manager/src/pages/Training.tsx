...
const getModuleIconBg = (completed: boolean, canStart: boolean) => {
  if (completed) return 'bg-[hsl(var(--success))]/10';
  if (canStart) return 'bg-primary/10';
  return 'bg-muted';
};

const getModuleIcon = (completed: boolean, canStart: boolean) => {
...
const Training = () => {
  ...
  if (view === 'category' && selectedCategory) {
    ...
    const catProgress =
      freshCat.totalCourses > 0 ? Math.round((freshCat.completedCourses / freshCat.totalCourses) * 100) : 0;
    return (
      ...
      <div className="px-5 pt-4 pb-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            {
              label: '進行中',
              value: courses.reduce((s, c) => s + c.courses.filter(x => x.status === 'in_progress').length, 0),
