...
import {
  ...
  FileEdit, CalendarDays, Send, Camera, ImageIcon, X, RotateCcw
} from "lucide-react";
...
const historyRecords: AttendanceRecord[] = [
  { date: "03/10", weekday: "一", clockIn: "09:02", clockOut: "--:--", status: "normal", statusLabel: "正常", hours: "--" },
  { date: "03/07", weekday: "五", clockIn: "08:55", clockOut: "18:10", status: "normal", statusLabel: "正常", hours: "9h15m" },
  { date: "03/06", weekday: "四", clockIn: "09:15", clockOut: "18:30", status: "late", statusLabel: "遲到", hours: "9h15m" },
  { date: "03/05", weekday: "三", clockIn: "08:50", clockOut: "18:05", status: "normal", statusLabel: "正常", hours: "9h15m" },
  { date: "03/04", weekday: "二", clockIn: "08:58", clockOut: "18:00", status: "normal", statusLabel: "正常", hours: "9h02m" },
  { date: "03/03", weekday: "一", clockIn: "09:00", clockOut: "20:30", status: "normal", statusLabel: "加班", hours: "11h30m" },
  { date: "02/28", weekday: "五", clockIn: "08:45", clockOut: "18:00", status: "normal", statusLabel: "正常", hours: "9h15m" },
  { date: "02/27", weekday: "四", clockIn: "--:--", clockOut: "--:--", status: "leave", statusLabel: "請假", hours: "--" },
  { date: "02/26", weekday: "三", clockIn: "09:05", clockOut: "17:30", status: "early", statusLabel: "早退", hours: "8h25m" },
  { date: "02/25", weekday: "二", clockIn: "08:50", clockOut: "18:10", status: "normal", statusLabel: "正常", hours: "9h20m" },
  { date: "02/24", weekday: "一", clockIn: "08:55", clockOut: "18:00", status: "normal", statusLabel: "正常", hours: "9h05m" },
  { date: "02/21", weekday: "五", clockIn: "--:--", clockOut: "--:--", status: "absent", statusLabel: "缺勤", hours: "--" },
  { date: "02/20", weekday: "四", clockIn: "09:10", clockOut: "--:--", status: "absent", statusLabel: "漏打卡", hours: "--" },
];
...
const ClockIn = () => {
  ...
  const handleSubmitSupplement = () => {
    ...
    setSupplementRequests(prev => [
      { date: supplementDate, type: supplementType, time: supplementTime, reason: supplementReason.trim(), status: "pending" },
      ...prev,
    ]);
      ...
      <DialogContent className="max-w-[340px] rounded-2xl">
        ...
        <div className="space-y-4">
                ...
                {availableAbsentRecords.map((r, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{r.date} 週{r.weekday}</span>
                    </div>
          ...
          <div>
            ...
            <Input value={supplementDate} onChange={e => setSupplementDate(e.target.value)} placeholder="例：03/10" className="h-9 text-sm" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1.5 block">補卡類型</label>
            <div className="flex gap-2">
              <button onClick={() => setSupplementType("clockIn")} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${supplementType === "clockIn" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                上班打卡
              </button>
              <button onClick={() => setSupplementType("clockOut")} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${supplementType === "clockOut" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                下班打卡
              </button>
            </div>
          </div>
          <div>
            ...
            <Input value={supplementTime} onChange={e => setSupplementTime(e.target.value)} placeholder="例：09:00" className="h-9 text-sm" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1.5 block">補卡原因</label>
            <Input value={supplementReason} onChange={e => setSupplementReason(e.target.value)} placeholder="請輸入補卡原因..." className="h-9 text-sm" />
          </div>
        </div>
        <DialogFooter className="gap-2">
          <Button variant="outline" size="sm" onClick={() => setShowSupplementDialog(false)}>取消</Button>
          <Button size="sm" className="gap-1" onClick={handleSubmitSupplement} disabled={!supplementDate || !supplementTime || !supplementReason.trim()}>
            <Send className="w-3.5 h-3.5" />提交申請
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
                  ...
                  <div key={i} className="bg-card rounded-xl border border-border p-4">
                    ...
                    {applied ? (
                      <div className={`flex items-center justify-between p-2.5 rounded-lg ${
                        applied.status === "approved" ? "bg-success/10" :
                        applied.status === "rejected" ? "bg-destructive/10" : "bg-warning/10"
                      }`}>
                        <span className="text-xs text-muted-foreground">
                          已申請{applied.type === "clockIn" ? "上班" : "下班"}補卡 · {applied.time}
                        </span>
                        <span className={`text-[10px] font-medium ${
                          applied.status === "approved" ? "text-success" :
                          applied.status === "rejected" ? "text-destructive" : "text-warning"
                        }`}>
                          {applied.status === "approved" ? "已通過" : applied.status === "rejected" ? "已駁回" : "審核中"}
                        </span>
                      </div>
                    ) : (
                      <Button
                ...
                <div key={i} className="bg-card rounded-xl border border-border p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                      req.status === "approved" ? "text-success bg-success/10" :
                      req.status === "rejected" ? "text-destructive bg-destructive/10" :
                      "text-warning bg-warning/10"
                    }`}>
                      {req.status === "approved" ? "已通過" : req.status === "rejected" ? "已駁回" : "審核中"}
                    </span>
                  </div>
                  <Separator className="my-2" />
          ...
          <div className={`rounded-full p-1 ${
            ...
            >
              ...
              <span className="text-sm mt-1 opacity-90">
                {clockedOut ? "✓ 已完成打卡" : (requirePhoto && !capturedPhoto) ? "📷 請先拍照" : "點擊下班打卡"}
              </span>
            </button>
          </div>
        </div>
          ...
          <div className="bg-card rounded-xl border border-border divide-y divide-border">
            ...
            <div className="flex items-center justify-between p-4">
              ...
              <span className="text-sm font-medium text-foreground">
                {clockedOut ? currentTime.toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" }) : "未打卡"}
              </span>
            </div>
          </div>
        </div>
