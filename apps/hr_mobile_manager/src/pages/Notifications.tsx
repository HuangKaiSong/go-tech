...
// ── Notifications Data ──
const chatAvatarClass = (type: string) => {
  if (type === 'department') return 'bg-accent text-accent-foreground';
  if (type === 'group') return 'bg-secondary text-secondary-foreground';
  return 'bg-primary/10 text-primary';
};

const chatPresenceText = (online?: boolean) => (online ? '在線' : '離線');
...
type Chat = {
  ...
  online?: boolean;
  owner?: string;
  type: 'department' | 'group' | 'individual';
  unread: number;
};
...
const NotificationTab = () => (
  <div className="px-5 pt-4">
    <div className="flex items-center justify-between mb-4">
      <span className="text-xs text-muted-foreground">共 {notifications.filter(n => !n.read).length} 條未讀</span>
    ...
    <div className="space-y-2">
      {notifications.map((notif, i) => (
        ...
        >
          <div className={`w-11 h-11 rounded-lg ${notif.color} flex items-center justify-center shrink-0`}>
...
const ChatList = ({
  ...
  chats: Chat[];
  filter: 'all' | 'department' | 'group' | 'individual';
  onCreateGroup: () => void;
  onOpen: (chat: Chat) => void;
}) => {
  const [search, setSearch] = useState('');
  const filtered = chats.filter(c => filter === 'all' || c.type === filter).filter(c => c.name.includes(search));
          ...
          >
            <div className="relative">
              <Avatar className="w-12 h-12">
                <AvatarFallback className={`text-sm font-medium ${chatAvatarClass(chat.type)}`}>
                  {chat.avatar}
                </AvatarFallback>
              </Avatar>
...
// ── Member Info Panel ──
const MemberInfoPanel = ({ name, onBack, onChat }: { name: string; onBack: () => void; onChat: () => void }) => {
    ...
    <div className="px-5 pt-4">
      ...
      <div className="flex flex-col items-center mb-6">
        <Avatar className="w-20 h-20 mb-3">
          <AvatarFallback className="text-2xl bg-primary/10 text-primary">{name.slice(0, 1)}</AvatarFallback>
        </Avatar>
        <p className="text-lg font-semibold text-foreground">{name}</p>
        {contact && <p className="text-sm text-muted-foreground">{contact.title}</p>}
      </div>
      <div className="bg-card rounded-xl border p-4 space-y-3 mb-4">
        <div className="flex justify-between">
          <span className="text-xs text-muted-foreground">部門</span>
          <span className="text-sm text-foreground">{contact?.dept || '—'}</span>
        </div>
        <Separator />
        <div className="flex justify-between">
          <span className="text-xs text-muted-foreground">職位</span>
          <span className="text-sm text-foreground">{contact?.title || '—'}</span>
        </div>
        <Separator />
        <div className="flex justify-between">
  ...
  return (
    <div className="px-5 pt-4">
      ...
      <div className="flex flex-col items-center mb-5">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">{chat.members?.length || 0} 位成員</p>
        {chat.owner && (
          <div className="flex items-center gap-1 mt-1">
            <Crown className="w-3 h-3 text-warning" />
            <span className="text-[10px] text-muted-foreground">群主：{chat.owner}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-center gap-6 mb-5">
        ...
        <button onClick={onToggleMute} className="flex flex-col items-center gap-1">
          ...
          <span className="text-[10px] text-muted-foreground">{chat.muted ? '已靜音' : '靜音'}</span>
        </button>
      ...
      <div className="bg-card rounded-xl border mb-4">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <span className="text-xs text-muted-foreground">{chat.members?.length || 0} 人</span>
        </div>
        <Separator />
        <div className="max-h-64 overflow-y-auto">
          {chat.members?.map(member => (
            ...
            >
              <div className="flex-1 flex items-center gap-3" onClick={() => onMemberClick(member)}>
                <Avatar className="w-9 h-9">
                  <AvatarFallback className="text-xs bg-primary/10 text-primary">{member.slice(0, 1)}</AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-sm text-foreground">{member}</p>
                    {member === chat.owner && <Crown className="w-3 h-3 text-warning" />}
                  </div>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem-5rem)]">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card">
        ...
        >
          <Avatar className="w-9 h-9">
            <AvatarFallback className="text-xs bg-primary/10 text-primary">{chat.avatar}</AvatarFallback>
          </Avatar>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-foreground truncate">{chat.name}</p>
            <p className="text-[10px] text-muted-foreground">
              {chat.type === 'individual' ? chatPresenceText(chat.online) : `${chat.members?.length || 0} 位成員`}
            </p>
          </div>
        </button>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-muted/30">
        {chat.messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.isMe ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex gap-2 max-w-[75%] ${msg.isMe ? 'flex-row-reverse' : ''}`}>
              {!msg.isMe && (
              ...
              <div>
                {!msg.isMe && chat.type !== 'individual' && (
                  <p className="text-[10px] text-muted-foreground mb-1 ml-1">{msg.sender}</p>
                )}
                <div
                ...
                <div className={`flex items-center gap-1 mt-0.5 ${msg.isMe ? 'justify-end' : ''}`}>
                  ...
                  {msg.isMe && <CheckCheck className="w-3 h-3 text-primary/60" />}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-3 py-2 border-t border-border bg-card flex items-end gap-2">
        ...
        />
        {input.trim() ? (
          <Button size="icon" className="w-9 h-9 rounded-full shrink-0" onClick={handleSend}>
...
const Notifications = () => {
  ...
  const [chatFilter] = useState<'all' | 'department' | 'group' | 'individual'>('all');
  const [showCreateGroup, setShowCreateGroup] = useState(false);
  const [groupName, setGroupName] = useState('');
  const [selectedMembers, setSelectedMembers] = useState<string[]>([]);
  const [memberSearch, setMemberSearch] = useState('');
  const [view, setView] = useState<'addMember' | 'chat' | 'groupInfo' | 'main' | 'memberInfo'>('main');
  ...
  useEffect(() => {
    ...
    const existingChat = chats.find(c => c.type === 'individual' && c.name === contactName);
    if (existingChat) {
      setActiveChat({ ...existingChat, unread: 0 });
      setChats(prev => prev.map(c => (c.id === existingChat.id ? { ...c, unread: 0 } : c)));
    } else {
  ...
  const handleAddMembers = () => {
    ...
    const newMembers = selectedMembers.filter(m => !activeChat.members?.includes(m));
    ...
    return (
      <MobileLayout title="">
        <MemberInfoPanel
          ...
          onChat={() => {
            const existing = chats.find(c => c.type === 'individual' && c.name === selectedMember);
            if (existing) {
  ...
  if (view === 'addMember' && activeChat) {
    ...
    const available = allContacts.filter(c => !existingMembers.includes(c.name));
    return (
      <MobileLayout title="">
        <div className="px-5 pt-4">
          <div className="flex items-center gap-3 mb-5">
          ...
          <ScrollArea className="h-72">
            <div className="space-y-1">
              {available
                .filter(c => c.name.includes(memberSearch) || c.dept.includes(memberSearch))
                .map(contact => (
                  <div
                    key={contact.name}
                  ...
                  >
                    ...
                    >
                      {selectedMembers.includes(contact.name) && <Check className="w-3 h-3 text-primary-foreground" />}
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">{contact.dept}</p>
                    </div>
                  </div>
                ))}
              {available.filter(c => c.name.includes(memberSearch) || c.dept.includes(memberSearch)).length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-6">沒有可邀請的同事</p>
              )}
            </div>
          </ScrollArea>
          <Button className="w-full mt-4" disabled={selectedMembers.length === 0} onClick={handleAddMembers}>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1.5 block">
                選擇成員 {selectedMembers.length > 0 && `(已選 ${selectedMembers.length})`}
              </label>
              <Input
                placeholder="搜索同事..."
                value={memberSearch}
