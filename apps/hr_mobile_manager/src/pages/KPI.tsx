...
const getSubmitLabel = (phase: Phase) => {
  if (phase === 'self') return '提交自評';
  if (phase === 'peer') return '提交互評';
  return '提交主管評核';
};

const getEditorTitle = (phase: Phase, peerName: string) => {
  if (phase === 'self') return '自我評分';
  if (phase === 'peer') return `評核 - ${peerName}`;
  return '主管評分';
};

const getPhaseShortLabel = (phase: Phase) => {
  if (phase === 'self') return '自評';
  if (phase === 'peer') return '互評';
  return '主管評核';
};
...
const KPI = () => {
  ...
  const peerAvg = getWeightedAvg(k =>
    k.peerScores.length ? Math.round(k.peerScores.reduce((s, p) => s + p.score, 0) / k.peerScores.length) : null
  );
    ...
    setKpiItems(prev =>
      prev.map(k => {
        ...
        if (phase === 'peer') {
          ...
          return {
            ...k,
            peerScores: [...filtered, { name: peers[currentPeerIndex], score: tempScore, comment: tempComment }]
          };
        }
  ...
  const submitPhase = () => {
    if (phase === 'self') {
      setPhase('peer');
      toast.success('自評已提交，進入互評階段');
    } else if (phase === 'peer') {
      setPhase('manager');
      toast.success('互評已提交，進入主管評核');
    } else if (phase === 'manager') {
      setPhase('completed');
      toast.success('評核流程已完成！');
    }
    ...
    <MobileLayout title="評價與KPI">
      <div className="px-5 pt-4 pb-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span
            className={`text-xs px-2.5 py-1 rounded-full font-medium flex items-center gap-1 ${currentPhase.bgColor} ${currentPhase.color}`}
          >
          ...
          {(['self', 'peer', 'manager', 'completed'] as Phase[]).map((p, i, arr) => {
            ...
            return (
              <div key={p} className="flex items-center flex-1 last:flex-none">
                ...
                {i < arr.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-1 mt-[-12px] ${arr.indexOf(phase) > i ? 'bg-primary' : 'bg-border'}`}
                  />
                )}
              </div>
            );
        ...
        <div className="flex flex-col items-center py-4">
          ...
          >
            <span
              className={`text-3xl font-bold ${phase === 'completed' && finalScore ? getScoreColor(finalScore) : 'text-primary'}`}
            >
              {displayScore || '--'}
            </span>
            <span className="text-[10px] text-muted-foreground">{phase === 'completed' ? '最終得分' : '自評得分'}</span>
          </div>
          {phase === 'completed' && selfAvg !== null && peerAvg !== null && managerAvg !== null && (
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  ...
                  <span>
                    互評：
                    {kpi.peerScores.length
                      ? Math.round(kpi.peerScores.reduce((s, p) => s + p.score, 0) / kpi.peerScores.length)
                      : '待評'}
                  </span>
        ...
        {/* Submit Button */}
        {phase !== 'completed' && (
          <Button className="w-full mt-5" disabled={!canSubmitPhase()} onClick={() => setConfirmOpen(true)}>
        ...
        <DialogContent className="max-w-[92vw] rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-base">
              {getEditorTitle(phase, peers[currentPeerIndex])} · {editingItem?.name}
            </DialogTitle>
            <DialogDescription className="text-xs">請根據實際表現給予 0-100 的評分並填寫評語</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="flex-row gap-2">
            ...
            <Button className="flex-1" onClick={saveScore}>
              確認儲存
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="max-w-[85vw] rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>提交後將無法修改{getPhaseShortLabel(phase)}結果，確定要提交嗎？</DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex-row gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setConfirmOpen(false)}>
              取消
            </Button>
            <Button className="flex-1" onClick={submitPhase}>
              確認提交
            </Button>
          </DialogFooter>
        </DialogContent>
