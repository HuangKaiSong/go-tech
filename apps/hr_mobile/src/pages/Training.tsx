...
const trainingData: TrainingCategory[] = [
  {
    ...
    courses: [
      {
        id: "o1", title: "公司文化與價值觀", duration: "45分鐘", status: "completed", progress: 100,
      ...
      {
        id: "o2", title: "員工守則與規章制度", duration: "60分鐘", status: "completed", progress: 100,
      ...
      {
        id: "o3", title: "資訊安全意識", duration: "30分鐘", status: "in_progress", progress: 60,
      ...
      {
        id: "o4", title: "辦公系統操作指南", duration: "40分鐘", status: "locked", progress: 0,
  ...
  },
  {
    ...
    courses: [
      {
        id: "s1", title: "項目管理基礎", duration: "90分鐘", status: "in_progress", progress: 33,
      ...
      {
        id: "s2", title: "溝通技巧與團隊協作", duration: "60分鐘", status: "locked", progress: 0,
      ...
      {
        id: "s3", title: "數據分析入門", duration: "75分鐘", status: "locked", progress: 0,
...
const Training = () => {
  ...
  const handleModuleComplete = (categoryId: string, courseId: string, moduleIndex: number) => {
    setCourses((prev) =>
      prev.map((cat) => {
    ...
    setSelectedCourse((prev) => {
      ...
      const updatedModules = prev.modules.map((m, i) => i === moduleIndex ? { ...m, completed: true } : m);
      const completedCount = updatedModules.filter((m) => m.completed).length;
      const newProgress = Math.round((completedCount / updatedModules.length) * 100);
      const allDone = completedCount === updatedModules.length;
      return { ...prev, modules: updatedModules, progress: newProgress, status: allDone ? "completed" : "in_progress" };
    });
  };
  ...
  if (view === "learning" && selectedCourse && selectedCategory) {
    return (
      <LearningPage
        ...
        onComplete={() => {
          handleModuleComplete(selectedCategory.id, selectedCourse.id, learningModuleIndex);
          setView("detail");
        }}
    ...
    return (
      <MobileLayout title="課程詳情">
        <div className="px-5 pt-4 pb-6">
          <button onClick={() => { openCategory(selectedCategory); }} className="flex items-center gap-1 text-sm text-primary mb-4">
          ...
          <div className="bg-card rounded-2xl border border-border p-5 mb-4">
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <h2 className="text-lg font-bold text-foreground">{selectedCourse.title}</h2>
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
              ...
              <span className="flex items-center gap-1"><FileText className="w-4 h-4" />{selectedCourse.modules.length} 個模塊</span>
            </div>
            <div className="flex items-center gap-3">
              <Progress value={selectedCourse.progress} className="h-2 flex-1" />
              <span className="text-xs font-semibold text-foreground">{selectedCourse.progress}%</span>
            </div>
          </div>
            ...
            {selectedCourse.modules.map((mod, i) => {
              ...
              return (
                <div key={i} className={`bg-card rounded-xl border p-4 flex items-center gap-3 ${mod.completed ? "border-[hsl(var(--success))]/30" : "border-border"}`}>
                  ...
                  {canStart && (
                    <button
                      onClick={() => { setLearningModuleIndex(i); setView("learning"); }}
  ...
  if (view === "category" && selectedCategory) {
    ...
    const catProgress = freshCat.totalCourses > 0 ? Math.round((freshCat.completedCourses / freshCat.totalCourses) * 100) : 0;
    return (
      <MobileLayout title={freshCat.title}>
        <div className="px-5 pt-4 pb-6">
          <button onClick={() => setView("list")} className="flex items-center gap-1 text-sm text-primary mb-4">
            <ArrowLeft className="w-5 h-5" /> 返回
          </button>
          ...
          <div className={`rounded-2xl p-5 mb-5 ${freshCat.color}`}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-primary-foreground">{freshCat.title}</h2>
                <p className="text-xs text-primary-foreground/70">{freshCat.description}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Progress value={catProgress} className="h-2 flex-1 bg-primary-foreground/20" />
              <span className="text-xs font-semibold text-primary-foreground">{freshCat.completedCourses}/{freshCat.totalCourses}</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {freshCat.courses.map((course) => {
              ...
              return (
                <button
                  key={course.id}
                  onClick={() => course.status !== "locked" && openDetail(course)}
      ...
      <div className="px-5 pt-4 pb-6">
        ...
        <div className="bg-gradient-to-br from-primary to-primary/80 rounded-2xl p-5 mb-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-primary-foreground/70">已完成 {totalCompleted}/{totalCourses} 門課程</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            { label: "進行中", value: courses.reduce((s, c) => s + c.courses.filter((x) => x.status === "in_progress").length, 0), color: "text-[hsl(var(--warning))]" },
          ].map((stat) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-xs text-muted-foreground">{cat.completedCourses}/{cat.totalCourses}</span>
                  </div>
                </div>
