...
import { BlockType, type CommonBlock } from './type';
...
const fileToDataUrl = (file: File) =>
  new Promise<string>((resolve, reject) => {
    ...
    reader.addEventListener('error', () => reject(new Error('Failed to read local file')));
...
export default function Common({
  ...
  element: HTMLElement;
  onPatchBlock: (blockId: string, blockSeq: number | undefined, patch: Partial<CommonBlock>) => void;
  ...
  const blockSeq = element.dataset.blockSeq ? Number(element.dataset.blockSeq) : undefined;
  let targetRole = role;
  if (role === 'phone') targetRole = 'phoneValue';
  else if (role === 'email') targetRole = 'emailValue';

  const isLogo = role === 'logo';
  const targetTextElement = useMemo(() => {
    if (targetRole === role) return element;
    const found = element.querySelector(`[data-block-role="${targetRole}"]`) as HTMLElement | null;
    return found || element;
  }, [element, role, targetRole]);
  const initialText = useMemo(() => targetTextElement.textContent || '', [targetTextElement]);
  const initialSrc = useMemo(() => (element as HTMLImageElement).getAttribute('src') || '', [element]);
  ...
  const applyLogo = async () => {
    ...
    const rawUrl =
      ...
      file?.previewUrl;
    let url = rawUrl && rawUrl.startsWith('/') && h5Host ? `${h5Host.replace(/\/$/, '')}${rawUrl}` : rawUrl;
