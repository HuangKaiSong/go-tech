...
const PromoCodesPage = () => {
  ...
  /** 暂存待编辑条目。plans 是异步加载的，handleOpenEdit 调用时可能还没数据， 先把条目存起来，等 plans 到位后由 useEffect 完成 discounts 初始化。 */
  const [pendingEditCode, setPendingEditCode] = useState<(typeof mockPromoCodes)[0] | null>(null);
  ...
  /** Plans 到位后将其与当前编辑条目的已有 discounts 合并，写入 RHF 的 discounts 字段。 编辑时用已保存的配置预填，未匹配到的套餐也补全为未勾选。 */
  useEffect(() => {
          ...
          <TableBody>
            {mockPromoCodes.map(code => (
              <TableRow key={code.id} className="hover:bg-muted/30">
                ...
                <TableCell className="text-center">
                  <span className={getStatusColor(code.status)}>{code.status}</span>
                </TableCell>
        ...
        <DialogContent className="sm:max-w-125">
          ...
          <form className="space-y-4 py-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <Input placeholder="請輸入數量" type="number" min={1} {...register('quantity')} />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                {errors.codeValue && <p className="text-xs text-destructive mt-1">{errors.codeValue.message}</p>}
              </div>
            </div>
