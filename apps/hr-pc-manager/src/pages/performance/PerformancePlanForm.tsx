...
import { Target, ArrowLeft, Save, Plus, Trash2, GripVertical } from "lucide-react";
...
const existingCriteria: CriterionForm[] = [
  { id: "1", category: "工作業績", name: "專案交付達成率", weight: "25", description: "按時按質完成所負責專案的比率", scoringMethod: "目標達成率計算" },
  { id: "2", category: "工作業績", name: "代碼品質與技術債務", weight: "20", description: "代碼審查通過率、bug 修復速度及技術債務清理", scoringMethod: "量化指標評分" },
  { id: "3", category: "工作能力", name: "團隊協作與溝通", weight: "20", description: "跨部門協作效率、知識分享及溝通能力", scoringMethod: "360度回饋評分" },
  { id: "4", category: "工作能力", name: "問題解決與創新", weight: "15", description: "面對技術難題的解決能力及創新貢獻", scoringMethod: "案例評估" },
  { id: "5", category: "工作態度", name: "工作積極性與責任感", weight: "10", description: "工作主動性、責任心及對團隊目標的貢獻", scoringMethod: "行為觀察評分" },
  { id: "6", category: "工作態度", name: "學習成長與自我提升", weight: "10", description: "專業技能提升、學習計畫執行及職業發展", scoringMethod: "成果記錄評估" },
];
...
export default function PerformancePlanForm() {
  ...
  const [form, setForm] = useState({
    ...
    selfWeight: isEdit ? "20" : "20",
    peerWeight: isEdit ? "30" : "30",
    managerWeight: isEdit ? "50" : "50",
  ...
  const addCriterion = () => {
    setCriteria(prev => [...prev, {
      id: String(nextId++), category: "工作業績", name: "", weight: "", description: "", scoringMethod: "量化指標評分",
    }]);
  };
  ...
  const handleSave = (asDraft: boolean) => {
    ...
    toast({ title: asDraft ? "已儲存為草稿" : "方案已儲存", description: `考核方案「${form.name}」已成功${isEdit ? "更新" : "建立"}` });
    ...
    <div>
      <div className="page-header">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => handleSave(true)}>儲存草稿</Button>
            <Button size="sm" onClick={() => handleSave(false)}>
              <Save className="h-4 w-4 mr-1" />{isEdit ? "更新方案" : "建立方案"}
            </Button>
          </div>
        </div>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div className="space-y-2 sm:col-span-2 lg:col-span-1">
              <Label>方案名稱 *</Label>
              <Input placeholder="例：2026 Q2 績效考核" value={form.name} onChange={e => updateForm("name", e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>週期類型 *</Label>
              <Select value={form.cycle} onValueChange={v => updateForm("cycle", v)}>
                <SelectTrigger><SelectValue placeholder="選擇週期" /></SelectTrigger>
                <SelectContent>
          ...
          <div className="mt-5 space-y-2">
            <Label>方案說明</Label>
            <Textarea rows={3} placeholder="描述此考核方案的目的與範圍..." value={form.description} onChange={e => updateForm("description", e.target.value)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>自評、互評、主管評的權重比例，總和需為 100%{evalTotalWeight !== 100 && <span className="text-destructive ml-2">（目前 {evalTotalWeight}%）</span>}</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div className="space-y-2">
              ...
              <Input type="date" value={form.managerDeadline} onChange={e => updateForm("managerDeadline", e.target.value)} />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={addCriterion}>
              <Plus className="h-4 w-4 mr-1" />新增指標
            </Button>
          </div>
        </CardHeader>
        <CardContent>
            ...
            {criteria.map((c, idx) => (
              <div key={c.id} className="rounded-lg border p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => removeCriterion(c.id)}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input className="h-9" placeholder="指標名稱" value={c.name} onChange={e => updateCriterion(c.id, "name", e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">權重 (%)</Label>
                    <Input className="h-9" type="number" min="0" max="100" placeholder="權重" value={c.weight} onChange={e => updateCriterion(c.id, "weight", e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                ...
                <div className="mt-3 space-y-1.5">
                  ...
                  <Input className="h-9" placeholder="描述此指標的衡量標準" value={c.description} onChange={e => updateCriterion(c.id, "description", e.target.value)} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex justify-end gap-2 pb-8">
        ...
        <Button variant="outline" onClick={() => handleSave(true)}>儲存草稿</Button>
