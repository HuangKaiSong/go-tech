import { ArrowLeft, CheckCircle, Edit2, MessageSquare, Save, Star, Target, User, UserCheck, Users } from 'lucide-react';
...
interface EvalCriterion {
  ...
  name: string;
  peerReviews: { comment: string; department: string; reviewer: string; score: number }[];
  selfComment: string;
...
const mockCriteria: EvalCriterion[] = [
  {
    ...
    selfScore: 92,
    selfComment: '本季度負責 3 個核心專案，均按時交付，其中支付系統重構提前一週完成，代碼品質通過率 98%。',
    peerReviews: [
      {
        reviewer: '黃志偉',
        department: '技術部',
        score: 90,
        comment: '技術能力紮實，程式碼品質高，Code Review 反饋及時有效。'
      },
    ...
    managerScore: 94,
    managerComment: '專案管理能力突出，交付品質穩定，是團隊技術骨幹。建議承擔更多架構設計工作。'
  },
  {
    id: 'C2',
    category: '工作業績',
    ...
    selfScore: 88,
    selfComment: '重構了 3 個核心模組，減少技術債務約 30%，引入自動化測試覆蓋率提升至 85%。',
    peerReviews: [
      {
        reviewer: '黃志偉',
        department: '技術部',
        score: 92,
        comment: '重構方案設計合理，團隊成員受益匪淺，測試覆蓋率大幅提升。'
      },
      { reviewer: '王美玲', department: '人事部', score: 85, comment: '系統穩定性明顯改善，HR 系統故障率下降。' }
    ],
    managerScore: 90,
    managerComment: '技術債務清理工作做得很好，但需注意平衡新功能開發與重構的時間分配。'
  },
  {
    id: 'C3',
    category: '工作能力',
    ...
    selfScore: 85,
    selfComment: '主導了 4 次技術分享會，協助 2 位新人完成入職培訓，跨部門需求對接效率提升。',
    peerReviews: [
      { reviewer: '黃志偉', department: '技術部', score: 90, comment: '技術分享內容豐富實用，對新人指導耐心細緻。' },
      {
        ...
        department: '銷售部',
        score: 86,
        comment: '回應速度快，能用淺顯語言解釋技術問題，溝通效果好。'
      },
      {
        reviewer: '陳大偉',
        department: '市場部',
        score: 84,
        comment: '跨部門合作態度積極，偶爾在需求優先級協調上有延遲。'
      }
    ],
    managerScore: 88,
    managerComment: '團隊影響力持續增強，知識分享做得出色。建議加強對下游團隊的主動溝通。'
  },
  {
    id: 'C4',
    category: '工作能力',
    ...
    selfScore: 90,
    selfComment: '提出並實現了快取優化方案，API 回應時間降低 40%。解決了 2 個長期存在的疑難 bug。',
    peerReviews: [
    ...
    managerScore: 95,
    managerComment: '創新能力是團隊亮點，快取優化方案已作為最佳實踐推廣。期待更多創新提案。'
  },
  {
    id: 'C5',
    category: '工作態度',
    ...
    selfScore: 92,
    selfComment: '主動承擔緊急需求，多次在週末協助排查線上問題，確保系統穩定運行。',
    peerReviews: [
    ...
    managerScore: 93,
    managerComment: '責任感和主動性值得表揚，是團隊可靠的核心成員。注意保持工作與生活平衡。'
  },
  {
    id: 'C6',
    category: '工作態度',
    ...
    selfScore: 88,
    selfComment: '完成雲端架構師認證考試，閱讀了 3 本技術書籍，參加 2 場外部技術研討會。',
    peerReviews: [
      { reviewer: '黃志偉', department: '技術部', score: 86, comment: '持續學習態度好，新技術應用能力強。' }
    ],
    managerScore: 90,
    managerComment: '學習計畫執行到位，建議將所學更多應用於實際專案，並帶領團隊共同成長。'
  }
];
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(s => (
        <Star
          key={s}
          className={`h-3.5 w-3.5 ${s <= stars ? 'text-warning fill-warning' : 'text-muted-foreground/20'}`}
        />
      ))}
...
export default function PerformanceEvalDetail() {
  const { evalId: _evalId } = useParams();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('overview');
  const [isScoring, setIsScoring] = useState(false);
  const [scores, setScores] = useState<Record<string, { comment: string; score: string }>>(
    Object.fromEntries(
      mockCriteria.map(c => [c.id, { score: String(c.managerScore ?? ''), comment: c.managerComment }])
    )
  );

  const { manager: wMgr, peer: wPeer, self: wSelf } = mockEmployee.weightConfig;

  const calcFinal = (c: EvalCriterion) => {
    if (c.selfScore === null || c.managerScore === null) return null;
    const peerAvg =
      c.peerReviews.length > 0 ? c.peerReviews.reduce((s, p) => s + p.score, 0) / c.peerReviews.length : 0;
  ...
  // ── Workflow steps
  const steps = [
    { label: '員工自評', date: mockEmployee.selfSubmittedAt, done: Boolean(mockEmployee.selfSubmittedAt), icon: User },
    { label: '同事互評', date: mockEmployee.peerCompletedAt, done: Boolean(mockEmployee.peerCompletedAt), icon: Users },
    {
      label: '主管評價',
      date: mockEmployee.managerReviewedAt,
      done: Boolean(mockEmployee.managerReviewedAt),
      icon: UserCheck
    },
    {
      label: '評估完成',
      date: mockEmployee.managerReviewedAt,
      done: mockEmployee.status === '已完成',
      icon: CheckCircle
    }
  ];
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="page-description">
              {mockEmployee.name} — {mockEmployee.plan}
            </p>
          </div>
          <div className="flex gap-2">
            {!isScoring ? (
              ...
              <>
                <Button variant="outline" size="sm" onClick={() => setIsScoring(false)}>
                  取消
                </Button>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {mockEmployee.department} · {mockEmployee.position} · {mockEmployee.employeeId}
                  </p>
                  <p className="text-sm text-muted-foreground mt-0.5">
                    {mockEmployee.plan} · {mockEmployee.cycle}
                  </p>
                </div>
              </div>
              <Badge variant="secondary" className={`${statusColors[mockEmployee.status] || ''} text-sm px-3 py-1`}>
                {mockEmployee.status}
              </Badge>
            </div>
            ...
            <div className="space-y-4">
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div
                    className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${step.done ? 'bg-success/10' : 'bg-muted'}`}
                  >
        ...
        <TabsContent value="overview">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                自評（{wSelf}%）、同事互評（{wPeer}%）、主管評分（{wMgr}%）加權計算最終得分
              </CardDescription>
            </CardHeader>
                  ...
                  {mockCriteria.map(c => {
                    const peerAvg =
                      c.peerReviews.length > 0
                        ? c.peerReviews.reduce((s, p) => s + p.score, 0) / c.peerReviews.length
                        : null;
                    ...
                    return (
                      <TableRow key={c.id}>
                        ...
                        <TableCell className="text-center">
                          <ScoreBadge score={c.selfScore} />
                        </TableCell>
                        <TableCell className="text-center">
                          <ScoreBadge score={peerAvg ? Math.round(peerAvg) : null} />
                        </TableCell>
                        <TableCell className="text-center">
                          <ScoreBadge score={c.managerScore} />
                        </TableCell>
                        <TableCell className="text-center">
                  ...
                  <TableRow className="bg-muted/30 font-semibold">
                    ...
                    <TableCell className="text-center">
                      <Badge className="bg-primary text-primary-foreground">
                        {overallScore.toFixed(1)} ({getGrade(overallScore)})
                      </Badge>
                    </TableCell>
                  </TableRow>
                </TableBody>
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  提交時間：{mockEmployee.selfSubmittedAt}　權重佔比：{wSelf}%
                </CardDescription>
              </CardHeader>
            </Card>
            {categories.map(cat => (
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {item.description} · 權重 {item.weight}%
                              </p>
                            </div>
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  完成時間：{mockEmployee.peerCompletedAt}　權重佔比：{wPeer}%　共{' '}
                  {new Set(mockCriteria.flatMap(c => c.peerReviews.map(p => p.reviewer))).size} 位同事參與
                </CardDescription>
              </CardHeader>
            </Card>
            {categories.map(cat => (
                        ...
                        <CardContent className="p-5">
                          <div className="flex items-center justify-between mb-3">
                            <div>
                              <h4 className="font-semibold">{item.name}</h4>
                              <p className="text-xs text-muted-foreground">
                                權重 {item.weight}% · 共 {item.peerReviews.length} 位評價
                              </p>
                            </div>
                            <Badge variant="secondary">
                              均分：
                              {(
                                item.peerReviews.reduce((s, p) => s + p.score, 0) / (item.peerReviews.length || 1)
                              ).toFixed(1)}
                            </Badge>
                          </div>
                          <Separator className="my-3" />
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {isScoring
                    ? '編輯模式：修改各項指標評分與評語後點擊儲存'
                    : `評價時間：${mockEmployee.managerReviewedAt}　權重佔比：${wMgr}%`}
                </CardDescription>
              </CardHeader>
            </Card>
            {categories.map(cat => (
                              ...
                              <div className="space-y-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <Select
                                    ...
                                    onValueChange={v =>
                                      setScores(prev => ({ ...prev, [item.id]: { ...prev[item.id], score: v } }))
                                    }
                                  >
                                    <SelectTrigger className="w-[120px]">
                                      <SelectValue placeholder="選擇分數" />
                                    </SelectTrigger>
                                    <SelectContent>
