...
import {
  ...
  Calendar, Briefcase, Edit, Save, X, Check,
} from "lucide-react";
...
const employeesDetail: Record<string, any> = {
  EMP001: {
    ...
    department: "技術部", position: "高級工程師", positionCode: "P001",
    positionGroup: "PG-TECH", departmentCode: "D01", departmentSub: "D01-BE",
    joinDate: "2022-03-15", probation: 3, probationPassDate: "2022-06-15",
    employmentType: "全職", partTime: false, flexibleHours: true,
    dailyHours: 8, weeklyDays: 5, annualHours: 2080,
    shiftType: "固定班", shiftCode: "A班",
    salaryType: "月薪", baseSalary: 85000, paymentType: "銀行轉帳",
  ...
  EMP002: {
    ...
    department: "銷售部", position: "銷售經理", positionCode: "P002",
    positionGroup: "PG-SALES", departmentCode: "D02", departmentSub: "",
    joinDate: "2021-07-01", probation: 3, probationPassDate: "2021-10-01",
    employmentType: "全職", partTime: false, flexibleHours: false,
    dailyHours: 8, weeklyDays: 5, annualHours: 2080,
    shiftType: "固定班", shiftCode: "A班",
    salaryType: "月薪", baseSalary: 72000, paymentType: "銀行轉帳",
    payrollGroup: "月結A組", leaveCategory: "年假A類", leaveGroup: "LG-01",
    contributionType: "標準供款", mpfType: "僱員強制", empMpfType: "標準",
...
const defaultEmployee: any = {
  ...
  department: "—", position: "—", positionCode: "—", positionGroup: "—",
  departmentCode: "—", departmentSub: "—", joinDate: "—", probation: 0,
  probationPassDate: "—", employmentType: "—", partTime: false, flexibleHours: false,
  dailyHours: 0, weeklyDays: 0, annualHours: 0, shiftType: "—", shiftCode: "—",
 ...
 */
type EditPerm = "readonly" | "hr_only" | "self_or_hr" | "view_restricted";

const FIELD_EDIT_PERM: Record<string, EditPerm> = {
  // Readonly — system
  id: "readonly", joinDate: "readonly",
  // Personal — self or HR
  name: "self_or_hr", englishName: "self_or_hr", gender: "self_or_hr",
  birthday: "self_or_hr", nationality: "self_or_hr", maritalStatus: "self_or_hr",
  phone: "self_or_hr", email: "self_or_hr", address: "self_or_hr",
  emergencyName: "self_or_hr", emergencyPhone: "self_or_hr", emergencyRelation: "self_or_hr",
  education: "self_or_hr", school: "self_or_hr", major: "self_or_hr",
  bankName: "self_or_hr", bankAccount: "self_or_hr",
  // View restricted + HR edit
  idNumber: "view_restricted",
  // HR only — org & salary
  department: "hr_only", position: "hr_only", positionCode: "hr_only",
  positionGroup: "hr_only", departmentCode: "hr_only", departmentSub: "hr_only",
  employmentType: "hr_only", storeCategory: "hr_only",
  probation: "hr_only", probationPassDate: "hr_only",
  shiftType: "hr_only", shiftCode: "hr_only",
  dailyHours: "hr_only", weeklyDays: "hr_only", annualHours: "hr_only",
  partTime: "hr_only", flexibleHours: "hr_only", overtimeLeave: "hr_only",
  leaveCategory: "hr_only", leaveGroup: "hr_only",
  salaryType: "hr_only", baseSalary: "hr_only", paymentType: "hr_only",
  payrollGroup: "hr_only", adwOvertime: "hr_only", bankRatio: "hr_only",
  contributionType: "hr_only", mpfType: "hr_only", empMpfType: "hr_only",
  employerMpf: "hr_only", orsoType: "hr_only",
  orsoEmpRatio: "hr_only", orsoEmployerRatio: "hr_only", employerOrso: "hr_only",
  status: "hr_only",
};
...
function EditableField({ label, fieldKey, value, editing, role, formData, onChange, type, colSpan }: {
  label: string; fieldKey: string; value: string | number | boolean;
  editing: boolean; role: UserRole;
      ...
      {isEditing ? (
        inferredType === "select" && selectOpts ? (
          <Select value={String(formData[fieldKey] ?? value)} onValueChange={(v) => onChange(fieldKey, v)}>
            ...
            <SelectContent>
              {selectOpts.map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}
            </SelectContent>
          </Select>
...
function SecureField({ label, value, fieldKey, role, editing, formData, onChange }: {
  label: string; value: string | number; fieldKey: string; role: UserRole;
    ...
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        {editing && sectionEditable && (
          <Badge variant="secondary" className="text-[10px] px-1.5 py-0 bg-primary/10 text-primary border-primary/20">可編輯</Badge>
        )}
        {editing && !sectionEditable && (
          <Badge variant="secondary" className="text-[10px] px-1.5 py-0">僅限查看</Badge>
        )}
      </div>
      {children}
    </div>
...
export default function EmployeeDetail() {
  ...
  const fp = (fieldKey: string, label: string, opts?: { type?: "text" | "number" | "date" | "select" | "boolean"; colSpan?: number }) => ({
    label, fieldKey, value: emp[fieldKey], editing, role: currentRole, formData, onChange: handleChange, ...opts,
  });
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />入職 {emp.joinDate}</span>
                <span>在職 {tenure}</span>
              </div>
            </div>
          </div>
            ...
            {editing ? (
              <>
                ...
                <Button size="sm" onClick={handleSave}>
                  <Check className="h-3.5 w-3.5 mr-1.5" />儲存
                </Button>
              </>
            ) : (
              <Button variant="outline" size="sm" onClick={() => setEditing(true)}>
                <Edit className="h-3.5 w-3.5 mr-1.5" />編輯
              </Button>
            )}
          </div>
        </div>
      ...
      {editing && (
        <div className="rounded-lg bg-primary/5 border border-primary/15 px-4 py-2.5 mb-6 flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">
            編輯模式 — 標示
            <Badge variant="secondary" className="text-[10px] px-1.5 py-0 mx-1 bg-primary/10 text-primary border-primary/20">可編輯</Badge>
        ...
        <div className="flex-1 min-w-0">
          {activeSection === "personal" && (
            <div className="space-y-8">
              <Section icon={User} title="基本資料" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr" || currentRole === "employee"}>
                <FieldGrid>
                  ...
                  <SecureField label="身份證號碼" value={emp.idNumber} fieldKey="idNumber" role={currentRole} editing={editing} formData={formData} onChange={handleChange} />
              ...
              <Section icon={Phone} title="聯絡資訊" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr" || currentRole === "employee"}>
                <FieldGrid>
              ...
              <Section icon={AlertTriangle} title="緊急聯絡人" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr" || currentRole === "employee"}>
                <FieldGrid>
              ...
              <Section icon={GraduationCap} title="學歷資訊" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr" || currentRole === "employee"}>
                <FieldGrid>
          ...
          {activeSection === "organization" && (
            <div className="space-y-8">
              <Section icon={Building2} title="職位資訊" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr"}>
                <FieldGrid>
              ...
              <Section icon={Clock} title="工作安排" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr"}>
                <FieldGrid>
              ...
              <Section icon={Calendar} title="出勤與假期" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr"}>
                <FieldGrid>
          ...
          {activeSection === "salary" && (
            <>
              {!canView("salary", currentRole) ? (
                ...
                <div className="space-y-8">
                  <Section icon={Wallet} title="薪資設定" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr"}>
                    <FieldGrid>
                  ...
                  <Section icon={Landmark} title="銀行帳戶" locked editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr" || currentRole === "employee"}>
                    <FieldGrid>
                      <SecureField label="銀行名稱" value={emp.bankName} fieldKey="bankName" role={currentRole} editing={editing} formData={formData} onChange={handleChange} />
                      <SecureField label="銀行帳號" value={emp.bankAccount} fieldKey="bankAccount" role={currentRole} editing={editing} formData={formData} onChange={handleChange} />
                  ...
                  <Section icon={Shield} title="強積金 / 退休計劃" editing={editing} sectionEditable={currentRole === "admin" || currentRole === "hr"}>
                    <FieldGrid>
