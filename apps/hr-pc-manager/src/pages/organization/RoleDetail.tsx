...
const membersDB: Record<
  string,
  { email: string; empId: string; joinDate: string; name: string; phone: string; status: string }[]
> = {
  R001: [
    {
      name: '王大明',
...
type PermLevel = 'edit' | 'full' | 'none' | 'view';

const permLevelLabels: Record<PermLevel, { color: string; icon: typeof Eye; label: string }> = {
...
const getDefaultPermissions = (level: string): Record<string, PermLevel> => {
  ...
  const rank = Number.parseInt(level[1], 10);
  ...
  permissionModules.forEach(mod => {
    mod.children.forEach(child => {
      if (isManager && rank >= 2) {
        ...
        perms[child.id] =
          child.id.startsWith('employee.self') || child.id.startsWith('attendance.records') ? 'view' : 'none';
      }
    });
  });
  return perms;
...
export default function RoleDetail() {
  ...
  const [permissions, setPermissions] = useState<Record<string, PermLevel>>(getDefaultPermissions(role?.level || 'P1'));
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant="secondary"
                className={role.status === '啟用' ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}
              >
                {role.status}
              </Badge>
            </div>
            <p className="page-description">
              {role.department} · 建立於 {role.createdAt}
            </p>
          </div>
        </div>
        {editing ? (
                        ...
                        if (field.key === 'department') {
                          return (
                            ...
                            >
                              ...
                              <SelectContent>
                                {['技術部', '銷售部', '運營部', '人力資源部', '財務部', '行政部'].map(d => (
                  ...
                  <TableBody>
                    {members.map(m => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge variant="outline" className={statusMemberColors[m.status] || ''}>
                            {m.status}
                          </Badge>
                        </TableCell>
                      </TableRow>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setPermissions(getDefaultPermissions(role.level));
                      }}
                    >
                      <X className="h-4 w-4 mr-1" />
                      取消
          ...
          {selectedMember && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <p className="text-muted-foreground">工號</p>
                  <p className="font-medium">{selectedMember.empId}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">狀態</p>
                  <Badge variant="outline" className={statusMemberColors[selectedMember.status] || ''}>
                    {selectedMember.status}
                  </Badge>
                </div>
                <div>
                  <p className="text-muted-foreground">到職日期</p>
                  <p className="font-medium">{selectedMember.joinDate}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">職等</p>
                  <Badge variant="outline" className={levelColors[role.level] || ''}>
                    {role.level}
                  </Badge>
                </div>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  navigate(`/employees/${selectedMember.empId}`);
                }}
              >
