...
import { ArrowLeft, Plus, Trash2, GripVertical, Save, Settings2, GitBranch, AlertTriangle } from "lucide-react";
...
export default function ApprovalRuleForm() {
  ...
  const [levels, setLevels] = useState<LevelForm[]>(
    isEdit
      ? [
          ...
          { id: genId(), name: "總經理審批", approverType: "指定人員", approver: "陳總經理", condition: "請假天數 > 3 天" },
        ]
  ...
  const addLevel = () => setLevels((p) => [...p, { id: genId(), name: "", approverType: "直屬主管", approver: "", condition: "" }]);
  ...
  const addCondition = () => setConditions((p) => [...p, { id: genId(), field: "", operator: "=", value: "", action: "" }]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleSave}><Save className="h-4 w-4 mr-2" />儲存</Button>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={type} onValueChange={setType}>
                    ...
                    <SelectContent>
                      {approvalTypes.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select value={lvl.approverType} onValueChange={(v) => updateLevel(lvl.id, "approverType", v)}>
                        ...
                        <SelectContent>
                          {approverTypes.map((a) => <SelectItem key={a.value} value={a.value}>{a.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">審批人</Label>
                      <Input value={lvl.approver} onChange={(e) => updateLevel(lvl.id, "approver", e.target.value)} placeholder="人員/角色名稱" />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">觸發條件（選填）</Label>
                    <Input value={lvl.condition} onChange={(e) => updateLevel(lvl.id, "condition", e.target.value)} placeholder="例如：金額 > 30,000 時觸發此層級" />
                  </div>
                </div>
              ))}
            </CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={addCondition} disabled={!type}><Plus className="h-4 w-4 mr-1" />新增條件</Button>
              </div>
            </CardHeader>
              ...
              {conditions.map((c) => (
                <div key={c.id} className="flex items-end gap-2 p-3 border rounded-lg bg-muted/30">
                  ...
                  <div className="space-y-1.5 flex-1">
                    ...
                    <Input value={c.value} onChange={(e) => updateCondition(c.id, "value", e.target.value)} placeholder="條件值" />
                  </div>
                  <div className="space-y-1.5 flex-1">
                    <Label className="text-xs">動作</Label>
                    <Input value={c.action} onChange={(e) => updateCondition(c.id, "action", e.target.value)} placeholder="觸發動作" />
                  </div>
                  <Button variant="ghost" size="icon" className="h-9 w-9 text-destructive shrink-0" onClick={() => removeCondition(c.id)}>
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
              ))}
              {conditions.length === 0 && type && (
                <p className="text-sm text-muted-foreground text-center py-4">尚未設定條件規則，所有申請將走完整審批流程</p>
              )}
            </CardContent>
          </Card>
        </div>
