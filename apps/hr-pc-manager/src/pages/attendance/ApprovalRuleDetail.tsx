...
interface RuleData {
  ...
  conditions: { label: string; operator: string; value: string; action: string }[];
  settings: { autoApprove: boolean; autoApproveCondition: string; timeLimit: number; overtimeAction: string; allowWithdraw: boolean; notifyApplicant: boolean; notifyNextApprover: boolean };
}

const mockRules: Record<string, RuleData> = {
  "1": {
    id: "1", name: "請假審批流程", type: "請假申請", applyScope: "全公司",
    description: "適用於全公司員工的請假審批流程，根據請假天數自動匹配不同的審批層級。",
    enabled: true, creator: "系統管理員", createdAt: "2026-01-01", updatedAt: "2026-02-20",
    levels: [
      { level: 1, name: "部門主管審批", approverType: "直屬主管", approver: "自動匹配" },
      { level: 2, name: "人事部審核", approverType: "指定角色", approver: "人事專員" },
      { level: 3, name: "總經理審批", approverType: "指定人員", approver: "陳總經理", condition: "請假天數 > 3 天時觸發" },
    ],
    conditions: [
    ...
    settings: { autoApprove: false, autoApproveCondition: "", timeLimit: 48, overtimeAction: "自動提醒", allowWithdraw: true, notifyApplicant: true, notifyNextApprover: true },
  },
  "2": {
    id: "2", name: "報銷審批流程", type: "報銷申請", applyScope: "全公司",
    description: "根據報銷金額自動匹配審批層級，確保大額報銷經過充分審核。",
    enabled: true, creator: "系統管理員", createdAt: "2026-01-01", updatedAt: "2026-02-18",
    levels: [
      { level: 1, name: "部門主管審批", approverType: "直屬主管", approver: "自動匹配" },
    ...
    settings: { autoApprove: false, autoApproveCondition: "", timeLimit: 72, overtimeAction: "自動提醒", allowWithdraw: true, notifyApplicant: true, notifyNextApprover: true },
  },
  "7": {
    id: "7", name: "離職審批流程", type: "離職申請", applyScope: "全公司",
    description: "適用於全公司員工的離職審批流程，根據員工在職天數及類型自動匹配審批層級，確保離職交接有序進行。",
    enabled: true, creator: "系統管理員", createdAt: "2026-02-15", updatedAt: "2026-03-01",
    levels: [
      { level: 1, name: "部門主管審批", approverType: "直屬主管", approver: "自動匹配" },
      { level: 2, name: "人事部審核", approverType: "指定角色", approver: "人事專員" },
      { level: 3, name: "人事經理確認", approverType: "指定角色", approver: "人事經理", condition: "正式員工觸發" },
      { level: 4, name: "總經理審批", approverType: "指定人員", approver: "陳總經理", condition: "在職超過 1 年或主管級以上" },
    ],
    conditions: [
    ...
    settings: { autoApprove: false, autoApproveCondition: "", timeLimit: 72, overtimeAction: "自動提醒並升級", allowWithdraw: true, notifyApplicant: true, notifyNextApprover: true },
  },
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="destructive" size="icon" onClick={() => { toast.success("規則已刪除"); navigate("/attendance/approval"); }}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                <div><p className="text-xs text-muted-foreground mb-1">申請類型</p><Badge className={`${typeColors[data.type] || ""} border`}>{data.type}</Badge></div>
                ...
                <div><p className="text-xs text-muted-foreground mb-1">審批層級</p><Badge variant="outline">{data.levels.length} 級</Badge></div>
                  ...
                  <div key={lvl.level} className="flex items-start gap-2">
                    ...
                    <div className="flex flex-col items-center">
                      <div className="w-28 rounded-lg border-2 border-border bg-card p-3 text-center">
                        ...
                        <p className="text-[10px] text-muted-foreground mt-1">{lvl.approverType}：{lvl.approver}</p>
                        {lvl.condition && (
                          <p className="text-[10px] text-warning mt-1 flex items-center justify-center gap-0.5">
                            <AlertTriangle className="h-2.5 w-2.5" />{lvl.condition}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-3">
                {data.conditions.map((c, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border">
                    <Badge variant="outline" className="shrink-0">{c.label}</Badge>
                    <span className="text-sm font-mono text-foreground">{c.operator} {c.value}</span>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={data.settings.allowWithdraw ? "default" : "outline"}>{data.settings.allowWithdraw ? "是" : "否"}</Badge>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant={data.settings.notifyApplicant ? "default" : "outline"}>{data.settings.notifyApplicant ? "是" : "否"}</Badge>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">通知下一審批人</span>
                <Badge variant={data.settings.notifyNextApprover ? "default" : "outline"}>{data.settings.notifyNextApprover ? "是" : "否"}</Badge>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">自動審批</span>
                <Badge variant={data.settings.autoApprove ? "default" : "outline"}>{data.settings.autoApprove ? "已啟用" : "未啟用"}</Badge>
              </div>
            </CardContent>
          </Card>
        </div>
