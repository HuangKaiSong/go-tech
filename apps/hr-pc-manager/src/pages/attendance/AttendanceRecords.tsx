import { BarChart3, ChevronLeft, ChevronRight, Clock, Download, Filter, Plus, Search, Table2 } from 'lucide-react';
...
export default function AttendanceRecords() {
  ...
  const dailyTrend = useMemo(() => {
    ...
    filteredData.forEach(r => {
      ...
      days[d].total += 1;
      if (r.status === '正常') days[d].normal += 1;
    });
  ...
  const handlePrevMonth = () => {
    const m = Number.parseInt(month === 'all' ? '01' : month, 10);
    if (m <= 1) {
      setYear(String(Number.parseInt(year, 10) - 1));
      setMonth('12');
    } else setMonth(String(m - 1).padStart(2, '0'));
  };
  const handleNextMonth = () => {
    const m = Number.parseInt(month === 'all' ? '12' : month, 10);
    if (m >= 12) {
      setYear(String(Number.parseInt(year, 10) + 1));
      setMonth('01');
    } else setMonth(String(m + 1).padStart(2, '0'));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">出勤狀態分布</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">各部門出勤統計</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
          ...
          <Card className="lg:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">每日出勤率趨勢</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
