...
import {
  MapPin, Clock, Shield, Plus, Pencil, Trash2, Copy,
  Building2, Wifi, Smartphone, Settings2, Search, Loader2, Navigation
} from "lucide-react";
...
const defaultLocations: ClockLocation[] = [
  { id: "L1", name: "總部大樓", address: "台北市信義區信義路五段7號", lat: "25.0330", lng: "121.5654", radius: 200, wifiSSID: "HQ-Office", ruleId: "R1", enabled: true },
  { id: "L2", name: "新竹研發中心", address: "新竹市東區光復路二段101號", lat: "24.8015", lng: "120.9718", radius: 150, wifiSSID: "RD-Center", ruleId: "R1", enabled: true },
  { id: "L3", name: "台中分公司", address: "台中市西屯區台灣大道三段99號", lat: "24.1627", lng: "120.6466", radius: 300, wifiSSID: "", ruleId: "R2", enabled: false },
];

const defaultSchedules: ClockSchedule[] = [
  { id: "S1", name: "標準班", type: "fixed", workStart: "09:00", workEnd: "18:00", lateGrace: 5, earlyLeaveGrace: 5, breakStart: "12:00", breakEnd: "13:00", workDays: [1, 2, 3, 4, 5], enabled: true },
  { id: "S2", name: "彈性班", type: "flexible", workStart: "08:00", workEnd: "17:00", lateGrace: 30, earlyLeaveGrace: 0, breakStart: "12:00", breakEnd: "13:00", workDays: [1, 2, 3, 4, 5], enabled: true },
  { id: "S3", name: "輪班制 A", type: "shift", workStart: "07:00", workEnd: "15:00", lateGrace: 5, earlyLeaveGrace: 5, breakStart: "11:00", breakEnd: "11:30", workDays: [1, 2, 3, 4, 5, 6], enabled: true },
];

const defaultRules: ClockRule[] = [
  { id: "R1", name: "預設打卡規則", allowMethods: ["gps", "wifi", "face"], requirePhoto: false, requireLocation: true, allowRemote: true, remoteApproval: true, overtimeAuto: true, overtimeMinMinutes: 30, missedClockAllowAppeal: true, appealDeadlineDays: 3, enabled: true },
  { id: "R2", name: "嚴格模式", allowMethods: ["gps", "face"], requirePhoto: true, requireLocation: true, allowRemote: false, remoteApproval: false, overtimeAuto: false, overtimeMinMinutes: 60, missedClockAllowAppeal: true, appealDeadlineDays: 1, enabled: false },
];
...
function LocationDialog({
  open, onOpenChange, editing, form, setForm, onSave, rules,
}: {
  open: boolean;
  ...
  const searchAddress = useCallback(async () => {
    if (!searchQuery.trim()) { toast.error("請輸入搜索地址"); return; }
    setSearching(true);
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <Label>地點名稱 *</Label>
              <Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="例：總部大樓" />
            </div>
            <div className="col-span-2">
              <Label>地址 *</Label>
              <Input value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} placeholder="詳細地址" />
            </div>
            <div>
              <Label>緯度</Label>
            ...
            <div>
              ...
              <Input value={form.wifiSSID} onChange={e => setForm(p => ({ ...p, wifiSSID: e.target.value }))} placeholder="辦公室 Wi-Fi 名稱" />
            </div>
          </div>
          ...
          {mapUrl && (
            <div>
              <Label className="mb-1.5 block flex items-center gap-1.5">
                <MapPin className="h-4 w-4 text-primary" />地圖預覽
              </Label>
                ...
                {rules.map(r => (
                  <SelectItem key={r.id} value={r.id}>
                    <span className="flex items-center gap-2">
                      {r.name}
                      <Badge variant={r.enabled ? "default" : "secondary"} className="text-xs">{r.enabled ? "啟用" : "停用"}</Badge>
                    </span>
                  </SelectItem>
                ))}
              </SelectContent>
...
export default function ClockInManagement() {
  ...
  // ── Location form ──
  const emptyLoc: ClockLocation = { id: "", name: "", address: "", lat: "", lng: "", radius: 200, wifiSSID: "", ruleId: "", enabled: true };
  const [locForm, setLocForm] = useState<ClockLocation>(emptyLoc);

  const openLocDialog = (loc?: ClockLocation) => {
    if (loc) { setEditingLoc(loc); setLocForm({ ...loc }); }
    else { setEditingLoc(null); setLocForm({ ...emptyLoc, id: `L${Date.now()}` }); }
    setLocDialog(true);
  };
  const saveLoc = () => {
    if (!locForm.name || !locForm.address) { toast.error("請填寫名稱與地址"); return; }
  ...
  const deleteLoc = (id: string) => { setLocations(prev => prev.filter(l => l.id !== id)); toast.success("已刪除打卡地點"); };

  // ── Schedule form ──
  const emptySched: ClockSchedule = { id: "", name: "", type: "fixed", workStart: "09:00", workEnd: "18:00", lateGrace: 5, earlyLeaveGrace: 5, breakStart: "12:00", breakEnd: "13:00", workDays: [1, 2, 3, 4, 5], enabled: true };
  const [schedForm, setSchedForm] = useState<ClockSchedule>(emptySched);

  const openSchedDialog = (s?: ClockSchedule) => {
    if (s) { setEditingSched(s); setSchedForm({ ...s }); }
    else { setEditingSched(null); setSchedForm({ ...emptySched, id: `S${Date.now()}` }); }
    setSchedDialog(true);
  };
  const saveSched = () => {
    if (!schedForm.name) { toast.error("請填寫班次名稱"); return; }
  ...
  const deleteSched = (id: string) => { setSchedules(prev => prev.filter(s => s.id !== id)); toast.success("已刪除班次"); };

  // ── Rule form ──
  const emptyRule: ClockRule = { id: "", name: "", allowMethods: ["gps"], requirePhoto: false, requireLocation: true, allowRemote: false, remoteApproval: false, overtimeAuto: true, overtimeMinMinutes: 30, missedClockAllowAppeal: true, appealDeadlineDays: 3, enabled: true };
  const [ruleForm, setRuleForm] = useState<ClockRule>(emptyRule);

  const openRuleDialog = (r?: ClockRule) => {
    if (r) { setEditingRule(r); setRuleForm({ ...r }); }
    else { setEditingRule(null); setRuleForm({ ...emptyRule, id: `R${Date.now()}` }); }
    setRuleDialog(true);
  };
  const saveRule = () => {
    if (!ruleForm.name) { toast.error("請填寫規則名稱"); return; }
  ...
  const deleteRule = (id: string) => { setRules(prev => prev.filter(r => r.id !== id)); toast.success("已刪除規則"); };
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="locations">
        <TabsList>
          <TabsTrigger value="locations" className="gap-1.5"><MapPin className="h-4 w-4" />打卡地點</TabsTrigger>
          <TabsTrigger value="schedules" className="gap-1.5"><Clock className="h-4 w-4" />班次時間</TabsTrigger>
          <TabsTrigger value="rules" className="gap-1.5"><Shield className="h-4 w-4" />打卡規則</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="locations">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Button size="sm" onClick={() => openLocDialog()}><Plus className="h-4 w-4 mr-1" />新增地點</Button>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {locations.map(loc => {
                    ...
                    <TableRow key={loc.id}>
                      ...
                      <TableCell className="text-sm text-muted-foreground">{loc.lat}, {loc.lng}</TableCell>
                      ...
                      <TableCell>
                        {associatedRule ? (
                          <Badge variant="outline" className="gap-1">
                            <Shield className="h-3 w-3" />{associatedRule.name}
                          </Badge>
                        ) : <span className="text-muted-foreground">—</span>}
                      </TableCell>
                      <TableCell><Badge variant={loc.enabled ? "default" : "secondary"}>{loc.enabled ? "啟用" : "停用"}</Badge></TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" onClick={() => openLocDialog(loc)}><Pencil className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" onClick={() => deleteLoc(loc.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                        </div>
                      </TableCell>
                    </TableRow>
                    );
        ...
        <TabsContent value="schedules">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Button size="sm" onClick={() => openSchedDialog()}><Plus className="h-4 w-4 mr-1" />新增班次</Button>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                    ...
                    <TableRow key={s.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          {s.type === "fixed" ? "固定班" : s.type === "flexible" ? "彈性班" : "輪班制"}
                        </Badge>
                      </TableCell>
                      <TableCell>{s.workStart}</TableCell>
                      <TableCell>{s.workEnd}</TableCell>
                      <TableCell className="text-sm text-muted-foreground">{s.breakStart}–{s.breakEnd}</TableCell>
                      ...
                      <TableCell><Badge variant={s.enabled ? "default" : "secondary"}>{s.enabled ? "啟用" : "停用"}</Badge></TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" onClick={() => openSchedDialog(s)}><Pencil className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" onClick={() => deleteSched(s.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <TabsContent value="rules">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Button size="sm" onClick={() => openRuleDialog()}><Plus className="h-4 w-4 mr-1" />新增規則</Button>
            </CardHeader>
            <CardContent>
                    ...
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" onClick={() => openRuleDialog(r)}><Pencil className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" onClick={() => deleteRule(r.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                          ...
                          <div className="flex gap-1 flex-wrap">
                            {r.allowMethods.map(m => (
                              <Badge key={m} variant="outline" className="text-xs">
                                {m === "gps" ? "GPS定位" : m === "wifi" ? "Wi-Fi" : m === "face" ? "人臉辨識" : m === "bluetooth" ? "藍牙" : m}
                              </Badge>
                            ))}
                          </div>
                        </div>
                        <div>
                          ...
                          <p className="text-foreground">{r.requirePhoto ? "需拍照" : "免拍照"} / {r.requireLocation ? "需定位" : "免定位"}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground mb-1">遠端打卡</p>
                          <p className="text-foreground">{r.allowRemote ? (r.remoteApproval ? "允許（需審批）" : "允許（免審批）") : "不允許"}</p>
                        </div>
                        <div>
      ...
      <Dialog open={schedDialog} onOpenChange={setSchedDialog}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div>
              <Label>班次名稱 *</Label>
              <Input value={schedForm.name} onChange={e => setSchedForm(p => ({ ...p, name: e.target.value }))} placeholder="例：標準班" />
            </div>
            <div>
              <Label>班次類型</Label>
              <Select value={schedForm.type} onValueChange={(v: "fixed" | "flexible" | "shift") => setSchedForm(p => ({ ...p, type: v }))}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>上班時間</Label><Input type="time" value={schedForm.workStart} onChange={e => setSchedForm(p => ({ ...p, workStart: e.target.value }))} /></div>
              <div><Label>下班時間</Label><Input type="time" value={schedForm.workEnd} onChange={e => setSchedForm(p => ({ ...p, workEnd: e.target.value }))} /></div>
              <div><Label>午休開始</Label><Input type="time" value={schedForm.breakStart} onChange={e => setSchedForm(p => ({ ...p, breakStart: e.target.value }))} /></div>
              <div><Label>午休結束</Label><Input type="time" value={schedForm.breakEnd} onChange={e => setSchedForm(p => ({ ...p, breakEnd: e.target.value }))} /></div>
              <div><Label>遲到寬限（分鐘）</Label><Input type="number" value={schedForm.lateGrace} onChange={e => setSchedForm(p => ({ ...p, lateGrace: Number(e.target.value) }))} /></div>
              <div><Label>早退寬限（分鐘）</Label><Input type="number" value={schedForm.earlyLeaveGrace} onChange={e => setSchedForm(p => ({ ...p, earlyLeaveGrace: Number(e.target.value) }))} /></div>
            </div>
            <div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSchedDialog(false)}>取消</Button>
      ...
      <Dialog open={ruleDialog} onOpenChange={setRuleDialog}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-5">
            <div>
              <Label>規則名稱 *</Label>
              <Input value={ruleForm.name} onChange={e => setRuleForm(p => ({ ...p, name: e.target.value }))} placeholder="例：預設打卡規則" />
            </div>

            <Separator />
            <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><Smartphone className="h-4 w-4" />打卡方式</h4>
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <label key={m.key} className="flex items-center gap-2 cursor-pointer">
                  <Checkbox checked={ruleForm.allowMethods.includes(m.key)} onCheckedChange={() => toggleMethod(m.key)} />
            ...
            <Separator />
            <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><Settings2 className="h-4 w-4" />驗證要求</h4>
            ...
            <Separator />
            <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><Building2 className="h-4 w-4" />遠端打卡</h4>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>允許遠端打卡</Label>
              ...
              {ruleForm.allowRemote && (
                <div className="flex items-center justify-between">
                  <Label>遠端打卡需審批</Label>
                  <Switch checked={ruleForm.remoteApproval} onCheckedChange={v => setRuleForm(p => ({ ...p, remoteApproval: v }))} />
                </div>
              )}
            </div>

            <Separator />
            <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5"><Clock className="h-4 w-4" />加班與補卡</h4>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>自動計算加班</Label>
                <Switch checked={ruleForm.overtimeAuto} onCheckedChange={v => setRuleForm(p => ({ ...p, overtimeAuto: v }))} />
              </div>
              {ruleForm.overtimeAuto && (
                <div>
                  <Label>最低加班時長（分鐘）</Label>
                  <Input type="number" value={ruleForm.overtimeMinMinutes} onChange={e => setRuleForm(p => ({ ...p, overtimeMinMinutes: Number(e.target.value) }))} />
                </div>
              )}
              <div className="flex items-center justify-between">
                <Label>允許補卡申請</Label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRuleDialog(false)}>取消</Button>
