...
export default function BonusPenaltyDetail() {
  ...
  const [editForm, setEditForm] = useState({
    ...
    amount: String(originalRecord?.amount || ""),
    applyMonth: originalRecord?.applyMonth || "",
  ...
  const infoItems = [
    ...
    { icon: DollarSign, label: "金額", value: isEditing ? null : `${record.type === "獎金" ? "+" : "-"}HK$ ${record.amount.toLocaleString()}`, color: record.type === "獎金" ? "text-success" : "text-destructive" },
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className={typeColors[record.type]}>{record.type}</Badge>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {infoItems.map(item => (
                <div key={item.label} className="flex items-center gap-4">
                  ...
                  {isEditing && item.label === "員工" && (
                    <Select value={editForm.employeeId} onValueChange={v => setEditForm(prev => ({ ...prev, employeeId: v }))}>
                      <SelectTrigger className="w-[280px] h-8"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {employeeOptions.map(e => <SelectItem key={e.id} value={e.id}>{e.name}（{e.id}）</SelectItem>)}
                      </SelectContent>
                    </Select>
                  )}
                  {isEditing && item.label === "金額" && (
                    <Input type="number" className="w-[160px] h-8" value={editForm.amount} onChange={e => setEditForm(prev => ({ ...prev, amount: e.target.value }))} />
                  )}
                  {isEditing && item.label === "計入月份" && (
                    <Select value={editForm.applyMonth} onValueChange={v => setEditForm(prev => ({ ...prev, applyMonth: v }))}>
                      <SelectTrigger className="w-[200px] h-8"><SelectValue /></SelectTrigger>
                      <SelectContent>
          ...
          <Card>
            ...
            <CardContent>
              {isEditing ? (
                <Textarea value={editForm.reason} onChange={e => setEditForm(prev => ({ ...prev, reason: e.target.value }))} rows={4} />
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              {isEditing ? (
                <Input value={editForm.note} onChange={e => setEditForm(prev => ({ ...prev, note: e.target.value }))} placeholder="選填備註" />
              ) : (
        ...
        <div className="space-y-4">
          <Card>
            <CardContent className="p-6 text-center">
              <div className={`text-3xl font-bold mb-2 ${record.type === "獎金" ? "text-success" : "text-destructive"}`}>
