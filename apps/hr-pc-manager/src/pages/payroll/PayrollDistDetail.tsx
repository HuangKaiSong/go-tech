...
import {
  ArrowLeft, Pencil, Download, Send, CheckCircle, DollarSign,
  Users, TrendingUp, TrendingDown, Search, Wallet, Ban,
  Building2, CreditCard, Clock, FileText, CalendarDays, User,
  XCircle, AlertCircle
} from "lucide-react";
...
export default function PayrollDistDetail() {
  ...
  const filtered = mockDistEmployees.filter(e =>
  ...
  const summaryCards = [
    ...
    { label: "扣款總額", value: `HK$ ${record.totalDeduction.toLocaleString()}`, icon: TrendingDown, color: "text-destructive" },
  ...
  const approvalTimeline = [
    ...
    { step: "提交審核", user: record.status !== "草稿" ? record.createdBy : "—", time: record.status !== "草稿" ? record.createdAt : "—", status: record.status === "草稿" ? "pending" : "completed" },
    { step: "審核通過", user: record.approvedBy || "—", time: record.approvedBy ? record.updatedAt : "—", status: ["已審核", "發放中", "已發放"].includes(record.status) ? "completed" : "pending" },
    { step: "執行發放", user: record.status === "已發放" ? "系統" : "—", time: record.status === "已發放" ? record.updatedAt : "—", status: record.status === "已發放" ? "completed" : "pending" },
  ];
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {canApprove && (
            <>
              <Button variant="outline" className="gap-2 text-destructive border-destructive/20 hover:bg-destructive/10" onClick={() => setRejectOpen(true)}>
                <XCircle className="h-4 w-4" /> 駁回
              </Button>
              <Button className="gap-2 bg-success text-success-foreground hover:bg-success/90" onClick={() => setApproveOpen(true)}>
                <CheckCircle className="h-4 w-4" /> 審核通過
              </Button>
            </>
          )}
          {canDistribute && (
            <Button className="gap-2 bg-success text-success-foreground hover:bg-success/90" onClick={() => toast.success("薪資已發放")}>
                    ...
                    <TableBody>
                      {filtered.map(emp => (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge variant="outline" className={empPayStatusColors[emp.payStatus]}>{emp.payStatus}</Badge>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{selectedEmployee.employeeId} · {selectedEmployee.position}</p>
                      </div>
                    </div>
                    <Separator />
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground flex items-center gap-1"><Building2 className="h-3.5 w-3.5" /> 部門</span>
                        <span className="font-medium">{selectedEmployee.department}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground flex items-center gap-1"><CreditCard className="h-3.5 w-3.5" /> 銀行</span>
                        <span className="font-medium">{selectedEmployee.bankName}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground flex items-center gap-1"><CreditCard className="h-3.5 w-3.5" /> 帳號</span>
                        <span className="font-medium font-mono">{selectedEmployee.bankAccount}</span>
                      </div>
                      <Separator />
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">發放狀態</span>
                        <Badge variant="outline" className={empPayStatusColors[selectedEmployee.payStatus]}>{selectedEmployee.payStatus}</Badge>
                      </div>
                      {selectedEmployee.payTime && selectedEmployee.payStatus === "已發放" && (
                        <>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> 發放時間</span>
                            <span className="font-medium">{selectedEmployee.payTime}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground flex items-center gap-1"><FileText className="h-3.5 w-3.5" /> 交易編號</span>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">HK$ {Math.round(record.totalNet / record.employeeCount).toLocaleString()}</span>
                </div>
                {record.approvedBy && (
                  <div className="flex justify-between text-sm">
              ...
              <div className="space-y-0">
                {approvalTimeline.map((step, i) => (
                  <div key={i} className="flex gap-4">
                    ...
                    <div className="pb-8">
                      <p className={`font-semibold ${step.status === "completed" ? "text-foreground" : "text-muted-foreground"}`}>{step.step}</p>
      ...
      {/* Approve Dialog */}
      <Dialog open={approveOpen} onOpenChange={() => { setApproveOpen(false); setApproveComment(""); }}>
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setApproveOpen(false); setApproveComment(""); }}>取消</Button>
            <Button className="gap-2 bg-success text-success-foreground hover:bg-success/90" onClick={() => { toast.success("已審核通過"); setApproveOpen(false); setApproveComment(""); }}>
      ...
      <Dialog open={rejectOpen} onOpenChange={() => { setRejectOpen(false); setRejectReason(""); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              駁回 {record.period} 的發薪批次，批次將退回草稿狀態
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea placeholder="請說明駁回原因..." value={rejectReason} onChange={e => setRejectReason(e.target.value)} rows={4} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setRejectOpen(false); setRejectReason(""); }}>取消</Button>
            <Button variant="destructive" className="gap-2" onClick={() => {
              if (!rejectReason.trim()) { toast.error("請填寫駁回原因"); return; }
              toast.success("已駁回"); setRejectOpen(false); setRejectReason("");
            }}>
